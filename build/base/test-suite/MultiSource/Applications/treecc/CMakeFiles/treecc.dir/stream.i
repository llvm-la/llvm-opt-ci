# 1 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Applications/treecc/stream.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 399 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Applications/treecc/stream.c" 2
# 21 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Applications/treecc/stream.c"
...
# 22 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Applications/treecc/stream.c" 2
...
# 23 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Applications/treecc/stream.c" 2
...
void TreeCCIncludeSkeleton(TreeCCContext *context, TreeCCStream *stream,
         const char *skeleton);
# 24 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Applications/treecc/stream.c" 2
...
# 25 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Applications/treecc/stream.c" 2
...
# 231 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Applications/treecc/stream.c"
int TreeCCStreamFlush(TreeCCStream *stream)
{
 int result;
 FILE *file;
 ...
 {
  if((file = fopen(stream->filename, "r")) != ((void*)0))
  {
   buffer = stream->firstBuf;
   changed = 0;
   while((size = fread(tempbuf, 1, 2048, file)) != 0)
   {
    if(!buffer)
    {
     changed = 1;
    ...
    else
    {
     ...
     if((memcmp((buffer->data),(tempbuf),(2048))) != 0)
     {
      changed = 1;
      break;
     }
 ...
 if((file = fopen(stream->filename, "w")) == ((void*)0))
 {
...
int TreeCCStreamFlushStdio(TreeCCStream *stream, FILE *file)
{
 ...
 while(buffer != 0)
 {
  ...
  {
   if(fwrite(buffer->data, 1, 2048, file) !=
     2048)
   {
    return 0;
   }
...
# 409 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Applications/treecc/stream.c"
  buf += templen;
  len -= templen;
  stream->posn += templen;
 }
...
# 436 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Applications/treecc/stream.c"
}
...
void TreeCCStreamPrint(TreeCCStream *stream, const char *format, ...)
{
 ...
 va_list va; __builtin_va_start(va, format);

 vsnprintf(tempbuf, sizeof(tempbuf), format, va);
...
# 489 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Applications/treecc/stream.c"
void TreeCCStreamCodeIndent(TreeCCStream *stream, char *code, int indent)
{
 int temp;
 while(*code != '\0')
 {
  do { if((stream)->posn < 2048) { (stream)->lastBuf->data[((stream)->posn)++] = (*code); (stream)->dirty = 1; } else { _StreamPut((*code), (stream)); } } while (0);
  if(*code == '\n')
  {
   ++(stream->linenum);
   for(temp = 0; temp < indent; ++temp)
   {
    do { if((stream)->posn < 2048) { (stream)->lastBuf->data[((stream)->posn)++] = ('\t'); (stream)->dirty = 1; } else { _StreamPut(('\t'), (stream)); } } while (0);
   }
  }
  ++code;
 }
          ...
          char indentchar, int indent)
{
 ...
 while(*code != '\0')
 {
  ...
  if(*code == '\n')
  {
   ...
   for(temp = 0; temp < indent; ++temp)
   {
    do { if((stream)->posn < 2048) { (stream)->lastBuf->data[((stream)->posn)++] = (indentchar); (stream)->dirty = 1; } else { _StreamPut((indentchar), (stream)); } } while (0);
   }
  }
  ++code;
 }
...
static void OutputMacroName(TreeCCStream *stream, const char *filename)
{
 while(*filename != '\0')
 {
  ...
  {
   do { if((stream)->posn < 2048) { (stream)->lastBuf->data[((stream)->posn)++] = (*filename); (stream)->dirty = 1; } else { _StreamPut((*filename), (stream)); } } while (0);
  }
  else
  {
   do { if((stream)->posn < 2048) { (stream)->lastBuf->data[((stream)->posn)++] = ('_'); (stream)->dirty = 1; } else { _StreamPut(('_'), (stream)); } } while (0);
  }
  ++filename;
 }
 do { if((stream)->posn < 2048) { (stream)->lastBuf->data[((stream)->posn)++] = ('\n'); (stream)->dirty = 1; } else { _StreamPut(('\n'), (stream)); } } while (0);
 ++(stream->linenum);
}
