# 1 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Applications/d/arg.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 399 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Applications/d/arg.c" 2
...
void d_version(char *);
# 5 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Applications/d/arg.c" 2
      ...
      switch (type) {
        ...
        case 'D':
          *(double *)desc[i].location = atof(arg);
          break;
        case 'L':
          *(int64 *)desc[i].location = atol(arg);
          break;
...
process_args(ArgumentState *arg_state, char **argv) {
 ...
 case 'F':
   break;
 case 'I': *(int *)desc[i].location = strtol(env, ((void*)0), 0); break;
 case 'D': *(double *)desc[i].location = strtod(env, ((void*)0)); break;
 case 'L': *(int64 *)desc[i].location = strtoll(env, ((void*)0), 0); break;
 case 'S': strncpy((char *)desc[i].location,env,
     strtol(desc[i].type+1, ((void*)0), 0));
   break;
      }
      if (desc[i].pfn)
  ...
  while ( *++argv ) {
       ...
       !strncmp(desc[i].name,(*argv)+2, len))
   {
     if (!end)
      ...
      arg_state->file_argument[arg_state->nfile_arguments] = ((void*)0);
    }
  }
}
