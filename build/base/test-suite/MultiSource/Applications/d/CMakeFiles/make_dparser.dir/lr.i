# 1 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Applications/d/lr.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 399 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Applications/d/lr.c" 2
...
void d_version(char *);
# 6 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Applications/d/lr.c" 2
...
insert_item(State *s, Elem *e) {
  ...
  if (set_add(&s->items_hash, i)) {
    do { if (!(&s->items)->v) { ((&s->items)->v = (&s->items)->e)[(&s->items)->n++] = (i); break; } else if ((&s->items)->v == ((&s->items)->e)) { if (((&s->items)->n < 3)) { (&s->items)->v[(&s->items)->n++] = (i); break; } } else if ((&s->items)->n & ((1 << 3) - 1)) { (&s->items)->v[(&s->items)->n++] = (i); break; } vec_add_internal((&s->items), i); } while (0);
    return 1;
  }
  return 0;
}
...
itemcmp(const void *ai, const void *aj) {
  uint i = (((uint)(*(Item**)ai)->rule->index << 8) + ((uint)((*(Item**)ai)->kind != ELEM_END ? (*(Item**)ai)->index : (*(Item**)ai)->rule->elems.n)));
  uint j = (((uint)(*(Item**)aj)->rule->index << 8) + ((uint)((*(Item**)aj)->kind != ELEM_END ? (*(Item**)aj)->index : (*(Item**)aj)->rule->elems.n)));
...
static void
free_state(State *s) {
  do { if ((&s->items)->v && (&s->items)->v != (&s->items)->e) free((&s->items)->v); do { (&s->items)->n = 0; (&s->items)->v = 0; } while(0); } while(0);
  do { if ((&s->items_hash)->v && (&s->items_hash)->v != (&s->items_hash)->e) free((&s->items_hash)->v); do { (&s->items_hash)->n = 0; (&s->items_hash)->v = 0; } while(0); } while(0);
  free(s);
}
...
maybe_add_state(Grammar *g, State *s) {
 ...
 if (s->items.v[j] != g->states.v[i]->items.v[j])
  ...
  s->index = g->states.n;
  do { if (!(&g->states)->v) { ((&g->states)->v = (&g->states)->e)[(&g->states)->n++] = (s); break; } else if ((&g->states)->v == ((&g->states)->e)) { if (((&g->states)->n < 3)) { (&g->states)->v[(&g->states)->n++] = (s); break; } } else if ((&g->states)->n & ((1 << 3) - 1)) { (&g->states)->v[(&g->states)->n++] = (s); break; } vec_add_internal((&g->states), s); } while (0);
  return s;
}
...
build_closure(Grammar *g, State *s) {
 ...
 insert_item(s, pp->rules.v[k]->elems.v ?
  ...
  s->hash = 0;
  for (j = 0; j < s->items.n; j++)
    s->hash += (((uint)(s->items.v[j])->rule->index << 8) + ((uint)((s->items.v[j])->kind != ELEM_END ? (s->items.v[j])->index : (s->items.v[j])->rule->elems.n)));
...
add_goto(State *s, State *ss, Elem *e) {
  ...
  g->state = ss;
  g->elem = clone_elem(e);
  do { if (!(&s->gotos)->v) { ((&s->gotos)->v = (&s->gotos)->e)[(&s->gotos)->n++] = (g); break; } else if ((&s->gotos)->v == ((&s->gotos)->e)) { if (((&s->gotos)->n < 3)) { (&s->gotos)->v[(&s->gotos)->n++] = (g); break; } } else if ((&s->gotos)->n & ((1 << 3) - 1)) { (&s->gotos)->v[(&s->gotos)->n++] = (g); break; } vec_add_internal((&s->gotos), g); } while (0);
}
    ...
    Rule *arule, State *astate)
{
 ...
 return;
    ...
    do { if (!(&s->reduce_actions)->v) { ((&s->reduce_actions)->v = (&s->reduce_actions)->e)[(&s->reduce_actions)->n++] = (a); break; } else if ((&s->reduce_actions)->v == ((&s->reduce_actions)->e)) { if (((&s->reduce_actions)->n < 3)) { (&s->reduce_actions)->v[(&s->reduce_actions)->n++] = (a); break; } } else if ((&s->reduce_actions)->n & ((1 << 3) - 1)) { (&s->reduce_actions)->v[(&s->reduce_actions)->n++] = (a); break; } vec_add_internal((&s->reduce_actions), a); } while (0);
  } else {
 ...
 return;
    a = new_Action(g, akind, aterm, arule, astate);
    do { if (!(&s->shift_actions)->v) { ((&s->shift_actions)->v = (&s->shift_actions)->e)[(&s->shift_actions)->n++] = (a); break; } else if ((&s->shift_actions)->v == ((&s->shift_actions)->e)) { if (((&s->shift_actions)->n < 3)) { (&s->shift_actions)->v[(&s->shift_actions)->n++] = (a); break; } } else if ((&s->shift_actions)->n & ((1 << 3) - 1)) { (&s->shift_actions)->v[(&s->shift_actions)->n++] = (a); break; } vec_add_internal((&s->shift_actions), a); } while (0);
  }
}
...
build_actions(Grammar *g) {
 ...
 add_action(g, s, ACTION_REDUCE, ((void*)0), e->rule, 0);
      else
 s->accept = 1;
    }
...
build_right_epsilon_hints(Grammar *g) {
 ...
 if (ss && r->elems.n)
   do { if (!(&s->right_epsilon_hints)->v) { ((&s->right_epsilon_hints)->v = (&s->right_epsilon_hints)->e)[(&s->right_epsilon_hints)->n++] = (new_Hint(r->elems.n - e->index - 1, ss, r)); break; } else if ((&s->right_epsilon_hints)->v == ((&s->right_epsilon_hints)->e)) { if (((&s->right_epsilon_hints)->n < 3)) { (&s->right_epsilon_hints)->v[(&s->right_epsilon_hints)->n++] = (new_Hint(r->elems.n - e->index - 1, ss, r)); break; } } else if ((&s->right_epsilon_hints)->n & ((1 << 3) - 1)) { (&s->right_epsilon_hints)->v[(&s->right_epsilon_hints)->n++] = (new_Hint(r->elems.n - e->index - 1, ss, r)); break; } vec_add_internal((&s->right_epsilon_hints), new_Hint(r->elems.n - e->index - 1, ss, r)); } while (0);
...
build_error_recovery(Grammar *g) {
 ...
 do { if (!(&s->error_recovery_hints)->v) { ((&s->error_recovery_hints)->v = (&s->error_recovery_hints)->e)[(&s->error_recovery_hints)->n++] = (new_Hint(depth, ((void*)0), r)); break; } else if ((&s->error_recovery_hints)->v == ((&s->error_recovery_hints)->e)) { if (((&s->error_recovery_hints)->n < 3)) { (&s->error_recovery_hints)->v[(&s->error_recovery_hints)->n++] = (new_Hint(depth, ((void*)0), r)); break; } } else if ((&s->error_recovery_hints)->n & ((1 << 3) - 1)) { (&s->error_recovery_hints)->v[(&s->error_recovery_hints)->n++] = (new_Hint(depth, ((void*)0), r)); break; } vec_add_internal((&s->error_recovery_hints), new_Hint(depth, ((void*)0), r)); } while (0);
      Ldone:;
      }
    }
