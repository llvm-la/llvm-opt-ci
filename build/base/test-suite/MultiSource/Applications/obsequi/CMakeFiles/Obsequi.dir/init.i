# 1 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Applications/obsequi/init.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 399 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Applications/obsequi/init.c" 2
...
# 3 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Applications/obsequi/init.c" 2
...
initialize_solver()
{
  ...
  if(g_trans_table == ((void*)0)){


    g_trans_table = calloc((1 << 23),sizeof(Hash_Entry));


    srandom(1);
    for(i=1; i<31; i++) {
      for(j=1; j<31; j++) {
        g_zobrist[i][j] = random() & ((1 << 23) - 1);
      }
    }
...
initialize_board(s32bit num_rows, s32bit num_cols, s32bit board[30][30])
{
  ...
  if(num_rows > 30 || num_rows < 1 || num_cols > 30 || num_cols < 1)
    ((_fatal_error_aux("/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Applications/obsequi/init.c", 102, ((1) ? (1) : 256), "Invalid board size %dX%d.\n", num_rows, num_cols )));

  if(num_rows * num_cols >= 128)
    ((_fatal_error_aux("/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Applications/obsequi/init.c", 105, ((1) ? (1) : 256), "Invalid board size %dX%d.\n", num_rows, num_cols )));


  if(g_trans_table == ((void*)0) || g_board_size[0] != num_rows
     || g_board_size[1] != num_cols) init = 1;

  g_board_size[0] = num_rows;
  g_board_size[1] = num_cols;

  if(init) initialize_solver();


  for(i = 0; i < 32; i++){
    g_board[0][i] = 0xFFFFFFFF;
    g_board[1][i] = 0xFFFFFFFF;
  }
  ...
  for(i = 0; i < num_rows; i++)
    for(j = 0; j < num_cols; j++)
      if(board[i][j] != 0){
        ...
        index = (i*num_cols)+j;
        g_norm_hashkey.key[index/32] |= ( 1 << (index%32) );

        index = (i*num_cols) + (num_cols - j - 1);
        g_flipV_hashkey.key[index/32] |= ( 1 << (index%32) );

        index = ( (num_rows - i - 1) *num_cols) + j;
        g_flipH_hashkey.key[index/32] |= ( 1 << (index%32) );

        index = ( (num_rows - i - 1) *num_cols) + (num_cols - j - 1);
        g_flipVH_hashkey.key[index/32] |= ( 1 << (index%32) );
      }
