# 1 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Applications/ClamAV/libclamav_matcher-ac.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 399 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Applications/ClamAV/libclamav_matcher-ac.c" 2
# 20 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Applications/ClamAV/libclamav_matcher-ac.c"
...
# 21 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Applications/ClamAV/libclamav_matcher-ac.c" 2
...
# 24 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Applications/ClamAV/libclamav_matcher-ac.c" 2
...
extern size_t strlcat (char *__restrict __dest,
         const char *__restrict __src, size_t __n)
  __attribute__ ((__nothrow__ )) __attribute__ ((__nonnull__ (1, 2))) ;
# 25 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Applications/ClamAV/libclamav_matcher-ac.c" 2
...
# 26 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Applications/ClamAV/libclamav_matcher-ac.c" 2
...
# 28 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Applications/ClamAV/libclamav_matcher-ac.c" 2
...
# 31 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Applications/ClamAV/libclamav_matcher-ac.c" 2
...
# 32 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Applications/ClamAV/libclamav_matcher-ac.c" 2
...
# 33 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Applications/ClamAV/libclamav_matcher-ac.c" 2
...
# 37 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Applications/ClamAV/libclamav_matcher-ac.c" 2

uint8_t cli_ac_mindepth = 2;
uint8_t cli_ac_maxdepth = 3;
...
int cli_ac_addpatt(struct cli_matcher *root, struct cli_ac_patt *pattern)
{
 ...
 uint8_t i, match;
 uint16_t len = (((root->ac_maxdepth)<(pattern->length))?(root->ac_maxdepth):(pattern->length));


    for(i = 0; i < len; i++) {
 if(pattern->pattern[i] & 0xff00) {
     len = i;
     break;
 }
    }
...
{
 struct bfs_list *bfs = ((void*)0), *bfs_last = ((void*)0);
...
# 434 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Applications/ClamAV/libclamav_matcher-ac.c"
inline static int ac_findmatch(const unsigned char *buffer, uint32_t offset, uint32_t length, const struct cli_ac_patt *pattern, uint32_t *end)
{
 ...
 switch(wc = pattern->pattern[i] & 0xff00) { case 0x0000: if((unsigned char) pattern->pattern[i] != buffer[bp]) return 0; break; case 0x0100: break; case 0x0200: found = 0; alt = pattern->alttable[altcnt]; if(alt->chmode) { for(j = 0; j < alt->num; j++) { if(alt->str[j] == buffer[bp]) { found = 1; break; } } } else { while(alt) { if(bp + alt->len <= length) { if(!memcmp(&buffer[bp], alt->str, alt->len)) { found = 1; bp += alt->len - 1; break; } } alt = alt->next; } } if(!found) return 0; altcnt++; break; case 0x0300: if((unsigned char) (pattern->pattern[i] & 0x00f0) != (buffer[bp] & 0xf0)) return 0; break; case 0x0400: if((unsigned char) (pattern->pattern[i] & 0x000f) != (buffer[bp] & 0x0f)) return 0; break; default: cli_errmsg("ac_findmatch: Unknown wildcard 0x%x\n", wc); return 0; };
 bp++;
    }
    *end = bp;
 ...
 for(i = 0; i < pattern->prefix_length; i++) {
     switch(wc = pattern->prefix[i] & 0xff00) { case 0x0000: if((unsigned char) pattern->prefix[i] != buffer[bp]) return 0; break; case 0x0100: break; case 0x0200: found = 0; alt = pattern->alttable[altcnt]; if(alt->chmode) { for(j = 0; j < alt->num; j++) { if(alt->str[j] == buffer[bp]) { found = 1; break; } } } else { while(alt) { if(bp + alt->len <= length) { if(!memcmp(&buffer[bp], alt->str, alt->len)) { found = 1; bp += alt->len - 1; break; } } alt = alt->next; } } if(!found) return 0; altcnt++; break; case 0x0300: if((unsigned char) (pattern->prefix[i] & 0x00f0) != (buffer[bp] & 0xf0)) return 0; break; case 0x0400: if((unsigned char) (pattern->prefix[i] & 0x000f) != (buffer[bp] & 0x0f)) return 0; break; default: cli_errmsg("ac_findmatch: Unknown wildcard 0x%x\n", wc); return 0; };
     bp++;
 }
    }
 ...
 if(current->final) {
  ...
  if(ac_findmatch(buffer, bp, length, patt, &matchend)) {
   ...
   if(pt->sigid) {
    ...
    mdata->offmatrix[pt->sigid - 1][0] = cli_malloc(pt->parts * (8 + 1) * sizeof(int32_t));
    if(!mdata->offmatrix[pt->sigid - 1][0]) {
        ...
        mdata->offmatrix[pt->sigid - 1] = ((void*)0);
        return -114;
    }
    memset(mdata->offmatrix[pt->sigid - 1][0], -1, pt->parts * (8 + 1) * sizeof(int32_t));
    mdata->offmatrix[pt->sigid - 1][0][0] = 0;
    for(j = 1; j < pt->parts; j++) {
        mdata->offmatrix[pt->sigid - 1][j] = mdata->offmatrix[pt->sigid - 1][0] + j * (8 + 1);
       ...
       if(pt->partno != 1) {
    found = 0;
    for(j = 1; j <= 8 && offmatrix[pt->partno - 2][j] != -1; j++) {
        found = 1;
        if(pt->maxdist)
    ...
    if(pt->type) {
     ...
     if(pt->type > type || pt->type >= CL_TYPE_SFX || pt->type == CL_TYPE_MSEXE) {
         ...
         type = pt->type;
         if(ftoffset && (!*ftoffset || (*ftoffset)->cnt < 10) && ((ftype == CL_TYPE_MSEXE && type >= CL_TYPE_SFX) || ((ftype == CL_TYPE_MSEXE || ftype == CL_TYPE_ZIP) && type == CL_TYPE_MSEXE))) {

      for(j = 1; j <= 8 && offmatrix[0][j] != -1; j++) {
       ...
       return -114;
         ...
         memset(offmatrix[0], -1, pt->parts * (8 + 1) * sizeof(int32_t));
...
int cli_ac_addsig(struct cli_matcher *root, const char *virname, const char *hexsig, uint32_t sigid, uint16_t parts, uint16_t partno, uint16_t type, uint32_t mindist, uint32_t maxdist, const char *offset, uint8_t target)
{
 ...
 return -117;
    ...
    if((new = (struct cli_ac_patt *) cli_calloc(1, sizeof(struct cli_ac_patt))) == ((void*)0))
 return -114;
 ...
 if(error) {
  ...
  ac_free_alt(new);
    ...
    if((new->pattern = cli_hex2ui(new->alt ? hex : hexsig)) == ((void*)0)) {
 if(new->alt) {
     free(hex);
     ac_free_alt(new);
 }
 ...
 return -116;
    ...
    if((new->virname = cli_calloc(namelen + 1, sizeof(char))) == ((void*)0)) {
 if(new->prefix)
     free(new->prefix);
 else
     free(new->pattern);
