# 1 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Applications/ClamAV/libclamav_unzip.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 399 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Applications/ClamAV/libclamav_unzip.c" 2
# 25 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Applications/ClamAV/libclamav_unzip.c"
...
# 26 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Applications/ClamAV/libclamav_unzip.c" 2
...
# 29 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Applications/ClamAV/libclamav_unzip.c" 2
...
extern size_t strlcat (char *__restrict __dest,
         const char *__restrict __src, size_t __n)
  __attribute__ ((__nothrow__ )) __attribute__ ((__nonnull__ (1, 2))) ;
# 30 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Applications/ClamAV/libclamav_unzip.c" 2
...
# 31 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Applications/ClamAV/libclamav_unzip.c" 2
...
# 32 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Applications/ClamAV/libclamav_unzip.c" 2
...
# 34 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Applications/ClamAV/libclamav_unzip.c" 2
...
# 38 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Applications/ClamAV/libclamav_unzip.c" 2
...
# 40 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Applications/ClamAV/libclamav_unzip.c" 2
...
# 41 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Applications/ClamAV/libclamav_unzip.c" 2
...
# 42 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Applications/ClamAV/libclamav_unzip.c" 2
# 51 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Applications/ClamAV/libclamav_unzip.c"
inline static void __fixup_rootseek(off_t offset_of_trailer, struct zip_disk_trailer *trailer)
{
    if((off_t) (trailer->z_rootseek) >
 offset_of_trailer - (off_t) (trailer->z_rootsize) &&
 offset_of_trailer > (off_t) (trailer->z_rootsize))
     trailer->z_rootseek = (uint32_t) (offset_of_trailer - (trailer->z_rootsize));
}
...
static int __zip_find_disk_trailer(int fd, off_t filesize, struct zip_disk_trailer *trailer, off_t *start)
{
 ...
 return -111;
    ...
    if(filesize < ((ssize_t)(sizeof(struct zip_disk_trailer)))) {
 cli_errmsg("Unzip: __zip_find_disk_trailer: File too short\n");
 return -124;
    }

    if(!(buf = cli_malloc(1024)))
 return -114;
 ...
 if(offset >= 1024) {
  ...
  offset -= 1024 - sizeof(struct zip_disk_trailer);
 ...
 for(tail = end - 1; tail >= buf; tail--) {
     if((*tail == 'P') && (end - tail >= ((ssize_t)(sizeof(struct zip_disk_trailer))) - 2) && cli_readint32(tail) == 0x06054b50) {
  if(end - tail >= ((ssize_t)(sizeof(struct zip_disk_trailer)))) {
  ...
  for(i = 0; i < 2; i++) {
   ...
   if(lseek(fd, u_rootseek + shift, 0) < 0) {
       cli_errmsg("Unzip: __zip_find_disk_trailer: Can't lseek descriptor %d\n", fd);
       free(buf);
       return -123;
   }

   if(cli_readn(fd, &dirent, sizeof(dirent)) < ((ssize_t)(sizeof(dirent)))) {
       cli_errmsg("Unzip: __zip_find_disk_trailer: Can't read %u bytes\n", (unsigned int) bufsize);
       free(buf);
       return -123;
   }

   if((dirent.z_magic) == 0x02014b50) {
...
static int __zip_parse_root_directory(int fd, struct zip_disk_trailer *trailer, zip_dir_hdr **hdr_return, off_t start)
{
 ...
 uint16_t *p_reclen = ((void*)0), entries;
 uint32_t offset;
 struct stat sb;
 uint16_t u_entries = (trailer->z_entries);
 uint32_t u_rootsize = (trailer->z_rootsize);
 uint32_t u_rootseek = (trailer->z_rootseek) + start;
 ...
 if(lseek(fd, u_rootseek + offset, 0) < 0) {
     free(hdr0);
 ...
 u_flags = (d->z_flags);
        ...
        hdr->d_crc32 = (d->z_crc32);
        hdr->d_csize = (d->z_csize);
        hdr->d_usize = (d->z_usize);
        hdr->d_off = (d->z_off) + start;

        hdr->d_compr = (d->z_compr);
...
static int __zip_inflate_init(zip_file *fp, zip_dir_hdr *hdr)
{
 ...
 fp->restlen = hdr->d_usize;
    ...
    if(fp->method) {
        ...
        ret = inflateInit2_((&fp->d_stream), (-15), "1.2.11", (int)sizeof(z_stream));
...
zip_file *zip_file_open(zip_dir *dir, const char *name, int d_off)
{
 ...
 hdr_name = hdr->d_name;
  ...
  dir->cache.buf32k = ((void*)0);
     } else {
                fp->buf32k = (char *) cli_malloc(32768);
                if(!fp->buf32k) {
      dir->errcode = -114;
      zip_file_close(fp);
      return ((void*)0);
  ...
  return ((void*)0);
     ...
     if(dataoff < ((ssize_t)(sizeof(struct zip_file_header)))) {
  ...
  return ((void*)0);
     ...
     dataoff = (hp->z_namlen) + (hp->z_extras);

            if(lseek(dir->fd, dataoff, 1) < 0) {
  cli_errmsg("Unzip: zip_file_open: Can't lseek descriptor %d\n", dir->fd);
  dir->errcode = -123;
  zip_file_close(fp);
  return ((void*)0);
...
ssize_t zip_file_read(zip_file *fp, char *buf, size_t len)
{
 ...
 case 9:
  ...
  if(fp->crestlen > 0 && fp->d_stream.avail_in == 0) {
      size_t cl = (fp->crestlen < 32768 ? fp->crestlen : 32768);
