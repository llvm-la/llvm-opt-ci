# 1 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Benchmarks/DOE-ProxyApps-C/SimpleMOC/tracks.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 399 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Benchmarks/DOE-ProxyApps-C/SimpleMOC/tracks.c" 2
...
# 2 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Benchmarks/DOE-ProxyApps-C/SimpleMOC/tracks.c" 2
...
float * generate_polar_angles( Input I )
{
 ...
 for( int i = 0; i < I.n_polar_angles; i++)
  polar_angles[i] = 3.14159265358979323846 * (i + 0.5) / I.n_polar_angles;
 ...
 *nbytes += I->ntracks_2D * sizeof(Track2D);
 ...
 for (int i=0; i < I->n_azimuthal; i++)
 {
  for (int j=0; j < num_tracks[i]; j++)
  {
   ...
   fseek( in, 5*sizeof(double) + sizeof(int), 1);
   ...
   for (int s=0; s < num_segments; s++)
   {
    ...
    fseek( in, 2*sizeof(int) + sizeof(double), 1);
    if (CMFD_obj)
     fseek( in, 2*sizeof(int), 1);
   }
  }
 }
