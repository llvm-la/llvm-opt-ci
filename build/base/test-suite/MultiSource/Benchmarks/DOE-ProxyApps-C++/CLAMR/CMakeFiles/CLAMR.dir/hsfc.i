# 1 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Benchmarks/DOE-ProxyApps-C++/CLAMR/hsfc.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 399 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Benchmarks/DOE-ProxyApps-C++/CLAMR/hsfc.c" 2
# 54 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Benchmarks/DOE-ProxyApps-C++/CLAMR/hsfc.c"
...
# 55 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Benchmarks/DOE-ProxyApps-C++/CLAMR/hsfc.c" 2
...
# 56 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Benchmarks/DOE-ProxyApps-C++/CLAMR/hsfc.c" 2
  ...
  unsigned key[] )
{
  ...
  const unsigned NBits = ( ( sizeof(unsigned) * 8 ) * NKey ) / 2 ;
  ...
  for ( i = 1 ; i <= NBits ; i++ ) {
    const unsigned s = ( sizeof(unsigned) * 8 ) - i ;
    const unsigned c = gray_inv[ reflect ^ (
      ( ( ( coord[0] >> s ) & 01 ) << order[0] ) |
      ( ( ( coord[1] >> s ) & 01 ) << order[1] ) ) ];

    const unsigned off = 2 * i ;
    const unsigned which = off / ( sizeof(unsigned) * 8 ) ;
    const unsigned shift = ( sizeof(unsigned) * 8 ) - off % ( sizeof(unsigned) * 8 ) ;
  ...
  unsigned key[] )
{
  ...
  const unsigned NBits = ( ( sizeof(unsigned) * 8 ) * NKey ) / 3 ;
  ...
  for ( i = 1 ; i <= NBits ; i++ ) {
    ...
    for ( n = 0 ; n < 3 ; ++n ) {
      const unsigned bit = 01 & ( c >> ( 2 - n ) );
      const unsigned off = 3 * i + n ;
      const unsigned which = off / ( sizeof(unsigned) * 8 ) ;
      const unsigned shift = ( sizeof(unsigned) * 8 ) - off % ( sizeof(unsigned) * 8 ) ;

      if ( ( sizeof(unsigned) * 8 ) == shift ) {
        key[ which - 1 ] |= bit ;
      }
      else {
        key[ which ] |= bit << shift ;
