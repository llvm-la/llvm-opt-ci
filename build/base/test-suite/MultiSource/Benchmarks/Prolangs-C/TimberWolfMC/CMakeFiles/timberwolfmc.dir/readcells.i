# 1 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Benchmarks/Prolangs-C/TimberWolfMC/readcells.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 399 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Benchmarks/Prolangs-C/TimberWolfMC/readcells.c" 2
...
# 2 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Benchmarks/Prolangs-C/TimberWolfMC/readcells.c" 2
int perim ;
extern int totPins ;
...
kArray = (KBOXPTR) malloc( (50 + 1) * sizeof( KBOX ));
for( k = 1 ; k <= 50 ; k++ ) {
    kArray[ k ].cap = 0 ;
    kArray[ k ].HV = 0 ;
    kArray[ k ].sp = 0 ;
...
while( fscanf( fp , " %s " , input ) == 1 ) {
 ...
 for( k = 1 ; k <= A[0].xc ; k++ ) {
     if( k < A[0].xc ) {
  if( k % 2 == 1 ) {
      pSideArray[k].length = ( (A[k + 1].yc - A[k].yc)>=0 ? (A[k + 1].yc - A[k].yc) : -(A[k + 1].yc - A[k].yc) ) ;
      pSideArray[k].vertical = 1 ;
      pSideArray[k].pincount = 0 ;
      pSideArray[k].position = A[k].xc ;
  } else {
      pSideArray[k].length = ( (A[k + 1].xc - A[k].xc)>=0 ? (A[k + 1].xc - A[k].xc) : -(A[k + 1].xc - A[k].xc) ) ;
      pSideArray[k].vertical = 0 ;
      pSideArray[k].pincount = 0 ;
      pSideArray[k].position = A[k].yc ;
  }
     } else {
  pSideArray[k].length = ( (A[1].xc - A[k].xc)>=0 ? (A[1].xc - A[k].xc) : -(A[1].xc - A[k].xc) ) ;
  pSideArray[k].vertical = 0 ;
  pSideArray[k].pincount = 0 ;
  pSideArray[k].position = A[k].yc ;
     }
 ...
 tile = ptr->config[ 0 ] = (TILEBOXPTR) malloc(
         sizeof( TILEBOX ) ) ;
 tile->nexttile = (TILEBOXPTR) ((void*)0) ;
 tile->termptr = (TERMBOXPTR) ((void*)0) ;
 tile->siteLocArray = (LOCBOX *) ((void*)0) ;
 xcenter = (maxx + minx) / 2 ;
 ycenter = (maxy + miny) / 2 ;
 ...
 if( ptr->softflag == 1 ) {
  ...
  if( ptr->config[0]->siteLocArray[m].xpos == xpos ) {
      ...
      hit = 1 ;
      distant = ( (ptr->config[0]-> siteLocArray[m].ypos - ypos)>=0 ? (ptr->config[0]-> siteLocArray[m].ypos - ypos) : -(ptr->config[0]-> siteLocArray[m].ypos - ypos) ) ;


      for( m++ ; m <= ptr->numsites ; m++ ) {
   if(ptr->config[0]->siteLocArray[m].xpos ==
          xpos ) {
       if( ( (ptr->config[0]-> siteLocArray[m].ypos - ypos)>=0 ? (ptr->config[0]-> siteLocArray[m].ypos - ypos) : -(ptr->config[0]-> siteLocArray[m].ypos - ypos) ) < distant ) {
   ...
   exit(0);
      ...
      hit = 1 ;
      distant = ( (ptr->config[0]-> siteLocArray[m].xpos - xpos)>=0 ? (ptr->config[0]-> siteLocArray[m].xpos - xpos) : -(ptr->config[0]-> siteLocArray[m].xpos - xpos) ) ;


      for( m++ ; m <= ptr->numsites ; m++ ) {
   if(ptr->config[0]->siteLocArray[m].ypos ==
          ypos ) {
       if( ( (ptr->config[0]-> siteLocArray[m].xpos - xpos)>=0 ? (ptr->config[0]-> siteLocArray[m].xpos - xpos) : -(ptr->config[0]-> siteLocArray[m].xpos - xpos) ) < distant ) {
 ...
 if( hit == 0 ) {
  ...
  term = term->nextterm = (TERMBOXPTR) malloc(
     ...
     term->nextterm = (TERMBOXPTR) ((void*)0) ;
     term->xpos = xpos ;
     term->ypos = ypos ;
     term->oxpos = xpos ;
     term->oypos = ypos ;
