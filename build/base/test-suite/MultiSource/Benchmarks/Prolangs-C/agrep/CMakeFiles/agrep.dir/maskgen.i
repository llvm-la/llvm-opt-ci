# 1 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Benchmarks/Prolangs-C/agrep/maskgen.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 399 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Benchmarks/Prolangs-C/agrep/maskgen.c" 2
...
# 2 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Benchmarks/Prolangs-C/agrep/agrep.h" 2
...
extern size_t strlcat (char *__restrict __dest,
         const char *__restrict __src, size_t __n)
  __attribute__ ((__nothrow__ )) __attribute__ ((__nonnull__ (1, 2))) ;
# 3 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Benchmarks/Prolangs-C/agrep/agrep.h" 2
...
# 4 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Benchmarks/Prolangs-C/agrep/agrep.h" 2
...
# 3 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Benchmarks/Prolangs-C/agrep/maskgen.c" 2
...
{
struct term { int flag; unsigned char class1[32];
            } position[32 +10];
unsigned char c;
...
for(i=0; i<32; i++) position[i].class1[0] = '\0';
for(i=0; i<32; i++) position[i].flag = 0;
wildmask = NO_ERR_MASK = endposition = 0;
No_error = 0;
M = strlen(Pattern);
if(NOUPPER) {
              for(i=0; i<M; i++) if(((*__ctype_b_loc ())[(int) ((Pattern[i]))] & (unsigned short int) _ISalpha))
                     if (((*__ctype_b_loc ())[(int) ((Pattern[i]))] & (unsigned short int) _ISupper)) Pattern[i] = (__extension__ ({ int __res; if (sizeof (Pattern[i]) > 1) { if (__builtin_constant_p (Pattern[i])) { int __c = (Pattern[i]); __res = __c < -128 || __c > 255 ? __c : (*__ctype_tolower_loc ())[__c]; } else __res = tolower (Pattern[i]); } else __res = (*__ctype_tolower_loc ())[(int) (Pattern[i])]; __res; }));
            }
...
for (i=0, j=1; i< M; i++)
{
  switch (Pattern[i])
  {
    ...
    case 242: break;
    case 243: break;
    case 246 : No_error = 1; EVEN++;
                  break;
    case 247 : No_error = 0; EVEN--;
                  if(EVEN < 0) {
                     fprintf(stderr, "%s: illegal pattern, unmatched '<', '>'\n", Progname);
                     exit(2);
                  }
                  break;
    case 244 : if(No_error == 1) NO_ERR_MASK = NO_ERR_MASK | Bit[j];
                  i=i+1;
                  if (Pattern[i] == 248) { position[j].flag = 1; i++; }
                  k=0;
                  while (Pattern[i] != 245 && i < M)
                  {
                    if(Pattern[i] == 237)
                  ...
                  j++; break;
    case 245 : fprintf(stderr, "%s: illegal pattern, unmatched '[', ']'\n", Progname);
                  exit(2);
                  break;
    case 251 : if(REGEX == 1 || AND == 1) {
    ...
    case 252 : position[j].flag = 2; position[j].class1[0] = '\0';
                  if(j > D_length) AND = 1;
                  if(OR || (REGEX == 1 && j>D_length)) {
                     fprintf(stderr, "illegal pattern \n");
                     exit(2);
                  }
                  endposition = endposition | Bit[j++]; break;
    ...
    case '\n' : NO_ERR_MASK = NO_ERR_MASK | Bit[j];
                  ...
                  position[j++].class1[2] = '\0';
                  break;
    case 241 : NO_ERR_MASK = NO_ERR_MASK | Bit[j];
                  ...
                  break;
    case 239 : NO_ERR_MASK |= Bit[j];
                  position[j].class1[0] = position[j].class1[1] = '\n';
                  position[j].class1[2] = position[j].class1[3] = 239;
                  position[j++].class1[4] = '\0';
                  break;
    default : if(No_error == 1) NO_ERR_MASK = NO_ERR_MASK | Bit[j];
                  position[j].flag = 0;
...
NO_ERR_MASK = ~NO_ERR_MASK >> (base-1);
  for (i=1; i<= 32 - M ; i++) Init[0] = Init[0] | Bit[i];
  ...
  for(c=0, i=0; i < 256; c++, i++)
  {
     for (k=1, l=0; k<=M ; k++, l=0) {
         while (position[k].class1[l] != '\0') {
               if (position[k].class1[l] == 238 && (c != '\n' || REGEX) )
                  { Mask[c] = Mask[c] | Bit[base + k]; break; }
               if (c >= position[k].class1[l] && c <= position[k].class1[l+1])
                  { Mask[c] = Mask[c] | Bit[base + k]; break; }
               l = l + 2; }
         if (position[k].flag == 1) Mask[c] = Mask[c] ^ Bit[base+k];
     }
  }
  if(NOUPPER) for(c='A'; c<='Z'; c=c+1) if (((*__ctype_b_loc ())[(int) ((c))] & (unsigned short int) _ISupper))
                  Mask[c] = Mask[(__extension__ ({ int __res; if (sizeof (c) > 1) { if (__builtin_constant_p (c)) { int __c = (c); __res = __c < -128 || __c > 255 ? __c : (*__ctype_tolower_loc ())[__c]; } else __res = tolower (c); } else __res = (*__ctype_tolower_loc ())[(int) (c)]; __res; }))];
  return(M);
}
