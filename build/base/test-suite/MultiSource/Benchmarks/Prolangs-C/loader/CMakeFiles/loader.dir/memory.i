# 1 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Benchmarks/Prolangs-C/loader/memory.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 399 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Benchmarks/Prolangs-C/loader/memory.c" 2
...
typedef int BOOLEAN;
# 9 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Benchmarks/Prolangs-C/loader/memory.c" 2
...
# 10 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Benchmarks/Prolangs-C/loader/memory.c" 2
...
# 11 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Benchmarks/Prolangs-C/loader/memory.c" 2
...
extern size_t strlcat (char *__restrict __dest,
         const char *__restrict __src, size_t __n)
  __attribute__ ((__nothrow__ )) __attribute__ ((__nonnull__ (1, 2))) ;
# 12 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Benchmarks/Prolangs-C/loader/memory.c" 2
...
# 13 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Benchmarks/Prolangs-C/loader/memory.c" 2
...
# 14 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Benchmarks/Prolangs-C/loader/memory.c" 2
...
# 15 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Benchmarks/Prolangs-C/loader/memory.c" 2


typedef char *MEM_SPACE;
# 38 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Benchmarks/Prolangs-C/loader/memory.c"
...
struct BUFFER {
  ...
  struct BUFFER_ELEMENT *END;
} LOCATIONS_USED = {((void*)0),((void*)0)};



void ADD_TO_BUFFER(int LOCATION,int LEN)
{
  if (LOCATIONS_USED.HEAD == ((void*)0)) {
 ...
 (struct BUFFER_ELEMENT *) malloc(sizeof(struct BUFFER_ELEMENT));
  ...
  (*LOCATIONS_USED.END).NEXT = ((void*)0);
}
...
void CREATE_MEMORY(MEM_SPACE *MEMORY)
{
  ...
  else (*MEMORY) = malloc((unsigned int) 1048576);
}
...
{
  BOOLEAN LOCAL_ERROR = 0;
  int TEMP;
  int INT_VAL;



  if ( (ADDRESS + BYTES <= 1048576) && (ADDRESS + BYTES >= 0) ) {
              ...
              BOOLEAN *ERROR)
{
  ...
  DO_STORE(VALUE,BYTES,ADDRESS,MEMORY,ERROR,1);
}
                    ...
                    BOOLEAN *ERROR)
{
  ...
  if ( (LOCATION + HALF_BYTES/2 <= 1048576)
      && (LOCATION + HALF_BYTES/2 >= 0)) {
    int I;


    for (I = 0; I < HALF_BYTES/2; I ++)
    ...
    DO_STORE(CHAR_MEM_VAL,HALF_BYTES/2, LOCATION,
      MEMORY, ERROR,0);
  } else {
    (void) printf("ERROR: Illegal store[2] ADDRESS = %d, BYTES = %d.\n",
    LOCATION,HALF_BYTES/2);
    (*ERROR) = 1;
  }
}
...
{
  return (START_OF_LINE_ADDR(ADDRESS) + 4 * 4 - 1);
}
...
int LINES_OF_GAP(struct BUFFER_ELEMENT *REC1,struct BUFFER_ELEMENT *REC2)
{
  ...
  if (REC1 == ((void*)0)) REC1_END = 0;
  else REC1_END = START_OF_LINE_ADDR( (*REC1).START + (*REC1).LENGTH - 1);

  if (REC2 == ((void*)0)) REC2_START = START_OF_LINE_ADDR(1048576 - 1);
  else REC2_START = START_OF_LINE_ADDR( (*REC2).START);

  return ( (REC2_START - REC1_END) / (4 * 4) );
}
                  ...
                  int PREV_ADDR,FILE *OUTPUT)
{
  ...
  char STR_ADDR[20/4 + 1];



  if ( LINES_OF_GAP(PREV,CURR) >= 3 ) {

    NUM_TO_STR(PREV_ADDR,16,20/4, STR_ADDR);
    (void) fprintf(OUTPUT,
   ...
   16,20/4, STR_ADDR);
      ...
      NUM_TO_STR(START_OF_LINE_ADDR( 1048576) -
   4*4,
   16,20/4, STR_ADDR);
...
{
  struct BUFFER_ELEMENT *STEP;
  struct BUFFER_ELEMENT *PREVIOUS = ((void*)0);
  int PREVIOUS_ADDR = 0;
  int ADDRESS;
  char STR_ADDR[20/4 + 1];
  ...
  if (LOCATIONS_USED.HEAD == ((void*)0))
    ...
    for (STEP = LOCATIONS_USED.HEAD; STEP != ((void*)0); STEP = (*STEP).NEXT) {
      ...
      do {
 BOOLEAN REST_EMPTY = 0;
 int I,J;
 char STR_BYTE[2 + 1];

 NUM_TO_STR(ADDRESS,16,20/4,STR_ADDR);
 ...
 for (I=0;I<4;I++) {
   for (J=0;J<4;J++) {
     ...
     if (!REST_EMPTY && (LOC >= ((*STEP).START + (*STEP).LENGTH)))
       {
  if ( ((*STEP).NEXT == ((void*)0)) ||
      (LINES_OF_GAP(STEP,(*STEP).NEXT) > 0)) REST_EMPTY = 1;
  else STEP = (*STEP).NEXT;
       }
     ...
     if ( REST_EMPTY || (LOC < (*STEP).START))
       ...
       NUM_TO_STR(INT(MEMORY[LOC]),16,2,STR_BYTE);
...
void OUTPUT_MEM(MEM_SPACE MEMORY,FILE *OUTPUT)
{
  ...
  while (CURRENT != ((void*)0)) {
    NUM_TO_STR((*CURRENT).START,16,20/4 +1,
        ADDRESS);
    NUM_TO_STR((*CURRENT).LENGTH,16,2,STR_BYTE);
    (void) fprintf(OUTPUT,"T%s%s",ADDRESS,STR_BYTE);

    for (COUNT = (*CURRENT).START;
  COUNT < ((*CURRENT).START + (*CURRENT).LENGTH);
  COUNT ++) {
      NUM_TO_STR(INT(MEMORY[COUNT]),16,2,STR_BYTE);
      (void) fprintf(OUTPUT,"%s",STR_BYTE);
    }
    (void) fprintf(OUTPUT,"\n");
