# 1 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Benchmarks/MiBench/consumer-typeset/z17.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 399 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Benchmarks/MiBench/consumer-typeset/z17.c" 2
# 30 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Benchmarks/MiBench/consumer-typeset/z17.c"
...
# 31 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Benchmarks/MiBench/consumer-typeset/z17.c" 2
# 57 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Benchmarks/MiBench/consumer-typeset/z17.c"
...
{ int w; float num;
  ...
  (*res_gap).onobreak = 0;
  (*res_gap).owidth = 0; (*res_gap).ounits = 1;
  (*res_gap).omode = 1; *res_inc = 158;


  if( !(((x)->os1.ou1.os11.otype) == 11 || ((x)->os1.ou1.os11.otype) == 12) )
  { Error(17, 1, "gap is not a simple word", 2, &(x)->os1.ou1.ofpos);
                                                            ;
    return;
  }
  str = (x)->os1.ostring;
  ...
  if( *str == '+' ) *res_inc = 159, str++;
  else if( *str == '-' ) *res_inc = 160, str++;


  if( sscanf((char *) str, "%f", &num) != 1 )
  { Error(17, 2, "width missing from %s", 2, &(x)->os1.ou1.ofpos, (x)->os1.ostring);
    Error(17, 3, "%s, %s and %s must be enclosed in double quotes",
      2, &(x)->os1.ou1.ofpos, ( (FULL_CHAR *) ("/") ), ( (FULL_CHAR *) ("|") ), ( (FULL_CHAR *) ("&") ));
                                                    ;
    return;
  }
  ...
  switch( *str++ )
  {
    case 'c': w = num*567; (*res_gap).ounits = 1; break;;
    case 'i': w = num*1440; (*res_gap).ounits = 1; break;;
    case 'p': w = num*20; (*res_gap).ounits = 1; break;;
    case 'm': w = num*120; (*res_gap).ounits = 1; break;;
    case 'f': w = num*FontSize((*style).ofont, x); (*res_gap).ounits = 1; break;;
    case 's': w = num*((*style).osu2.ospace_gap).owidth; (*res_gap).ounits = 1; break;;
    case 'v': w = num*((*style).osu1.oline_gap).owidth; (*res_gap).ounits = 1; break;;
    case 'y': w = num*(*style).oyunit; (*res_gap).ounits = 1; break;;
    case 'z': w = num*(*style).ozunit; (*res_gap).ounits = 1; break;;
    case 'w': w = num*4096; (*res_gap).ounits = 5; break;;
    case 'b': w = num*4096; (*res_gap).ounits = 2; break;;
    case 'r': w = num*4096; (*res_gap).ounits = 3; break;;

    case 'd': if( *res_inc == 160 ) num = - num;
   *res_inc = 158;
   while( num > 180.0 ) num -= 360.0;
   while( num < -180.0 ) num += 360.0;
   { if( !((num>=-180.0) && (num<=180.0)) ) Error(1, 2, "assert failed in %s", 0, no_fpos, "GetGap: dg!"); };
   w = num*128; (*res_gap).ounits = 4; break;;
  ...
  if( (*res_gap).ounits == 3 && w > 4096 )
  { Error(17, 5, "%.1fr too large (1.0r substituted)", 2, &(x)->os1.ou1.ofpos, num);
    w = 4096;
  }
  (*res_gap).owidth = w;
  ...
  {
    case 'u':
    case '\0': (*res_gap).omode = 1; break;
    case 'e': (*res_gap).omode = 1; str++; break;
    case 'h': (*res_gap).omode = 2; str++; break;
    case 'x': (*res_gap).omode = 3; str++; break;
    case 'o': (*res_gap).omode = 4; str++; break;
    case 'k': (*res_gap).omode = 5; str++; break;
    case 't': (*res_gap).omode = 6; str++; break;
 ...
 2, &(x)->os1.ou1.ofpos, (x)->os1.ostring);
    else (*res_gap).onobreak = 1;
    str++;
  }


  if( *str != '\0' )
    Error(17, 8, "invalid width or gap %s", 2, &(x)->os1.ou1.ofpos, (x)->os1.ostring);


                                              ;
}
# 178 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Benchmarks/MiBench/consumer-typeset/z17.c"
FULL_LENGTH MinGap(FULL_LENGTH a, FULL_LENGTH b, FULL_LENGTH c, GAP *xgap)
{ FULL_LENGTH res; int w;
  switch( (*xgap).ounits )
  {
    case 1: w = (*xgap).owidth;
   break;
   ...
   break;
    ...
    case 5: w = (*xgap).owidth * (b + c) / 4096;
   break;

    default: { if( !(0) ) Error(1, 2, "assert failed in %s", 0, no_fpos, "MinGap: units"); };
   break;
  }
  switch( (*xgap).omode )
  {
    case 0: { if( !(0) ) Error(1, 2, "assert failed in %s", 0, no_fpos, "MinGap: NO_MODE"); };
   res = 0;
   break;

    case 7:
    case 2:
    case 1: res = ((8388607) < (a + w + b) ? (8388607) : (a + w + b));
   break;

    case 3: if( BackEnd->fractional_spacing_avail )
     res = ((w) < (a + b + (FULL_LENGTH) (0.1 * w)) ? (a + b + (FULL_LENGTH) (0.1 * w)) : (w));
   else
     res = ((w) < (a + b) ? (a + b) : (w));
   break;
   ...
   break;
    ...
    case 5: res = ((((a) < (b) ? (b) : (a))) < (w) ? (w) : (((a) < (b) ? (b) : (a))));
   break;

    case 6: res = a + b;
   break;

    default: { if( !(0) ) Error(1, 2, "assert failed in %s", 0, no_fpos, "MinGap: mode"); };
   res = 0;
   break;
...
# 246 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Benchmarks/MiBench/consumer-typeset/z17.c"
FULL_LENGTH ExtraGap(FULL_LENGTH a, FULL_LENGTH b, GAP *xgap, int dir)
{ FULL_LENGTH tmp, res;
  FULL_LENGTH w = (*xgap).ounits == 1 ? (*xgap).owidth : 0;
  switch( (*xgap).omode )
  {
    case 0: { if( !(0) ) Error(1, 2, "assert failed in %s", 0, no_fpos, "ExtraGap: NO_MODE"); };
   res = 0;
   break;
   ...
   break;
    ...
    case 3: if( BackEnd->fractional_spacing_avail )
     res = ((0) < ((FULL_LENGTH) (0.9 * w) - a - b) ? ((FULL_LENGTH) (0.9 * w) - a - b) : (0));
   else
     res = ((0) < (w - a - b) ? (w - a - b) : (0));
   break;
   ...
   break;
    ...
    case 5: tmp = ((a) < (((b) < (w) ? (w) : (b))) ? (((b) < (w) ? (w) : (b))) : (a));
   res = dir == 151 ? tmp - b : tmp - a;
   break;

    case 6: res = 0;
   break;

    default: { if( !(0) ) Error(1, 2, "assert failed in %s", 0, no_fpos, "ExtraGap: mode"); };
   res = 0;
   break;
...
# 298 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Benchmarks/MiBench/consumer-typeset/z17.c"
...
{ FULL_LENGTH res; int w, w2;
  switch( (*xgap).ounits )
  {
   ...
   break;
    ...
    case 2: if( (*xgap).owidth > 4096 )
     w = 8388607;
   else
     w = ((*xgap).owidth * frame_size) / 4096;
   break;

    case 3: w = ((*xgap).owidth * (frame_size - b - f)) / 4096;
   w = ((w) < (0) ? (0) : (w));
   break;

    case 5: w = (*xgap).owidth * (b + f) / 4096;
   break;

    default: { if( !(0) ) Error(1, 2, "assert failed in %s", 0, no_fpos, "ActualGap: units"); };
   break;
  }
  switch( (*xgap).omode )
  {
    case 0: Error(17, 10, "cannot continue after previous error(s)", 1, no_fpos);
   { if( !(0) ) Error(1, 2, "assert failed in %s", 0, no_fpos, "ActualGap: NO_MODE"); };
   w2 = 0;
   break;
   ...
   break;
    ...
    case 3: if( BackEnd->fractional_spacing_avail )
     w2 = ((w) < (prevf + b + (FULL_LENGTH) (0.1 * w)) ? (prevf + b + (FULL_LENGTH) (0.1 * w)) : (w));
   else
     w2 = ((w) < (prevf + b) ? (prevf + b) : (w));
   break;

    case 4: w2 = w;
   break;

    case 5: w2 = ((((prevf) < (b) ? (b) : (prevf))) < (w) ? (w) : (((prevf) < (b) ? (b) : (prevf))));
   break;

    case 6: w2 = w + b - mk;
   w2 = ((w2) < (prevf + b) ? (prevf + b) : (w2));
   break;

    default: { if( !(0) ) Error(1, 2, "assert failed in %s", 0, no_fpos, "ActualGap: mode"); };
   w2 = 0;
   break;
  }
  res = ((8388607) < (w2) ? (8388607) : (w2));
