# 1 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Benchmarks/BitBench/five11/five11.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 399 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Benchmarks/BitBench/five11/five11.c" 2
...
# 2 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Benchmarks/BitBench/five11/five11.c" 2
...
# 3 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Benchmarks/BitBench/five11/five11.c" 2
...
# 5 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Benchmarks/BitBench/five11/five11.c" 2
...
# 6 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Benchmarks/BitBench/five11/five11.c" 2
...
int read_x_bits(unsigned char* buf, int offset, int x) {
  int byte_offset, bit_offset,present,initbits,res;
  byte_offset = offset >> 3;
  bit_offset = offset & 7;
  initbits = 8-bit_offset;
  present = (int)buf[byte_offset] & 255;
  res = present & ((1 << initbits) - 1);
  while (x > initbits) {
    byte_offset++;
    present = (int)buf[byte_offset] & 255;
    res = (res << 8) | present;
  ...
  return res >> (initbits-x);
}

int read_11_bits(unsigned char* buf, int offset) {
  int byte_offset, ioffset,present,initbits,res;
  int b0,b1,b2;
  byte_offset = offset >> 3;
  ioffset = (8-(offset & 7));
  b0 = (int)buf[byte_offset] & 255;
  b1 = (int)buf[byte_offset+1] & 255;
  res = b0 << (11-ioffset);
  if (ioffset >= 3) {
     return (res | (b1 >> (ioffset-3))) & ((1 << 11) - 1);
  }
  else
    b2 = (int)buf[byte_offset+2] & 255;
    return (res | (b1 << (3-ioffset)) | b2 >> (ioffset+5)) & ((1 << 11) - 1);
}

linked_list_lli* five11(unsigned char* inbuf) {
  int offset,nof_packets, nof_channels,i,j,chan;
  linked_list_int *packet=((void*)0);
  linked_list_lli *res=((void*)0);
  offset = 0;
  nof_packets=read_x_bits(inbuf,offset,16);
  offset=offset+16;
  for(i=0;i<nof_packets;i++){
    nof_channels=read_x_bits(inbuf,offset,5);
    offset+=5;
    for(j=0;j<nof_channels;j++){
      chan=read_11_bits(inbuf,offset);
      offset=offset+11;
      packet = cons1(chan,packet);
    }
    offset=offset+pad_size(nof_channels);
    res = cons2(packet,res);
    packet=((void*)0);
  }
    return res;
}
...
{
  return fread(buffer, 1, 150000, in);
}
