# 1 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Benchmarks/7zip/C/MtCoder.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 400 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Benchmarks/7zip/C/MtCoder.c" 2
...
# 5 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Benchmarks/7zip/C/MtCoder.c" 2

# 1 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Benchmarks/7zip/C/MtCoder.h" 1
...
# 8 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Benchmarks/7zip/C/Threads.h" 2
...
extern size_t strlcat (char *__restrict __dest,
         const char *__restrict __src, size_t __n)
  __attribute__ ((__nothrow__ )) __attribute__ ((__nonnull__ (1, 2))) ;
# 14 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Benchmarks/7zip/CPP/Common/MyWindows.h" 2
...
# 8 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Benchmarks/7zip/C/MtCoder.h" 2
...
# 7 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Benchmarks/7zip/C/MtCoder.c" 2
...
{
  p->stop = 0;
  { int __result__ = (AutoResetEvent_CreateNotSignaled(&p->startEvent)); if (__result__ != 0) return __result__; };
  { int __result__ = (AutoResetEvent_CreateNotSignaled(&p->finishedEvent)); if (__result__ != 0) return __result__; };
...
{
  return (p && p->Progress(p, inSize, outSize) != 0) ? 10 : 0;
}
...
SRes MtProgress_Set(CMtProgress *p, unsigned index, UInt64 inSize, UInt64 outSize)
{
  ...
  if (inSize != (UInt64)(Int64)-1) { p->totalInSize += inSize - p->inSizes[index]; p->inSizes[index] = inSize; }
  if (outSize != (UInt64)(Int64)-1) { p->totalOutSize += outSize - p->outSizes[index]; p->outSizes[index] = outSize; }
  if (p->res == 0)
...
static void CMtThread_Destruct(CMtThread *p)
{
  ...
  if (p->mtCoder->alloc)
    (p->mtCoder->alloc)->Free((p->mtCoder->alloc), p->outBuf);
  p->outBuf = 0;

  if (p->mtCoder->alloc)
    (p->mtCoder->alloc)->Free((p->mtCoder->alloc), p->inBuf);
  p->inBuf = 0;
}
...
{
  if (p->inBuf == 0 || p->inBufSize != p->mtCoder->blockSize) { (p->mtCoder->alloc)->Free((p->mtCoder->alloc), p->inBuf); p->inBufSize = p->mtCoder->blockSize; p->inBuf = (Byte *)(p->mtCoder->alloc)->Alloc((p->mtCoder->alloc), p->inBufSize); if (p->inBuf == 0) return 2; }
  if (p->outBuf == 0 || p->outBufSize != p->mtCoder->destBlockSize) { (p->mtCoder->alloc)->Free((p->mtCoder->alloc), p->outBuf); p->outBufSize = p->mtCoder->destBlockSize; p->outBuf = (Byte *)(p->mtCoder->alloc)->Alloc((p->mtCoder->alloc), p->outBufSize); if (p->outBuf == 0) return 2; }

  p->stopReading = 0;
  p->stopWriting = 0;
  { if((AutoResetEvent_CreateNotSignaled(&p->canRead)) != 0) return 12; };
  { if((AutoResetEvent_CreateNotSignaled(&p->canWrite)) != 0) return 12; };
...
static SRes MtThread_Process(CMtThread *p, Bool *stop)
{
  ...
  *stop = 1;
  ...
  next = &p->mtCoder->threads[p->index == p->mtCoder->numThreads - 1 ? 0 : p->index + 1];

  if (p->stopReading)
  {
    next->stopReading = 1;
    return Event_Set(&next->canRead) == 0 ? 0 : 12;
  }

  {
    ...
    { int __result__ = (FullRead(p->mtCoder->inStream, p->inBuf, &size)); if (__result__ != 0) return __result__; };
    next->stopReading = *stop = (size != p->mtCoder->blockSize);
    if (Event_Set(&next->canRead) != 0)
      return 12;

    { int __result__ = (p->mtCoder->mtCallback->Code(p->mtCoder->mtCallback, p->index, p->outBuf, &destSize, p->inBuf, size, *stop)); if (__result__ != 0) return __result__; };
    ...
    if (p->mtCoder->outStream->Write(p->mtCoder->outStream, p->outBuf, destSize) != destSize)
      return 9;
    return Event_Set(&next->canWrite) == 0 ? 0 : 12;
  }
}
...
static THREAD_FUNC_RET_TYPE ThreadFunc(void *pp)
{
  ...
  {
    Bool stop;
    CMtThread *next = &p->mtCoder->threads[p->index == p->mtCoder->numThreads - 1 ? 0 : p->index + 1];
...
SRes MtCoder_Code(CMtCoder *p)
{
  ...
  {
    { int __result__ = (CMtThread_Prepare(&p->threads[i])); if (__result__ != 0) return __result__; };
  }
  ...
  if (res == 0)
  {
    ...
    {
      CMtThread *t = &p->threads[i];
      if (LoopThread_StartSubThread(&t->thread) != 0)
      {
        res = 12;
