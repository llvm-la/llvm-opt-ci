# 1 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Benchmarks/MallocBench/gs/idict.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 399 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Benchmarks/MallocBench/gs/idict.c" 2
# 22 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Benchmarks/MallocBench/gs/idict.c"
...
# 23 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Benchmarks/MallocBench/gs/idict.c" 2
...
# 24 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Benchmarks/MallocBench/gs/idict.c" 2
...
# 25 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Benchmarks/MallocBench/gs/idict.c" 2
...
# 26 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Benchmarks/MallocBench/gs/idict.c" 2
...
# 27 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Benchmarks/MallocBench/gs/idict.c" 2
...
# 28 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Benchmarks/MallocBench/gs/idict.c" 2


extern ref dstack[];
# 46 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Benchmarks/MallocBench/gs/idict.c"
struct dict_s {
 ref count;
 ref contents;
};
...
uint dict_max_size = (32767 * 2 + 1) / 2 - 1;
...
{ uint asize = (size == 0 ? 1 : size) + 1;
 ...
 pair *pp;
 if ( pdict == 0 ) return (-25);
 pp = (pair *)alloc(asize, sizeof(pair), "dict_create(pairs)");
 if ( pp == 0 )
    { alloc_free((char *)pdict, 1, sizeof(dict), "dict_create");
  return (-25);
    }
 ((&pdict->count)->value.intval = (0), ((&pdict->count)->type_attrs = (5) << 2));
 (((&pdict->contents)->value.refs = ((ref *)pp), ((&pdict->contents)->type_attrs = ((0) << 2) + ((0x100 +0x200 +2)))), (&pdict->contents)->size = (asize * 2));

 ((pref)->value.pdict = (pdict), ((pref)->type_attrs = ((2) << 2) + ((0x100 +0x200 +2))));
 pp = (pair *)((pdict)->contents.value.refs);
 while ( asize-- )
  pp->key.type_attrs = pp->value.type_attrs = (8 << 2),
  pp++;
 return 0;
   }
...
nh: hash = kpname->index * 40503;
 ...
 default:
  hash = ((((pkey)->type_attrs & 0xff) >> 2) >= 16 ? 9 : (((pkey)->type_attrs & 0xff) >> 2)) * 99;
  ktype = -1;
    }
 do
    { dict *pdict = pdref->value.pdict;
  uint size = (((pdict)->contents.size >> 1) - 1);
  pair *ppbot = (pair *)((pdict)->contents.value.refs);
  register pair *pp;
  int wrap = 0;
  register int etype;
# 152 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Benchmarks/MallocBench/gs/idict.c"
  for ( pp = ppbot + (hash % size) + 2; ; )
     { if ( (etype = (((&(--pp)->key)->type_attrs & 0xff) >> 2)) == ktype )
      {
...
int
dict_put(ref *pdref , ref *pkey, ref *pvalue)
{ ref *pvslot;
 if ( dict_lookup(pdref,pdref,pkey,&pvslot) <= 0 )
    {
  dict *pdict = pdref->value.pdict;
  if ( pdict->count.value.intval == (((pdict)->contents.size >> 1) - 1) )
   return (-2);
  pdict->count.value.intval++;
  pvslot[-1] = *pkey;

  if ( (((pkey)->type_attrs & 0xff) >> 2) == 7 )
     { name *pname = pkey->value.pname;
   if ( pname->pvalue == ((ref *)0) &&
    (pdict == (dstack[0]).value.pdict ||
     pdict == (dstack[1]).value.pdict) )
      {
    pname->pvalue = pvslot;
      }
   else
...
uint
dict_length(ref *pdref )
{ return (uint)(pdref->value.pdict->count.value.intval);
}


uint
dict_maxlength(ref *pdref )
{ return (((pdref->value.pdict)->contents.size >> 1) - 1);
}


int
dict_copy(ref *pdrfrom , ref *pdrto )
{ dict *pdict = pdrfrom->value.pdict;
 int count = (((pdict)->contents.size >> 1) - 1) + 1;
 pair *pp = (pair *)((pdict)->contents.value.refs);
 int code;
 while ( count-- )
    { if ( (((&pp->key)->type_attrs & 0xff) >> 2) != 8 )
...
{ return (int)((((pdref->value.pdict)->contents.size >> 1) - 1) + 1);
}


int
dict_next(ref *pdref, int index, ref *eltp )
{ pair *pp = (pair *)((pdref->value.pdict)->contents.value.refs) + index;
 while ( pp--, --index >= 0 )
    { if ( (((&pp->key)->type_attrs & 0xff) >> 2) != 8 )
     { eltp[0] = pp->key;
   eltp[1] = pp->value;
