# 1 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Benchmarks/MallocBench/espresso/opo.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 398 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Benchmarks/MallocBench/espresso/opo.c" 2
...
# 2 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Benchmarks/MallocBench/espresso/opo.c" 2
# 50 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Benchmarks/MallocBench/espresso/opo.c"
...
int opo_strategy;
{
 ...
 PLA->phase = find_phase(PLA, 0, (pset) ((void*)0));
    }
...
pPLA PLA;
{
 ...
 if ((phase)) { (void) free((char *) (phase)); (phase) = 0; };
    }
}
...
pset find_phase(PLA, first_output, phase1)
pPLA PLA;
int first_output;
pset phase1;
{
...
pset_family opo(phase, T, D, R, first_output)
pset phase;
pset_family T, D, R;
int first_output;
{
 ...
 if (! (pdest[(((i) >> 5) + 1)] & (1 << ((i) & (32 -1))))) {
    ...
    if ((select)) { (void) free((char *) (select)); (select) = 0; };
    sf_free(temp);
 ...
 for(p1=T2->data,last1=p1+T2->count*T2->wsize;p1<last1;p1+=T2->wsize) {
  ...
  (void) set_or(not_covered, not_covered, set_and(tmp, p, p1));
    ...
    sf_free(T);
    sf_free(T2);
    if ((tmp)) { (void) free((char *) (tmp)); (tmp) = 0; };
 ...
 if ((not_covered[(((ind) >> 5) + 1)] & (1 << ((ind) & (32 -1))))) {
  ...
  (phase[(((ind) >> 5) + 1)] &= ~ (1 << ((ind) & (32 -1))));
    ...
    if ((not_covered)) { (void) free((char *) (not_covered)); (not_covered) = 0; };
    return T1;
}
...
# 405 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Benchmarks/MallocBench/espresso/opo.c"
...
int first_output;
{
 ...
 for(i = first_part; i <= last_part; i++)
  ...
  (pf[(((i+offset) >> 5) + 1)] |= 1 << ((i+offset) & (32 -1)));
    ...
    if ((mask1)) { (void) free((char *) (mask1)); (mask1) = 0; };
}
