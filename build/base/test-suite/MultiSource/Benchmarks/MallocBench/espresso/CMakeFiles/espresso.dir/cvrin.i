# 1 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Benchmarks/MallocBench/espresso/cvrin.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 398 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Benchmarks/MallocBench/espresso/cvrin.c" 2
...
# 7 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Benchmarks/MallocBench/espresso/cvrin.c" 2
...
{
    register int ch, i = 0;
    while ((ch = getc(fp)) != (-1) && ((*__ctype_b_loc ())[(int) ((ch))] & (unsigned short int) _ISspace))
 ;
    word[i++] = ch;
    while ((ch = getc(fp)) != (-1) && ! ((*__ctype_b_loc ())[(int) ((ch))] & (unsigned short int) _ISspace))
 word[i++] = ch;
...
pPLA PLA;
{
 ...
 if (cube.part_size[var] < 0) {
  ...
  for(i = first; i <= last; i++)
      if (PLA->label[i] == (char *) ((void*)0)) {
   PLA->label[i] = util_strsav(token);
   (cf[(((i+offset) >> 5) + 1)] |= 1 << ((i+offset) & (32 -1)));
   break;
...
loop:
 ...
 case '.':
  ...
  if (cube.fullset != ((void*)0)) {
      fprintf(stderr, "extra .i ignored\n");
      skip_line(fp, stdout, 0);
  } else {
...
# 477 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Benchmarks/MallocBench/espresso/cvrin.c"
    ...
    pPLA *PLA_return;
{
 ...
 totals(time, 0, PLA->F, &cost);
    ...
    time = util_cpu_time();
    if (pos || PLA->phase != ((void*)0) || PLA->symbolic_output != ((symbolic_t *) 0)) {
 needs_offset = 1;
    }
 ...
 set_diff(PLA->phase, cube.fullset, cube.var_mask[cube.num_vars-1]);
    } else if (PLA->phase != ((void*)0)) {
 (void) set_phase(PLA);
    }


    if (PLA->pair != (ppair) ((void*)0)) {
 set_pair(PLA);
    }
 ...
 if (needs_offset) {
     sf_free(PLA->R);
{long t=util_cpu_time();PLA->R=complement(cube2list(PLA->F,PLA->D));totals(t,1,PLA->R,&(cost));};
 }
    }
...
pPLA new_PLA()
{
    ...
    PLA->F = PLA->D = PLA->R = (pset_family) ((void*)0);
    PLA->phase = (pset) ((void*)0);
    PLA->pair = (ppair) ((void*)0);
    PLA->label = (char **) ((void*)0);
    PLA->filename = (char *) ((void*)0);
    PLA->pla_type = 0;
...
pPLA PLA;
{
    ...
    if (PLA->F != (pset_family) ((void*)0))
 sf_free(PLA->F);
    if (PLA->R != (pset_family) ((void*)0))
 sf_free(PLA->R);
    if (PLA->D != (pset_family) ((void*)0))
 sf_free(PLA->D);
    if (PLA->phase != (pset) ((void*)0))
 if ((PLA->phase)) { (void) free((char *) (PLA->phase)); (PLA->phase) = 0; };
    if (PLA->pair != (ppair) ((void*)0)) {
 if ((PLA->pair->var1)) { (void) free((char *) (PLA->pair->var1)); (PLA->pair->var1) = 0; };
 if ((PLA->pair->var2)) { (void) free((char *) (PLA->pair->var2)); (PLA->pair->var2) = 0; };
 if ((PLA->pair)) { (void) free((char *) (PLA->pair)); (PLA->pair) = 0; };
    }
    if (PLA->label != ((void*)0)) {
 for(i = 0; i < cube.size; i++)
     if (PLA->label[i] != ((void*)0))
  if ((PLA->label[i])) { (void) free((char *) (PLA->label[i])); (PLA->label[i]) = 0; };
 if ((PLA->label)) { (void) free((char *) (PLA->label)); (PLA->label) = 0; };
    }
    if (PLA->filename != ((void*)0)) {
 if ((PLA->filename)) { (void) free((char *) (PLA->filename)); (PLA->filename) = 0; };
    }
    for(p1 = PLA->symbolic; p1 != ((symbolic_t *) 0); p1 = p1next) {
 for(p2 = p1->symbolic_list; p2 != ((symbolic_list_t *) 0); p2 = p2next) {
     p2next = p2->next;
     if ((p2)) { (void) free((char *) (p2)); (p2) = 0; };
 }
 p1next = p1->next;
 if ((p1)) { (void) free((char *) (p1)); (p1) = 0; };
    }
    PLA->symbolic = ((symbolic_t *) 0);
 ...
 if ((p1)) { (void) free((char *) (p1)); (p1) = 0; };
    ...
    PLA->symbolic_output = ((symbolic_t *) 0);
    if ((PLA)) { (void) free((char *) (PLA)); (PLA) = 0; };
}
