# 1 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Benchmarks/MallocBench/espresso/cvrout.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 398 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Benchmarks/MallocBench/espresso/cvrout.c" 2
...
# 7 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Benchmarks/MallocBench/espresso/cvrout.c" 2
...
int output_type;
{
    ...
    if (output_type != 1) {
 fprintf(fp, ".type ");
 if (output_type & 1) putc('f', fp);
 if (output_type & 2) putc('d', fp);
 if (output_type & 4) putc('r', fp);
 putc('\n', fp);
    }
 ...
 if (PLA->label != ((void*)0) && PLA->label[first] != ((void*)0)) {
 ...
 fprintf(fp, "#.phase ");
 for(i = first; i <= last; i++)
     putc((PLA->phase[(((i) >> 5) + 1)] & (1 << ((i) & (32 -1)))) ? '1' : '0', fp);
 fprintf(fp, "\n");
    }
}
...
pset phase;
{
 ...
 putc(' ', fp);
 for(i = cube.first_part[var]; i <= cube.last_part[var]; i++) {
     if (phase == (pset) ((void*)0) || (phase[(((i) >> 5) + 1)] & (1 << ((i) & (32 -1))))) {
  out_map = "~1";
     } else {
  out_map = "~0";
     }
...
int level;
{
 ...
 printf("\n");
    ...
    if (verbose_debug) {
 printf("cofactor=%s\n", pc1(T[0]));
 for(T1 = T+2, cnt = 1; (p = *T1++) != (pset) ((void*)0); cnt++)
  ...
  if (weight[i] > 0) {
   ...
   if ((((A)->data + (A)->wsize * (i))[(((j) >> 5) + 1)] & (1 << ((j) & (32 -1))))) {
    ...
    PLA->label[cube.first_part[var]+j]);
     ...
     if ((weight)) { (void) free((char *) (weight)); (weight) = 0; };
 }
    }
}
