# 1 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Benchmarks/MallocBench/espresso/contain.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 398 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Benchmarks/MallocBench/espresso/contain.c" 2
# 16 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Benchmarks/MallocBench/espresso/contain.c"
...
# 17 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Benchmarks/MallocBench/espresso/contain.c" 2
...
# 69 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Benchmarks/MallocBench/espresso/contain.c"
...
# 109 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Benchmarks/MallocBench/espresso/contain.c"
...
int (*compare)();
{
    ...
    dest = 0;
    if (A1[0] != (pset) ((void*)0)) {
 for(i = 0, j = 1; A1[j] != (pset) ((void*)0); j++)
   ...
   pset *B1;
{
 ...
 for(pb = B1; (b = *pb++) != ((void*)0) && (b[0] >> 16) > (a[0] >> 16); ) {
     {register int i_=(a[0] & 0x03ff); do if (a[i_]&~b[i_]) break; while (--i_>0); if (i_ != 0) continue;};
   ...
   int totcnt, size;
{
    ...
    R->count = totcnt;
    for(pr = R->data, pa = A1; (p = *pa++) != ((void*)0); pr += R->wsize)
 {register int i_=(p[0] & 0x03ff); do pr[i_]=p[i_]; while (--i_>=0);};
    if ((A1)) { (void) free((char *) (A1)); (A1) = 0; };
    return R;
}
   ...
   register pset pfirst;
{
    ...
    for(pr = R->data, pa = A1, i=0; (p = *pa++) != ((void*)0); pr += R->wsize, i++) {
 {register int i_=(p[0] & 0x03ff); do pr[i_]=p[i_]; while (--i_>=0);};
 new_row_indices[i] = row_indices[(p - pfirst)/R->wsize];
    }
    for(i = 0; i < totcnt; i++)
 row_indices[i] = new_row_indices[i];
    if ((new_row_indices)) { (void) free((char *) (new_row_indices)); (new_row_indices) = 0; };
    if ((A1)) { (void) free((char *) (A1)); (A1) = 0; };
    return R;
}
   ...
   int totcnt, size;
{
 ...
 if (desc1(*pmin, *pmax) > 0) {
    ...
    if ((A1)) { (void) free((char *) (A1)); (A1) = 0; };
    if ((B1)) { (void) free((char *) (B1)); (B1) = 0; };
    if ((E1)) { (void) free((char *) (E1)); (E1) = 0; };
    return R;
}
