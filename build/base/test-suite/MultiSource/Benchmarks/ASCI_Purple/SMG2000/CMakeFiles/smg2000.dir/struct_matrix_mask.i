# 1 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Benchmarks/ASCI_Purple/SMG2000/struct_matrix_mask.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 399 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Benchmarks/ASCI_Purple/SMG2000/struct_matrix_mask.c" 2
# 15 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Benchmarks/ASCI_Purple/SMG2000/struct_matrix_mask.c"
...
# 16 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Benchmarks/ASCI_Purple/SMG2000/struct_matrix_mask.c" 2
# 33 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Benchmarks/ASCI_Purple/SMG2000/struct_matrix_mask.c"
hypre_StructMatrix *
                              ...
                              int *stencil_indices )
{
   ...
   mask = ( (hypre_StructMatrix *)hypre_CAlloc((unsigned int)(1), (unsigned int)sizeof(hypre_StructMatrix)) );

   ((mask) -> comm) = ((matrix) -> comm);

   hypre_StructGridRef(((matrix) -> grid),
                       &((mask) -> grid));

   ((mask) -> user_stencil) =
      hypre_StructStencilRef(((matrix) -> user_stencil));

   mask_stencil_size = num_stencil_indices;
   mask_stencil_shape = ( (hypre_Index *)hypre_CAlloc((unsigned int)(num_stencil_indices), (unsigned int)sizeof(hypre_Index)) );
   for (i = 0; i < num_stencil_indices; i++)
   {
      ( (mask_stencil_shape[i][0]) = (stencil_shape[stencil_indices[i]][0]), (mask_stencil_shape[i][1]) = (stencil_shape[stencil_indices[i]][1]), (mask_stencil_shape[i][2]) = (stencil_shape[stencil_indices[i]][2]) );

   }
   ((mask) -> stencil) =
      hypre_StructStencilCreate(((stencil) -> dim),
                                mask_stencil_size,
                                mask_stencil_shape);

   ((mask) -> num_values) = ((matrix) -> num_values);

   ((mask) -> data_space) =
      hypre_BoxArrayDuplicate(((matrix) -> data_space));

   ((mask) -> data) = ((matrix) -> data);
   ((mask) -> data_alloced) = 0;
   ((mask) -> data_size) = ((matrix) -> data_size);
   data_space = ((matrix) -> data_space);
   data_indices = ((matrix) -> data_indices);
   mask_data_indices = ( (int * *)hypre_CAlloc((unsigned int)(((data_space) -> size)), (unsigned int)sizeof(int *)) );
   for (i = 0; i < ((data_space) -> size); i++)
      {
         mask_data_indices[i] = ( (int *)hypre_MAlloc((unsigned int)(sizeof(int) * (num_stencil_indices))) );
         for (j = 0; j < num_stencil_indices; j++)
         {
            mask_data_indices[i][j] = data_indices[i][stencil_indices[j]];
         }
      }
   ((mask) -> data_indices) = mask_data_indices;

   ((mask) -> symmetric) = ((matrix) -> symmetric);

   ((mask) -> symm_elements) = ( (int *)hypre_MAlloc((unsigned int)(sizeof(int) * (stencil_size))) );
   for (i = 0; i < stencil_size; i++)
   {
      ((mask) -> symm_elements)[i] =
         ((matrix) -> symm_elements)[i];
   }
   ...
   ((mask) -> global_size) =
      ((((mask) -> grid)) -> global_size) *
      mask_stencil_size;

   ((mask) -> comm_pkg) = ((void*)0);
