# 1 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/SingleSource/Benchmarks/Stanford/Puzzle.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 399 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/SingleSource/Benchmarks/Stanford/Puzzle.c" 2
...
# 2 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/SingleSource/Benchmarks/Stanford/Puzzle.c" 2
...
# 3 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/SingleSource/Benchmarks/Stanford/Puzzle.c" 2
# 45 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/SingleSource/Benchmarks/Stanford/Puzzle.c"
struct node {
...
# 75 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/SingleSource/Benchmarks/Stanford/Puzzle.c"
...
int Place (int i, int j) {
 int k;
 for ( k = 0; k <= piecemax[i]; k++ ) if ( p[i][k] ) puzzl[j+k] = 1;
 piececount[class[i]] = piececount[class[i]] - 1;
 for ( k = j; k <= 511; k++ ) if ( ! puzzl[k] ) return (k);
 return (0);
}

void Remove (int i, int j) {
 int k;
 for ( k = 0; k <= piecemax[i]; k++ ) if ( p[i][k] ) puzzl[j+k] = 0;
 piececount[class[i]] = piececount[class[i]] + 1;
}
...
void Puzzle () {
    int i, j, k, m;
    for ( m = 0; m <= 511; m++ ) puzzl[m] = 1;
    for( i = 1; i <= 5; i++ )for( j = 1; j <= 5; j++ )for( k = 1; k <= 5; k++ ) puzzl[i+8*(j+8*k)] = 0;
    for( i = 0; i <= 12; i++ )for( m = 0; m<= 511; m++ ) p[i][m] = 0;
    for( i = 0; i <= 3; i++ )for( j = 0; j <= 1; j++ )for( k = 0; k <= 0; k++ ) p[0][i+8*(j+8*k)] = 1;
    class[0] = 0;
    piecemax[0] = 3+8*1+8*8*0;
    for( i = 0; i <= 1; i++ )for( j = 0; j <= 0; j++ )for( k = 0; k <= 3; k++ ) p[1][i+8*(j+8*k)] = 1;
    class[1] = 0;
    piecemax[1] = 1+8*0+8*8*3;
    for( i = 0; i <= 0; i++ )for( j = 0; j <= 3; j++ )for( k = 0; k <= 1; k++ ) p[2][i+8*(j+8*k)] = 1;
    class[2] = 0;
    piecemax[2] = 0+8*3+8*8*1;
    for( i = 0; i <= 1; i++ )for( j = 0; j <= 3; j++ )for( k = 0; k <= 0; k++ ) p[3][i+8*(j+8*k)] = 1;
    class[3] = 0;
    piecemax[3] = 1+8*3+8*8*0;
    for( i = 0; i <= 3; i++ )for( j = 0; j <= 0; j++ )for( k = 0; k <= 1; k++ ) p[4][i+8*(j+8*k)] = 1;
    class[4] = 0;
    piecemax[4] = 3+8*0+8*8*1;
    for( i = 0; i <= 0; i++ )for( j = 0; j <= 1; j++ )for( k = 0; k <= 3; k++ ) p[5][i+8*(j+8*k)] = 1;
    class[5] = 0;
    piecemax[5] = 0+8*1+8*8*3;
    for( i = 0; i <= 2; i++ )for( j = 0; j <= 0; j++ )for( k = 0; k <= 0; k++ ) p[6][i+8*(j+8*k)] = 1;
    class[6] = 1;
    piecemax[6] = 2+8*0+8*8*0;
    for( i = 0; i <= 0; i++ )for( j = 0; j <= 2; j++ )for( k = 0; k <= 0; k++ ) p[7][i+8*(j+8*k)] = 1;
    class[7] = 1;
    piecemax[7] = 0+8*2+8*8*0;
    for( i = 0; i <= 0; i++ )for( j = 0; j <= 0; j++ )for( k = 0; k <= 2; k++ ) p[8][i+8*(j+8*k)] = 1;
    class[8] = 1;
    piecemax[8] = 0+8*0+8*8*2;
    for( i = 0; i <= 1; i++ )for( j = 0; j <= 1; j++ )for( k = 0; k <= 0; k++ ) p[9][i+8*(j+8*k)] = 1;
    class[9] = 2;
    piecemax[9] = 1+8*1+8*8*0;
    for( i = 0; i <= 1; i++ )for( j = 0; j <= 0; j++ )for( k = 0; k <= 1; k++ ) p[10][i+8*(j+8*k)] = 1;
    class[10] = 2;
    piecemax[10] = 1+8*0+8*8*1;
    for( i = 0; i <= 0; i++ )for( j = 0; j <= 1; j++ )for( k = 0; k <= 1; k++ ) p[11][i+8*(j+8*k)] = 1;
    class[11] = 2;
    piecemax[11] = 0+8*1+8*8*1;
    for( i = 0; i <= 1; i++ )for( j = 0; j <= 1; j++ )for( k = 0; k <= 1; k++ ) p[12][i+8*(j+8*k)] = 1;
    class[12] = 3;
