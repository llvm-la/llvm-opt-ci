# 1 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/SingleSource/Benchmarks/Misc/himenobmtxpa.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 399 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/SingleSource/Benchmarks/Misc/himenobmtxpa.c" 2
# 39 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/SingleSource/Benchmarks/Misc/himenobmtxpa.c"
...
# 40 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/SingleSource/Benchmarks/Misc/himenobmtxpa.c" 2
...
# 41 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/SingleSource/Benchmarks/Misc/himenobmtxpa.c" 2
...
extern size_t strlcat (char *__restrict __dest,
         const char *__restrict __src, size_t __n)
  __attribute__ ((__nothrow__ )) __attribute__ ((__nonnull__ (1, 2))) ;
# 42 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/SingleSource/Benchmarks/Misc/himenobmtxpa.c" 2
...
# 43 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/SingleSource/Benchmarks/Misc/himenobmtxpa.c" 2
...
mat_set_init(Matrix* Mat)
{
  ...
  for(i=0; i<Mat->mrows; i++)
    for(j=0; j<Mat->mcols; j++)
      for(k=0; k<Mat->mdeps; k++)
        Mat->m[(0) * Mat->mrows * Mat->mcols * Mat->mdeps + (i) * Mat->mcols* Mat->mdeps + (j) * Mat->mdeps + (k)]= (float)(i*i)
       ...
       Matrix* p,Matrix* bnd,Matrix* wrk1,Matrix* wrk2)
{
  ...
  for(n=0 ; n<nn ; n++){
    ...
    for(i=1 ; i<imax; i++)
      for(j=1 ; j<jmax ; j++)
        for(k=1 ; k<kmax ; k++){
          s0= a->m[(0) * a->mrows * a->mcols * a->mdeps + (i) * a->mcols* a->mdeps + (j) * a->mdeps + (k)]*p->m[(0) * p->mrows * p->mcols * p->mdeps + (i+1) * p->mcols* p->mdeps + (j) * p->mdeps + (k)]
            + a->m[(1) * a->mrows * a->mcols * a->mdeps + (i) * a->mcols* a->mdeps + (j) * a->mdeps + (k)]*p->m[(0) * p->mrows * p->mcols * p->mdeps + (i) * p->mcols* p->mdeps + (j+1) * p->mdeps + (k)]
            + a->m[(2) * a->mrows * a->mcols * a->mdeps + (i) * a->mcols* a->mdeps + (j) * a->mdeps + (k)]*p->m[(0) * p->mrows * p->mcols * p->mdeps + (i) * p->mcols* p->mdeps + (j) * p->mdeps + (k+1)]
            + b->m[(0) * b->mrows * b->mcols * b->mdeps + (i) * b->mcols* b->mdeps + (j) * b->mdeps + (k)]
             *( p->m[(0) * p->mrows * p->mcols * p->mdeps + (i+1) * p->mcols* p->mdeps + (j+1) * p->mdeps + (k)] - p->m[(0) * p->mrows * p->mcols * p->mdeps + (i+1) * p->mcols* p->mdeps + (j-1) * p->mdeps + (k)]
              - p->m[(0) * p->mrows * p->mcols * p->mdeps + (i-1) * p->mcols* p->mdeps + (j+1) * p->mdeps + (k)] + p->m[(0) * p->mrows * p->mcols * p->mdeps + (i-1) * p->mcols* p->mdeps + (j-1) * p->mdeps + (k)] )
            + b->m[(1) * b->mrows * b->mcols * b->mdeps + (i) * b->mcols* b->mdeps + (j) * b->mdeps + (k)]
             *( p->m[(0) * p->mrows * p->mcols * p->mdeps + (i) * p->mcols* p->mdeps + (j+1) * p->mdeps + (k+1)] - p->m[(0) * p->mrows * p->mcols * p->mdeps + (i) * p->mcols* p->mdeps + (j-1) * p->mdeps + (k+1)]
              - p->m[(0) * p->mrows * p->mcols * p->mdeps + (i) * p->mcols* p->mdeps + (j+1) * p->mdeps + (k-1)] + p->m[(0) * p->mrows * p->mcols * p->mdeps + (i) * p->mcols* p->mdeps + (j-1) * p->mdeps + (k-1)] )
            + b->m[(2) * b->mrows * b->mcols * b->mdeps + (i) * b->mcols* b->mdeps + (j) * b->mdeps + (k)]
             *( p->m[(0) * p->mrows * p->mcols * p->mdeps + (i+1) * p->mcols* p->mdeps + (j) * p->mdeps + (k+1)] - p->m[(0) * p->mrows * p->mcols * p->mdeps + (i-1) * p->mcols* p->mdeps + (j) * p->mdeps + (k+1)]
              - p->m[(0) * p->mrows * p->mcols * p->mdeps + (i+1) * p->mcols* p->mdeps + (j) * p->mdeps + (k-1)] + p->m[(0) * p->mrows * p->mcols * p->mdeps + (i-1) * p->mcols* p->mdeps + (j) * p->mdeps + (k-1)] )
            + c->m[(0) * c->mrows * c->mcols * c->mdeps + (i) * c->mcols* c->mdeps + (j) * c->mdeps + (k)] * p->m[(0) * p->mrows * p->mcols * p->mdeps + (i-1) * p->mcols* p->mdeps + (j) * p->mdeps + (k)]
            + c->m[(1) * c->mrows * c->mcols * c->mdeps + (i) * c->mcols* c->mdeps + (j) * c->mdeps + (k)] * p->m[(0) * p->mrows * p->mcols * p->mdeps + (i) * p->mcols* p->mdeps + (j-1) * p->mdeps + (k)]
            + c->m[(2) * c->mrows * c->mcols * c->mdeps + (i) * c->mcols* c->mdeps + (j) * c->mdeps + (k)] * p->m[(0) * p->mrows * p->mcols * p->mdeps + (i) * p->mcols* p->mdeps + (j) * p->mdeps + (k-1)]
            + wrk1->m[(0) * wrk1->mrows * wrk1->mcols * wrk1->mdeps + (i) * wrk1->mcols* wrk1->mdeps + (j) * wrk1->mdeps + (k)];
