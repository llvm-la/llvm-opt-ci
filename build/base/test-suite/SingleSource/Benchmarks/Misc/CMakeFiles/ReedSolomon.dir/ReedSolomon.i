# 1 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/SingleSource/Benchmarks/Misc/ReedSolomon.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 399 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/SingleSource/Benchmarks/Misc/ReedSolomon.c" 2
# 49 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/SingleSource/Benchmarks/Misc/ReedSolomon.c"
...
# 50 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/SingleSource/Benchmarks/Misc/ReedSolomon.c" 2
...
# 51 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/SingleSource/Benchmarks/Misc/ReedSolomon.c" 2
...
# 52 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/SingleSource/Benchmarks/Misc/ReedSolomon.c" 2





static int pp [8 +1] = { 1, 0, 1, 1, 1, 0, 0, 0, 1} ;
static int alpha_to [255 +1], index_of [255 +1], gg [255 -239 +1] ;
static int recd [255], data [239], bb [255 -239] ;
static inited = 0;
...
static void gen_poly()
...

 {
   ...
   for (i=2; i<=255 -239; i++)
    { gg[i] = 1 ;
      for (j=i-1; j>0; j--)
        if (gg[j] != 0) gg[j] = gg[j-1]^ alpha_to[(index_of[gg[j]]+i)%255] ;
        else gg[j] = gg[j-1] ;
      gg[0] = alpha_to[(index_of[gg[0]]+i)%255] ;
    }

   for (i=0; i<=255 -239; i++) gg[i] = index_of[gg[i]] ;
 }
...
static void encode_rs()
...

 {
   ...
   for (i=0; i<255 -239; i++) bb[i] = 0 ;
   for (i=239 -1; i>=0; i--)
    { feedback = index_of[data[i]^bb[255 -239 -1]] ;
       if (feedback != -1)
        { for (j=255 -239 -1; j>0; j--)
            if (gg[j] != -1)
...
# 164 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/SingleSource/Benchmarks/Misc/ReedSolomon.c"
 {
   register int i,j,u,q ;
   int elp[255 -239 +2][255 -239], d[255 -239 +2], l[255 -239 +2], u_lu[255 -239 +2], s[255 -239 +1] ;
   int count=0, syn_error=0, root[8], loc[8], z[8 +1], err[255], reg[8 +1] ;
   ...
   if (syn_error)
    {
# 191 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/SingleSource/Benchmarks/Misc/ReedSolomon.c"
      d[0] = 0 ;
      d[1] = s[1] ;
      elp[0][0] = 0 ;
      elp[1][0] = 1 ;
      ...
      do
      {
        ...
        if (d[u]==-1)
          ...
          { q = u-1 ;
            ...
            for (i=0; i<255 -239; i++) elp[u+1][i] = 0 ;
            for (i=0; i<=l[q]; i++)
              if (elp[q][i]!=-1)
...
int main(void) {
  ...
  int i, j, k;
# 431 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/SingleSource/Benchmarks/Misc/ReedSolomon.c"
  for (i=0; i<150000; ++i) {
