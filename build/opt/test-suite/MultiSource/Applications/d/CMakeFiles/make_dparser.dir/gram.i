# 1 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Applications/d/gram.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 399 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Applications/d/gram.c" 2
...
void d_version(char *);
# 6 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Applications/d/gram.c" 2
...
new_production(Grammar *g, char *name) {
  ...
  do { if (!(&g->productions)->v) { ((&g->productions)->v = (&g->productions)->e)[(&g->productions)->n++] = (p); break; } else if ((&g->productions)->v == ((&g->productions)->e)) { if (((&g->productions)->n < 3)) { (&g->productions)->v[(&g->productions)->n++] = (p); break; } } else if ((&g->productions)->n & ((1 << 3) - 1)) { (&g->productions)->v[(&g->productions)->n++] = (p); break; } vec_add_internal((&g->productions), p); } while (0);
  p->name = name;
...
new_elem_term(Term *t, Rule *r) {
  ...
  e->e.term = t;
  e->rule = r;
  do { if (!(&r->elems)->v) { ((&r->elems)->v = (&r->elems)->e)[(&r->elems)->n++] = (e); break; } else if ((&r->elems)->v == ((&r->elems)->e)) { if (((&r->elems)->n < 3)) { (&r->elems)->v[(&r->elems)->n++] = (e); break; } } else if ((&r->elems)->n & ((1 << 3) - 1)) { (&r->elems)->v[(&r->elems)->n++] = (e); break; } vec_add_internal((&r->elems), e); } while (0);
  return e;
}
...
new_term_string(Grammar *g, char *s, char *e, Rule *r) {
  ...
  t->string[e - s] = 0;
  t->string_len = e - s;
  do { if (!(&g->terminals)->v) { ((&g->terminals)->v = (&g->terminals)->e)[(&g->terminals)->n++] = (t); break; } else if ((&g->terminals)->v == ((&g->terminals)->e)) { if (((&g->terminals)->n < 3)) { (&g->terminals)->v[(&g->terminals)->n++] = (t); break; } } else if ((&g->terminals)->n & ((1 << 3) - 1)) { (&g->terminals)->v[(&g->terminals)->n++] = (t); break; } vec_add_internal((&g->terminals), t); } while (0);
...
unescape_term_string(Term *t) {
 ...
 case 'x':
   length = 0;
   if (((*__ctype_b_loc ())[(int) ((s[2]))] & (unsigned short int) _ISxdigit)) {
     base = 16;
     start = s + 2;
     length++;
     if (((*__ctype_b_loc ())[(int) ((s[3]))] & (unsigned short int) _ISxdigit))
       length++;
   }
   s += length + 1;
   goto Lncont;
 case 'd':
   length = 0;
   if (((*__ctype_b_loc ())[(int) ((s[2]))] & (unsigned short int) _ISdigit)) {
     base = 10;
     start = s + 2;
     length++;
     if (((*__ctype_b_loc ())[(int) ((s[3]))] & (unsigned short int) _ISdigit)) {
       length++;
       if (((*__ctype_b_loc ())[(int) ((s[4]))] & (unsigned short int) _ISdigit) && ((s[2] < '2') || ((s[2] == '2') && ((s[3] < '5') ||
                   ((s[3] == '5') && (s[4] < '6'))))))
  length++;
     }
   }
 ...
 case '4': case '5': case '6': case '7':
   ...
   base = 8;
   start = s + 1;
   if (((*__ctype_b_loc ())[(int) ((s[2]))] & (unsigned short int) _ISdigit) && (s[2] != '8') && (s[2] != '9')) {
     length++;
     if (((*__ctype_b_loc ())[(int) ((s[3]))] & (unsigned short int) _ISdigit) && (s[3] != '8') && (s[3] != '9')) {
       length++;
     }
   }
   s += length;
 ...
 Lncont:
   if (length > 0) {
     ...
     start[length] = '\0';
     *ss = (char) strtol(start, ((void*)0), base);
...
new_ident(char *s, char *e, Rule *r) {
  ...
  x->rule = r;
  if (r)
    do { if (!(&r->elems)->v) { ((&r->elems)->v = (&r->elems)->e)[(&r->elems)->n++] = (x); break; } else if ((&r->elems)->v == ((&r->elems)->e)) { if (((&r->elems)->n < 3)) { (&r->elems)->v[(&r->elems)->n++] = (x); break; } } else if ((&r->elems)->n & ((1 << 3) - 1)) { (&r->elems)->v[(&r->elems)->n++] = (x); break; } vec_add_internal((&r->elems), x); } while (0);
  return x;
}
...
new_declaration(Grammar *g, Elem *e, uint kind) {
  ...
  do { if (!(&g->declarations)->v) { ((&g->declarations)->v = (&g->declarations)->e)[(&g->declarations)->n++] = (d); break; } else if ((&g->declarations)->v == ((&g->declarations)->e)) { if (((&g->declarations)->n < 3)) { (&g->declarations)->v[(&g->declarations)->n++] = (d); break; } } else if ((&g->declarations)->n & ((1 << 3) - 1)) { (&g->declarations)->v[(&g->declarations)->n++] = (d); break; } vec_add_internal((&g->declarations), d); } while (0);
}
...
add_declaration(Grammar *g, char *start, char *end, uint kind, uint line) {
  ...
  switch (kind) {
    ...
    default:
      new_declaration(g, new_ident(start, end, ((void*)0)), kind);
      break;
  }
}
...
find_pass(Grammar *g, char *start, char *end) {
  int i, l;
  while (*start && ((*__ctype_b_loc ())[(int) ((*start))] & (unsigned short int) _ISspace)) start++;
  l = end - start;
...
add_pass(Grammar *g, char *start, char *end, uint kind, uint line) {
  if (find_pass(g, start, end))
    ...
    do { if (!(&g->passes)->v) { ((&g->passes)->v = (&g->passes)->e)[(&g->passes)->n++] = (p); break; } else if ((&g->passes)->v == ((&g->passes)->e)) { if (((&g->passes)->n < 3)) { (&g->passes)->v[(&g->passes)->n++] = (p); break; } } else if ((&g->passes)->n & ((1 << 3) - 1)) { (&g->passes)->v[(&g->passes)->n++] = (p); break; } vec_add_internal((&g->passes), p); } while (0);
  }
}
       ...
       char *code_start, char *code_end, uint pass_line, uint code_line)
{
  ...
  while (r->pass_code.n <= p->index) do { if (!(&r->pass_code)->v) { ((&r->pass_code)->v = (&r->pass_code)->e)[(&r->pass_code)->n++] = (((void*)0)); break; } else if ((&r->pass_code)->v == ((&r->pass_code)->e)) { if (((&r->pass_code)->n < 3)) { (&r->pass_code)->v[(&r->pass_code)->n++] = (((void*)0)); break; } } else if ((&r->pass_code)->n & ((1 << 3) - 1)) { (&r->pass_code)->v[(&r->pass_code)->n++] = (((void*)0)); break; } vec_add_internal((&r->pass_code), ((void*)0)); } while (0);
...
new_internal_production(Grammar *g, Production *p) {
  ...
  Production *pp = ((void*)0), *tp = ((void*)0), *ttp;
  int i, found = 0;
...
conditional_EBNF(Grammar *g) {
  ...
  rr = new_rule(g, pp);
  do { if (!(&rr->elems)->v) { ((&rr->elems)->v = (&rr->elems)->e)[(&rr->elems)->n++] = (((g->r)->elems.v[(g->r)->elems.n-1])); break; } else if ((&rr->elems)->v == ((&rr->elems)->e)) { if (((&rr->elems)->n < 3)) { (&rr->elems)->v[(&rr->elems)->n++] = (((g->r)->elems.v[(g->r)->elems.n-1])); break; } } else if ((&rr->elems)->n & ((1 << 3) - 1)) { (&rr->elems)->v[(&rr->elems)->n++] = (((g->r)->elems.v[(g->r)->elems.n-1])); break; } vec_add_internal((&rr->elems), ((g->r)->elems.v[(g->r)->elems.n-1])); } while (0);
  ((g->r)->elems.v[(g->r)->elems.n-1])->rule = rr;
  rr->elems.v[rr->elems.n - 1]->rule = rr;
  do { if (!(&pp->rules)->v) { ((&pp->rules)->v = (&pp->rules)->e)[(&pp->rules)->n++] = (rr); break; } else if ((&pp->rules)->v == ((&pp->rules)->e)) { if (((&pp->rules)->n < 3)) { (&pp->rules)->v[(&pp->rules)->n++] = (rr); break; } } else if ((&pp->rules)->n & ((1 << 3) - 1)) { (&pp->rules)->v[(&pp->rules)->n++] = (rr); break; } vec_add_internal((&pp->rules), rr); } while (0);
  do { if (!(&pp->rules)->v) { ((&pp->rules)->v = (&pp->rules)->e)[(&pp->rules)->n++] = (new_rule(g, pp)); break; } else if ((&pp->rules)->v == ((&pp->rules)->e)) { if (((&pp->rules)->n < 3)) { (&pp->rules)->v[(&pp->rules)->n++] = (new_rule(g, pp)); break; } } else if ((&pp->rules)->n & ((1 << 3) - 1)) { (&pp->rules)->v[(&pp->rules)->n++] = (new_rule(g, pp)); break; } vec_add_internal((&pp->rules), new_rule(g, pp)); } while (0);
  ((g->r)->elems.v[(g->r)->elems.n-1]) = new_elem_nterm(pp, g->r);
}
...
star_EBNF(Grammar *g) {
  ...
  if (!g->right_recursive_BNF) {
    do { if (!(&rr->elems)->v) { ((&rr->elems)->v = (&rr->elems)->e)[(&rr->elems)->n++] = (new_elem_nterm(pp, rr)); break; } else if ((&rr->elems)->v == ((&rr->elems)->e)) { if (((&rr->elems)->n < 3)) { (&rr->elems)->v[(&rr->elems)->n++] = (new_elem_nterm(pp, rr)); break; } } else if ((&rr->elems)->n & ((1 << 3) - 1)) { (&rr->elems)->v[(&rr->elems)->n++] = (new_elem_nterm(pp, rr)); break; } vec_add_internal((&rr->elems), new_elem_nterm(pp, rr)); } while (0);
    do { if (!(&rr->elems)->v) { ((&rr->elems)->v = (&rr->elems)->e)[(&rr->elems)->n++] = (((g->r)->elems.v[(g->r)->elems.n-1])); break; } else if ((&rr->elems)->v == ((&rr->elems)->e)) { if (((&rr->elems)->n < 3)) { (&rr->elems)->v[(&rr->elems)->n++] = (((g->r)->elems.v[(g->r)->elems.n-1])); break; } } else if ((&rr->elems)->n & ((1 << 3) - 1)) { (&rr->elems)->v[(&rr->elems)->n++] = (((g->r)->elems.v[(g->r)->elems.n-1])); break; } vec_add_internal((&rr->elems), ((g->r)->elems.v[(g->r)->elems.n-1])); } while (0);
    ((g->r)->elems.v[(g->r)->elems.n-1]) = new_elem_nterm(pp, g->r);
    ((rr)->elems.v[(rr)->elems.n-1])->rule = rr;
  } else {
...
plus_EBNF(Grammar *g) {
  ...
  rr = new_rule(g, pp);
  if (!g->right_recursive_BNF) {
    elem = ((g->r)->elems.v[(g->r)->elems.n-1]);
    do { if (!(&rr->elems)->v) { ((&rr->elems)->v = (&rr->elems)->e)[(&rr->elems)->n++] = (new_elem_nterm(pp, rr)); break; } else if ((&rr->elems)->v == ((&rr->elems)->e)) { if (((&rr->elems)->n < 3)) { (&rr->elems)->v[(&rr->elems)->n++] = (new_elem_nterm(pp, rr)); break; } } else if ((&rr->elems)->n & ((1 << 3) - 1)) { (&rr->elems)->v[(&rr->elems)->n++] = (new_elem_nterm(pp, rr)); break; } vec_add_internal((&rr->elems), new_elem_nterm(pp, rr)); } while (0);
    do { if (!(&rr->elems)->v) { ((&rr->elems)->v = (&rr->elems)->e)[(&rr->elems)->n++] = (dup_elem(elem, rr)); break; } else if ((&rr->elems)->v == ((&rr->elems)->e)) { if (((&rr->elems)->n < 3)) { (&rr->elems)->v[(&rr->elems)->n++] = (dup_elem(elem, rr)); break; } } else if ((&rr->elems)->n & ((1 << 3) - 1)) { (&rr->elems)->v[(&rr->elems)->n++] = (dup_elem(elem, rr)); break; } vec_add_internal((&rr->elems), dup_elem(elem, rr)); } while (0);
    ((g->r)->elems.v[(g->r)->elems.n-1]) = new_elem_nterm(pp, g->r);
  } else {
    elem = ((g->r)->elems.v[(g->r)->elems.n-1]);
    do { if (!(&rr->elems)->v) { ((&rr->elems)->v = (&rr->elems)->e)[(&rr->elems)->n++] = (dup_elem(elem, rr)); break; } else if ((&rr->elems)->v == ((&rr->elems)->e)) { if (((&rr->elems)->n < 3)) { (&rr->elems)->v[(&rr->elems)->n++] = (dup_elem(elem, rr)); break; } } else if ((&rr->elems)->n & ((1 << 3) - 1)) { (&rr->elems)->v[(&rr->elems)->n++] = (dup_elem(elem, rr)); break; } vec_add_internal((&rr->elems), dup_elem(elem, rr)); } while (0);
  ...
  do { if (!(&pp->rules)->v) { ((&pp->rules)->v = (&pp->rules)->e)[(&pp->rules)->n++] = (rr); break; } else if ((&pp->rules)->v == ((&pp->rules)->e)) { if (((&pp->rules)->n < 3)) { (&pp->rules)->v[(&pp->rules)->n++] = (rr); break; } } else if ((&pp->rules)->n & ((1 << 3) - 1)) { (&pp->rules)->v[(&pp->rules)->n++] = (rr); break; } vec_add_internal((&pp->rules), rr); } while (0);
  rr = new_rule(g, pp);
  do { if (!(&rr->elems)->v) { ((&rr->elems)->v = (&rr->elems)->e)[(&rr->elems)->n++] = (elem); break; } else if ((&rr->elems)->v == ((&rr->elems)->e)) { if (((&rr->elems)->n < 3)) { (&rr->elems)->v[(&rr->elems)->n++] = (elem); break; } } else if ((&rr->elems)->n & ((1 << 3) - 1)) { (&rr->elems)->v[(&rr->elems)->n++] = (elem); break; } vec_add_internal((&rr->elems), elem); } while (0);
  elem->rule = rr;
  do { if (!(&pp->rules)->v) { ((&pp->rules)->v = (&pp->rules)->e)[(&pp->rules)->n++] = (rr); break; } else if ((&pp->rules)->v == ((&pp->rules)->e)) { if (((&pp->rules)->n < 3)) { (&pp->rules)->v[(&pp->rules)->n++] = (rr); break; } } else if ((&pp->rules)->n & ((1 << 3) - 1)) { (&pp->rules)->v[(&pp->rules)->n++] = (rr); break; } vec_add_internal((&pp->rules), rr); } while (0);
}
...
initialize_productions(Grammar *g) {
  ...
  rrr = new_rule(g, ppp);
  do { if (!(&rrr->elems)->v) { ((&rrr->elems)->v = (&rrr->elems)->e)[(&rrr->elems)->n++] = (new_elem_nterm(((void*)0), rrr)); break; } else if ((&rrr->elems)->v == ((&rrr->elems)->e)) { if (((&rrr->elems)->n < 3)) { (&rrr->elems)->v[(&rrr->elems)->n++] = (new_elem_nterm(((void*)0), rrr)); break; } } else if ((&rrr->elems)->n & ((1 << 3) - 1)) { (&rrr->elems)->v[(&rrr->elems)->n++] = (new_elem_nterm(((void*)0), rrr)); break; } vec_add_internal((&rrr->elems), new_elem_nterm(((void*)0), rrr)); } while (0);
  do { if (!(&ppp->rules)->v) { ((&ppp->rules)->v = (&ppp->rules)->e)[(&ppp->rules)->n++] = (rrr); break; } else if ((&ppp->rules)->v == ((&ppp->rules)->e)) { if (((&ppp->rules)->n < 3)) { (&ppp->rules)->v[(&ppp->rules)->n++] = (rrr); break; } } else if ((&ppp->rules)->n & ((1 << 3) - 1)) { (&ppp->rules)->v[(&ppp->rules)->n++] = (rrr); break; } vec_add_internal((&ppp->rules), rrr); } while (0);
...
finish_productions(Grammar *g) {
  ...
  do { if (!(&rr->elems)->v) { ((&rr->elems)->v = (&rr->elems)->e)[(&rr->elems)->n++] = (new_elem_nterm(((void*)0), rr)); break; } else if ((&rr->elems)->v == ((&rr->elems)->e)) { if (((&rr->elems)->n < 3)) { (&rr->elems)->v[(&rr->elems)->n++] = (new_elem_nterm(((void*)0), rr)); break; } } else if ((&rr->elems)->n & ((1 << 3) - 1)) { (&rr->elems)->v[(&rr->elems)->n++] = (new_elem_nterm(((void*)0), rr)); break; } vec_add_internal((&rr->elems), new_elem_nterm(((void*)0), rr)); } while (0);
  do { if (!(&pp->rules)->v) { ((&pp->rules)->v = (&pp->rules)->e)[(&pp->rules)->n++] = (rr); break; } else if ((&pp->rules)->v == ((&pp->rules)->e)) { if (((&pp->rules)->n < 3)) { (&pp->rules)->v[(&pp->rules)->n++] = (rr); break; } } else if ((&pp->rules)->n & ((1 << 3) - 1)) { (&pp->rules)->v[(&pp->rules)->n++] = (rr); break; } vec_add_internal((&pp->rules), rr); } while (0);
...
void
print_term(Term *t) {
  char *s = t->string ? escape_string(t->string) : ((void*)0);
  if (t->kind == TERM_STRING) {
...
make_elems_for_productions(Grammar *g) {
 ...
 if (j >= g->productions.n) {
   ...
   g->productions.v[i]->elem->rule->index = g->rule_index++;
 }
      }
    }
  if (!g->states_for_all_nterms &&
      ...
      (ppp = lookup_production(g, "whitespace", sizeof("whitespace")-1)))
  {
    ...
    ppp->elem->rule->index = g->rule_index++;
  }
}
...
propogate_declarations(Grammar *g) {
  ...
  if (g->tokenizer)
    new_declaration(g, new_elem_nterm(g->productions.v[0], ((void*)0)), DECLARE_TOKENIZE);
  if (g->longest_match)
    new_declaration(g, new_elem_nterm(g->productions.v[0], ((void*)0)), DECLARE_LONGEST_MATCH);
...
merge_shift_actions(State *to, State *from) {
 ...
 goto Lnext;
    do { if (!(&to->shift_actions)->v) { ((&to->shift_actions)->v = (&to->shift_actions)->e)[(&to->shift_actions)->n++] = (from->shift_actions.v[i]); break; } else if ((&to->shift_actions)->v == ((&to->shift_actions)->e)) { if (((&to->shift_actions)->n < 3)) { (&to->shift_actions)->v[(&to->shift_actions)->n++] = (from->shift_actions.v[i]); break; } } else if ((&to->shift_actions)->n & ((1 << 3) - 1)) { (&to->shift_actions)->v[(&to->shift_actions)->n++] = (from->shift_actions.v[i]); break; } vec_add_internal((&to->shift_actions), from->shift_actions.v[i]); } while (0);
  Lnext:;
  }
}
