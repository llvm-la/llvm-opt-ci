# 1 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Applications/d/scan.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 399 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Applications/d/scan.c" 2
...
void d_version(char *);
# 6 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Applications/d/scan.c" 2
...
scan_buffer(d_loc_t *loc, D_State *parse_state, ShiftResult *results) {
  ...
  int nresults = 0, i, j;
  D_Shift **shift = ((void*)0);

  if (parse_state->scan_kind != 1) {
    ...
    uint32 sb, so;
    c = (uint8)*s++;
    while ((state = st[state].scanner_block[(sb = (c >> (8 - 2)))]
     [(so = c & ((1 << (8 - 2)) - 1))]))
    {
      state -= 1;
      if (st[state].shift) {
 ...
 results[nresults++].shift = *shift;
    ...
    if (nresults && parse_state->scan_kind == 2) {
      char *end = results[nresults-1].loc.s;
      int longest = 0;
      for (i = nresults - 1; i >= 0; i--) {
 if (results[i].shift->shift_kind == 1)
   longest = 1;
 ...
 for (j = i; j >= 0; j--)
   if (results[j].shift->shift_kind != 1) {
     if (i != j)
       results[i] = results[j];
     i--;
   }
