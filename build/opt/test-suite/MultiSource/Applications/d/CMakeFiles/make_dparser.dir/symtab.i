# 1 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Applications/d/symtab.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 399 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Applications/d/symtab.c" 2
...
void d_version(char *);
# 5 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Applications/d/symtab.c" 2
...
typedef struct D_SymHash {
  int index;
  int grow;
  struct { uint n; uint i; D_Sym* *v; D_Sym* e[3]; } syms;
} D_SymHash;
# 43 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Applications/d/symtab.c"
static void
symhash_add(D_SymHash *sh, D_Sym *s) {
  uint i, h = s->hash % sh->syms.n, n;
  D_Sym **v = sh->syms.v, *x;
  struct { uint n; uint i; D_Sym* *v; D_Sym* e[3]; } vv, tv;
  ...
  if (sh->index > sh->grow) {
    ...
    v = sh->syms.v;
    n = sh->syms.n;
    do { (&tv)->n = 0; (&tv)->v = 0; } while(0);
    for (i = 0; i < vv.n; i++)

      while (vv.v[i]) {
 x = vv.v[i];
 vv.v[i] = x->next;
 do { if (!(&tv)->v) { ((&tv)->v = (&tv)->e)[(&tv)->n++] = (x); break; } else if ((&tv)->v == ((&tv)->e)) { if (((&tv)->n < 3)) { (&tv)->v[(&tv)->n++] = (x); break; } } else if ((&tv)->n & ((1 << 3) - 1)) { (&tv)->v[(&tv)->n++] = (x); break; } vec_add_internal((&tv), x); } while (0);
      }
      while (tv.v[i]) {
 x = tv.v[i];
 tv.v[i] = x->next;
