# 1 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Applications/obsequi/display.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 399 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Applications/obsequi/display.c" 2
...
# 4 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Applications/obsequi/display.c" 2
...
{
  if(! (key.key[index/32] & ( 1 << (index%32) )) ){
    ...
    ((_fatal_error_aux("/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Applications/obsequi/display.c", 23, ((1) ? (1) : 256), "HashKey Incorrect.\n" )));
  }
}
...
check_hashkey_bit_not_set(Hash_Key key, s32bit index)
{
  if( (key.key[index/32] & ( 1 << (index%32) )) ){
    ...
    ((_fatal_error_aux("/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Applications/obsequi/display.c", 38, ((1) ? (1) : 256), "HashKey Incorrect.\n" )));
  }
}
...
check_hashkey_code(Hash_Key key)
{
  ...
  n_rows = g_board_size[0], n_cols = g_board_size[1];
  ...
  if(code != 0){
    ((_fatal_error_aux("/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Applications/obsequi/display.c", 59, ((1) ? (1) : 256), "Invalid hash code.\n" )));
  }
}
...
check_board_sanity()
{
  ...
  for(j = 0; j < g_board_size[0] + 2; j++)
    for(i = 0; i < g_board_size[1] + 2; i++){
      count = 0;

      if(g_board[1][i] & ( 1 << (j) )) count++;
      if(g_board[0][j] & ( 1 << (i) )) count++;
      ...
      if(count == 1){
        ...
        ((_fatal_error_aux("/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Applications/obsequi/display.c", 136, ((1) ? (1) : 256), "Board is inconsistent.\n" )));
      }
    }
}
# 158 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Applications/obsequi/display.c"
extern void
...
print_board(s32bit player)
{
  ...
  col = g_board_size[player_index^1];

  for(i = 0; i < row; i++){
    for(j = 0; j < col; j++){
      if(g_board[player_index][i+1] & ( 1 << (j+1) ))
        printf(" #");
      else
        printf(" 0");
    }
...
print_board_info(s32bit player)
{
  ...
  num_rows = g_board_size[0];
  num_cols = g_board_size[1];
  max_dim = (((num_rows) > (num_cols)) ? (num_rows) : (num_cols));

  sprintf(str[1], "Number of rows    = %d", g_board_size[0]);
  sprintf(str[2], "Number of columns = %d", g_board_size[1]);
  ...
  for(i = 0; i < max_dim; i++){
    printf("%6d) %7d %7d %7d %7d  %s\n", i + 1,
           g_info[1][i+1].real, g_info[1][i+1].safe,
           g_info[0][i+1].real, g_info[0][i+1].safe,
           (i < 2) ? str[i+1] : null_str);
  }

  printf("Totals: %7d %7d %7d %7d\n",
         g_info_totals[1].real, g_info_totals[1].safe,
         g_info_totals[0].real, g_info_totals[0].safe);
}
...
print_u64bit(u64bit val)
{
  ...
  if(i > 10) ((_fatal_error_aux("/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Applications/obsequi/display.c", 255, ((1) ? (1) : 256), "Too large???\n" )));
...
{
  if(info.bit1_index == -1) ((_fatal_error_aux("/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Applications/obsequi/display.c", 266, ((1) ? (1) : 256), "bit1_index equal to -1" )));
