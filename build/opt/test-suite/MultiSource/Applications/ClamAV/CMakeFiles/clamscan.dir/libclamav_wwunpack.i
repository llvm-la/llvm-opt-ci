# 1 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Applications/ClamAV/libclamav_wwunpack.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 399 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Applications/ClamAV/libclamav_wwunpack.c" 2
# 50 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Applications/ClamAV/libclamav_wwunpack.c"
...
# 51 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Applications/ClamAV/libclamav_wwunpack.c" 2
...
# 54 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Applications/ClamAV/libclamav_wwunpack.c" 2
...
extern size_t strlcat (char *__restrict __dest,
         const char *__restrict __src, size_t __n)
  __attribute__ ((__nothrow__ )) __attribute__ ((__nonnull__ (1, 2))) ;
# 55 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Applications/ClamAV/libclamav_wwunpack.c" 2
...
# 57 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Applications/ClamAV/libclamav_wwunpack.c" 2
...
# 58 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Applications/ClamAV/libclamav_wwunpack.c" 2
# 1 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Applications/ClamAV/wwunpack.h" 1
# 23 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Applications/ClamAV/wwunpack.h"
...
# 24 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Applications/ClamAV/wwunpack.h" 2

int wwunpack(char *, uint32_t, uint32_t, uint32_t, uint32_t, uint32_t, char *, uint32_t, uint16_t);
# 59 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Applications/ClamAV/libclamav_wwunpack.c" 2
...
static int getbitmap(uint32_t *bitmap, char **src, uint8_t *bits, char *buf, unsigned int size) {
  if (! (size > 0 && 4 > 0 && 4 <= size && *src >= buf && *src + 4 <= buf + size && *src + 4 > buf)) return 1;
...
static int wunpsect(char *packed, char *unpacked, unsigned int psize, unsigned int usize) {
  ...
  while (1) {
    ...
    bits--;
    if (!lostbit && bits) {
      if (!((psize > 0 && 1 > 0 && 1 <= psize && src >= packed && src + 1 <= packed + psize && src + 1 > packed) && (usize > 0 && 1 > 0 && 1 <= usize && dst >= unpacked && dst + 1 <= unpacked + usize && dst + 1 > unpacked))) return 1;
      *dst++=*src++;
      continue;
    }
 ...
 *dst++=*src++;
 ...
 backbytes-=0x9f;
      ...
      eax&=0xffff;
      backbytes+=eax;
      if (!((usize > 0 && 2 > 0 && 2 <= usize && dst-backbytes >= unpacked && dst-backbytes + 2 <= unpacked + usize && dst-backbytes + 2 > unpacked) && (usize > 0 && 2 > 0 && 2 <= usize && dst >= unpacked && dst + 2 <= unpacked + usize && dst + 2 > unpacked))) return 1;
      *dst=*(dst-backbytes);
      dst++;
      *dst=*(dst-backbytes);
      dst++;
 ...
 if (eax&0xff) {
  ...
  if (getbits(14, &eax, &bitmap, &bits, &src, packed, psize)) return 1;
    ...
    if (!((usize > 0 && backsize > 0 && backsize <= usize && dst-backbytes >= unpacked && dst-backbytes + backsize <= unpacked + usize && dst-backbytes + backsize > unpacked) && (usize > 0 && backsize > 0 && backsize <= usize && dst >= unpacked && dst + backsize <= unpacked + usize && dst + backsize > unpacked))) return 1;
    while(backsize--){
      *dst=*(dst-backbytes);
      dst++;
    }
...
int wwunpack(char *exe, uint32_t exesz, uint32_t headsize, uint32_t min, uint32_t wwprva, uint32_t e_lfanew, char *wwp, uint32_t wwpsz, uint16_t sects) {
  ...
  while(1) {
    if (!(wwpsz > 0 && 17 > 0 && 17 <= wwpsz && stuff >= wwp && stuff + 17 <= wwp + wwpsz && stuff + 17 > wwp)) {
    ...
    if((packed = (char *) cli_calloc(csize, sizeof(char))) == ((void*)0)) {
      cli_dbgmsg("WWPack: Can't allocate %d bytes\n", csize);
      return 1;
    }
    unpacked=exe+headsize+rva-min;
    if (!(exesz > 0 && csize > 0 && csize <= exesz && unpacked >= exe && unpacked + csize <= exe + exesz && unpacked + csize > exe)) {
      free(packed);
  ...
  csize=cli_readint32(stuff+0x50)-(((wwpsz)/0x1000+((wwpsz)%0x1000!=0))*0x1000);
  ...
  while (sects--) {
    ...
    csize=(((((r>v)?r:v)/0x1000+(((r>v)?r:v)%0x1000!=0))*0x1000));
