# 1 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Benchmarks/VersaBench/beamformer/beamformer.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 399 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Benchmarks/VersaBench/beamformer/beamformer.c" 2
# 33 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Benchmarks/VersaBench/beamformer/beamformer.c"
...
# 34 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Benchmarks/VersaBench/beamformer/beamformer.c" 2
...
# 35 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Benchmarks/VersaBench/beamformer/beamformer.c" 2
...
# 36 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Benchmarks/VersaBench/beamformer/beamformer.c" 2
...
# 37 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Benchmarks/VersaBench/beamformer/beamformer.c" 2
# 71 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Benchmarks/VersaBench/beamformer/beamformer.c"
struct BeamFirData
{
  int len, count, pos;
  float *weight, *buffer;
...
# 124 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Benchmarks/VersaBench/beamformer/beamformer.c"
float detector_out_StrictFP[4][((1024/1)/2)];
...
# 179 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Benchmarks/VersaBench/beamformer/beamformer.c"
void begin(void)
{



  struct BeamFirData coarse_fir_data[12];
  struct BeamFirData fine_fir_data[12];
  struct BeamFirData mf_fir_data[4];
  float inputs[2*1024*12];
  float predec[2*(1024/1)*12];
  float postdec[12][2*((1024/1)/2)*12];
  float beam_weights[4][2*12];
  float beam_input[2*12*((1024/1)/2)];
  float beam_output[2*((1024/1)/2)];
  float beam_fir_output[2*((1024/1)/2)];
  float beam_fir_mag[((1024/1)/2)];
  float detector_out[4][((1024/1)/2)];
  int i, j;

  for (i = 0; i < 12; i++)
  {
    BeamFirSetup(&coarse_fir_data[i], 64);
    BeamFirSetup(&fine_fir_data[i], 64);
  }
  for (i = 0; i < 4; i++)
  {
    BeamFirSetup(&mf_fir_data[i], (1*((1024/1)/2)));
  ...
  while (numiters == -1 || numiters-- > 0) {
    for (i = 0; i < 12; i++) {
      for (j = 0; j < 12; j++)
        InputGenerate(i, inputs + j*1024*2,
                      1024);
      for (j = 0; j < (1024/1); j++)
    ...
    for (i = 0; i < 12; i++)
      for (j = 0; j < ((1024/1)/2); j++)
      {
        beam_input[j*12*2+2*i] = postdec[i][2*j];
        beam_input[j*12*2+2*i+1] = postdec[i][2*j+1];
      }
    for (i = 0; i < 4; i++)
    {
      ...
      Magnitude(beam_fir_output, beam_fir_mag, ((1024/1)/2));
...
# 311 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Benchmarks/VersaBench/beamformer/beamformer.c"
  data->weight[0] = 1.0;
  data->weight[1] = 0.0;
  for (i = 1; i < 2*n; i++) {
    data->weight[i] = 0.0;
...
void Detector(int beam, float *data, float *output)
{
  ...
  for (sample = 0; sample < ((1024/1)/2); sample++)
  {
    float outputVal;
    if (beam == (4/4) && sample == (1024/4)) {
      if (data[sample] >= 0.1)
        outputVal = beam+1;
      else
        outputVal = 0;
...
#pragma STDC FP_CONTRACT OFF
  ...
  for (i = 0; i < 12; i++)
  {
    BeamFirSetup_StrictFP(&coarse_fir_data[i], 64);
    BeamFirSetup_StrictFP(&fine_fir_data[i], 64);
  }
  for (i = 0; i < 4; i++)
  {
    BeamFirSetup_StrictFP(&mf_fir_data[i], (1*((1024/1)/2)));
    BeamFormWeights_StrictFP(i, beam_weights[i]);
  }

  for (i = 0; i < 12; i++) {
    for (j = 0; j < 12; j++)
      InputGenerate_StrictFP(i, inputs + j*1024*2,
                             1024);
    for (j = 0; j < (1024/1); j++)
  ...
  for (i = 0; i < 4; i++)
    {
      ...
      for (j = 0; j < ((1024/1)/2); j++)
        BeamFirFilter_StrictFP(&mf_fir_data[i],
                               ((1024/1)/2), 1,
                               beam_output+j*2, beam_fir_output+j*2);
      Magnitude_StrictFP(beam_fir_output, beam_fir_mag, ((1024/1)/2));
...
#pragma STDC FP_CONTRACT OFF
  ...
  data->weight = malloc(sizeof(float)*2*n);
  data->buffer = malloc(sizeof(float)*2*n);
# 549 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Benchmarks/VersaBench/beamformer/beamformer.c"
  data->weight[0] = 1.0;
  data->weight[1] = 0.0;
  for (i = 1; i < 2*n; i++) {
    data->weight[i] = 0.0;
