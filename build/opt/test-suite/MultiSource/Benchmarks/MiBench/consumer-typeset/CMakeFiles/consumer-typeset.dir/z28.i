# 1 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Benchmarks/MiBench/consumer-typeset/z28.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 399 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Benchmarks/MiBench/consumer-typeset/z28.c" 2
# 30 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Benchmarks/MiBench/consumer-typeset/z28.c"
...
# 31 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Benchmarks/MiBench/consumer-typeset/z28.c" 2




static unsigned print_block[20];
static int start_block[20];
static char message[20][512];
static int message_fnum[20];
static FILE *fp = ((void*)0);
static unsigned error_seen = 0;
static int block_top = 0;
static int mess_top = 0;
# 53 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Benchmarks/MiBench/consumer-typeset/z28.c"
void ErrorInit(FULL_CHAR *str)
{ if( fp != ((void*)0) )
    Error(28, 1, "-e argument appears twice in command line", 1, no_fpos);
  fp = fopen( (char *) (str), ("w") );
  if( fp == ((void*)0) )
    Error(28, 2, "cannot open error file %s", 1, no_fpos, str);
}
# 70 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Benchmarks/MiBench/consumer-typeset/z28.c"
unsigned ErrorSeen(void)
{ return error_seen;
}
# 84 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Benchmarks/MiBench/consumer-typeset/z28.c"
...
# 102 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Benchmarks/MiBench/consumer-typeset/z28.c"
void EnterErrorBlock(unsigned ok_to_print)
{ if( block_top < 20 )
  { print_block[block_top] = ok_to_print;
    start_block[block_top] = mess_top;
    block_top++;
  }
  else Error(28, 3, "too many levels of error messages", 1, no_fpos);
}
# 121 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Benchmarks/MiBench/consumer-typeset/z28.c"
void LeaveErrorBlock(unsigned commit)
{ int i;
                                         ;
  { if( !(block_top > 0) ) Error(1, 2, "assert failed in %s", 0, no_fpos, "LeaveErrorBlock: no matching EnterErrorBlock!"); };
  { if( !(commit || !print_block[block_top - 1]) ) Error(1, 2, "assert failed in %s", 0, no_fpos, "LeaveErrorBlock: commit!"); };
  if( fp == ((void*)0) ) fp = stderr;
  if( commit )
...
# 152 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Benchmarks/MiBench/consumer-typeset/z28.c"
void CheckErrorBlocks(void)
{ { if( !(block_top == 0) ) Error(1, 2, "assert failed in %s", 0, no_fpos, "CheckErrorBlocks: block_top != 0!"); };
}
# 166 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Benchmarks/MiBench/consumer-typeset/z28.c"
POINTER Error(int set_num, int msg_num, char *str, int etype, FILE_POS *pos, ...)
{
  va_list ap;
  char val[512];
  __builtin_va_start(ap, pos);
  vsprintf(val, (MsgCat ? catgets(MsgCat, set_num, msg_num, str) : str), ap);
  if( fp == ((void*)0) ) fp = stderr;
  switch( etype )
  {
    ...
    case 0:
      ...
      while( block_top > 0 ) LeaveErrorBlock(1);
      if( AltErrorFormat )
      {
        fprintf(fp, (MsgCat ? catgets(MsgCat, 28, 7, "%s internal error: %s\n") : "%s internal error: %s\n"),
   EchoAltFilePos(pos), val);

      }
      else
      {
        PrintFileBanner((*pos).ofile_num);
        fprintf(fp, (MsgCat ? catgets(MsgCat, 28, 4, "  %6s internal error: %s\n") : "  %6s internal error: %s\n"),
   EchoFileLine(pos), val);
    ...
    case 1:
      ...
      if( AltErrorFormat )
      {
        fprintf(fp, (MsgCat ? catgets(MsgCat, 28, 8, "%s: fatal error: %s\n") : "%s: fatal error: %s\n"),
   EchoAltFilePos(pos), val);
      }
      else
      {
        PrintFileBanner((*pos).ofile_num);
        fprintf(fp, (MsgCat ? catgets(MsgCat, 28, 5, "  %6s: fatal error: %s\n") : "  %6s: fatal error: %s\n"),
   EchoFileLine(pos), val);
      }
 ...
 { message_fnum[mess_top] = (*pos).ofile_num;
   sprintf(message[mess_top++], "  %6s: %s\n",
     EchoFileLine(pos), val);
 }
      }
      else Error(28, 6, "too many error messages", 1, pos);
      error_seen = 1;
      break;
    ...
    default:
      ...
      { if( !(0) ) Error(1, 2, "assert failed in %s", 0, no_fpos, "Error: invalid error type"); };
      break;
