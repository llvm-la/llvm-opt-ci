# 1 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Benchmarks/MiBench/consumer-typeset/z36.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 399 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Benchmarks/MiBench/consumer-typeset/z36.c" 2
# 30 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Benchmarks/MiBench/consumer-typeset/z36.c"
...
# 31 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Benchmarks/MiBench/consumer-typeset/z36.c" 2
# 49 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Benchmarks/MiBench/consumer-typeset/z36.c"
...
{ FULL_CHAR *p, *q;
  ...
  while( *q != '\0' )
  {
    ...
    { for( i = 0; tex_codes[i][0] != '\0'; i += 3 )
      {
 if( StringBeginsWith(q+1, ( (FULL_CHAR *) (tex_codes[i+1]) )) )
   break;
      }
      if( tex_codes[i][0] != '\0' )
      {
 strcpy((char *)(p),(char *)(tex_codes[i+2]));
 p += strlen((char *)(( (FULL_CHAR *) (tex_codes[i+2]) )));
 q += strlen((char *)(( (FULL_CHAR *) (tex_codes[i+1]) ))) + 1;
      }
      else
      {
 Error(36, 1, "in hyphenation file %s, unknown escape sequence (line %d)",
   1, no_fpos, fname, hline_num);
      }
    }
    else *p++ = *q++;
  }
...
# 146 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Benchmarks/MiBench/consumer-typeset/z36.c"
...
{ int magic;
  ...
  int node_lim;
  int node_free;
  FULL_CHAR *string_mem;
  int string_lim;
  int string_first;
} *TRIE;

static TRIE HyphTables[64] = { ((void*)0) };
static unsigned TriedFile[64] = { 0 };
# 463 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Benchmarks/MiBench/consumer-typeset/z36.c"
...
{ TRIE T; int i;
  ...
  if( T == (TRIE) ((void*)0) )
    Error(36, 4, "run out of memory while constructing hyphenation table",
      1, no_fpos);
  T->magic = 5361534; T->class_count = 1;
  for( i = 0; i < 256; i++ ) T->class[i] = 0;
...
# 492 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Benchmarks/MiBench/consumer-typeset/z36.c"
static short NewTrieString(FULL_CHAR *str, TRIE T)
{ short res = T->string_first - strlen((char *)(str)) - 1;
  if( res >= 0 )
  { T->string_first = res; strcpy((char *)(&(T->string_mem[res])),(char *)(str));
  }
  return res;
}
# 509 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Benchmarks/MiBench/consumer-typeset/z36.c"
static int NewTrieNode(TRIE T)
{ int i; int res;
  if( T->node_free + T->class_count > T->node_lim )
    Error(36, 5, "hyphenation trie node limit exceeded", 0, no_fpos);
  res = T->node_free; T->node_free += T->class_count;
  for( i = res; i < T->node_free; i++ ) T->node_mem[i] = 0;
  return res;
}
# 528 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Benchmarks/MiBench/consumer-typeset/z36.c"
static void AddClassToTrie(FULL_CHAR *str, TRIE T)
{ int i;
  { if( !(T->string_first == T->string_lim) ) Error(1, 2, "assert failed in %s", 0, no_fpos, "AddClassToTrie: after insertion"); };
  for( i = 0; str[i] != '\0'; i++ )
...
# 548 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Benchmarks/MiBench/consumer-typeset/z36.c"
static unsigned TrieInsert(FULL_CHAR *key, FULL_CHAR *value, TRIE T,
FULL_CHAR *fname, int hline_num)
{ FULL_CHAR str[512], compressed_value[512];
  int i, curr_node, next_node, pos, ch; short strpos;
                                                      ;


  if( T->node_free == 0 )
  { T->class_count = 4 * ( ((T->class_count) - 1)/(4) + 1 );
    ch = NewTrieNode(T);
  }

  { register FULL_CHAR *p, *q, *prev; prev = (value) - 1; p = (compressed_value); for( q = (value); *q != (FULL_CHAR) '\0'; q++ ) { if( *q != (FULL_CHAR) '0' || q - prev - 1 >= 15 ) { (*p++ = ((q - prev - 1) << 4) | (*q - '0' + 2)); prev = q; } } *p++ = (FULL_CHAR) '\0'; };


  { int i; for( i = 0; key[i] != '\0'; i++ ) if( T->class[key[i]] != 0 ) str[i] = T->class[key[i]]; else Error(36, 2, "in hyphenation file %s, line %d: character (octal %o) is not in any class", 1, no_fpos, fname, hline_num, key[i]); str[i] = '\0'; };
  curr_node = i = 0;
  for(;;)
  {
 ...
 return 0;
    ...
    { pos = - next_node;
      ...
      T->node_mem[curr_node + str[i]] = next_node = NewTrieNode(T)/4;
      T->node_mem[4*next_node + ch] = -(pos+1);
    }


    curr_node = 4*next_node; i++;
  }
}
# 650 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Benchmarks/MiBench/consumer-typeset/z36.c"
static int BeGetInt(FILE *fp, int *pv)
{ int c;
  if ((c = getc(fp)) == (-1)) return -1;
  *pv = (c & 0xFF) << 24;
  if ((c = getc(fp)) == (-1)) return -1;
  *pv |= (c & 0xFF) << 16;
  if ((c = getc(fp)) == (-1)) return -1;
...
# 681 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Benchmarks/MiBench/consumer-typeset/z36.c"
...
# 725 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Benchmarks/MiBench/consumer-typeset/z36.c"
static TRIE TrieRead(LANGUAGE_NUM lnum, unsigned *success)
{ TRIE T; FILE_NUM unpacked_fnum, packed_fnum; OBJECT fname;
  ...
  { if( !(fname == ( (OBJECT) ((void*)0) ) || (((fname)->os1.ou1.os11.otype) == 11 || ((fname)->os1.ou1.os11.otype) == 12)) ) Error(1, 2, "assert failed in %s", 0, no_fpos, "TrieRead: fname!"); };
  if( fname == ( (OBJECT) ((void*)0) ) )
  { *success = 0;
    return (TRIE) ((void*)0);
  }
                                                           ...
                                                           ;
  packed_fnum = DefineFile((fname)->os1.ostring, ( (FULL_CHAR *) (".lp") ),
    &(fname)->os1.ou1.ofpos, 8, 6);
  if( InitializeAll )
  {
  ...
  else
  {
    ...
    packed_fp = OpenFile(packed_fnum, 0, 0);
  }

  if( packed_fp == ((void*)0) )
  {

    FULL_CHAR str[512], key[512], value[512],
    buff[512 +10];
    int bpos, bcount;
                                                             ;
    unpacked_fnum = DefineFile((fname)->os1.ostring, ( (FULL_CHAR *) (".lh") ),
      &(fname)->os1.ou1.ofpos, 7, 6);
    unpacked_fp = OpenFile(unpacked_fnum, 0, 0);
    if( unpacked_fp == ((void*)0) )
    { Error(36, 8, "cannot open hyphenation file %s",
 2, no_fpos, FileName(unpacked_fnum));
      *success = 0;
      return (TRIE) ((void*)0);
    }


    if( fgets( (char *) (str), (512), (unpacked_fp) ) == ((void*)0) ||
        ( !(strcmp((char *)(str), (char *)(( (FULL_CHAR *) ("Lout hyphenation information\n") )))==0) &&
   !(strcmp((char *)(str), (char *)(( (FULL_CHAR *) ("Lout hyphenation placeholder\n") )))==0) )
      )
      Error(36, 9, "header line of hyphenation file %s missing",
 1, no_fpos, FileName(unpacked_fnum));


    if( !(strcmp((char *)(str), (char *)(( (FULL_CHAR *) ("Lout hyphenation information\n") )))==0) )
    { *success = 1;
      return (TRIE) ((void*)0);
    }
    ...
    hline_num = 1;
    length_limit = 0;
    while( fgets( (char *) (buff), (512), (unpacked_fp) ) != ((void*)0) )
    {
      hline_num++; bpos = 0;
 ...
 str[0] != '%' )
      {
 bpos += bcount;
        DecodeEscapes(str, (fname)->os1.ostring, hline_num);

        switch( state )
        {
   case 0:

     if( !(strcmp((char *)(str), (char *)(( (FULL_CHAR *) ("Classes:") )))==0) )
   ...
   case 1:
     ...
     if( (strcmp((char *)(str), (char *)(( (FULL_CHAR *) ("Exceptions:") )))==0) )
     { state = 2;
     }
     else if( (strcmp((char *)(str), (char *)(( (FULL_CHAR *) ("Patterns:") )))==0) )
     { state = 4;
     }
     else if( (strcmp((char *)(str), (char *)(( (FULL_CHAR *) ("LengthLimit:") )))==0) )
     { state = 3;
     }
     else
     { ;
   ...
   case 2:
     ...
     if( (strcmp((char *)(str), (char *)(( (FULL_CHAR *) ("Patterns:") )))==0) )
     { state = 4;
     }
     else if( (strcmp((char *)(str), (char *)(( (FULL_CHAR *) ("LengthLimit:") )))==0) )
     { state = 3;
     ...
     { prev = '8'; j = 0;
       key[j] = '.', value[j++] = prev, prev = '8';
       for( i = 0; str[i] != '\0'; i++ )
       { if( str[i] == '-' ) prev = '9';
         else key[j] = str[i], value[j++] = prev, prev = '8';
       }
       key[j] = '.', value[j++] = prev, prev = '8';
       key[j] = '\0'; value[j] = prev; value[j+1] = '\0';
       if( !TrieInsert(key, value, T, (fname)->os1.ostring, hline_num) )
       {
         Error(36, 11, "hyphenation file %s%s is too large (at line %d)",
    2, &(fname)->os1.ou1.ofpos, (fname)->os1.ostring, ( (FULL_CHAR *) (".lh") ), hline_num);
         *success = 0;
         return (TRIE) ((void*)0);
       }
     }
  ...
  2, &(fname)->os1.ou1.ofpos, (fname)->os1.ostring, ( (FULL_CHAR *) (".lh") ), hline_num);
   ...
   default:
     ...
     { if( !(0) ) Error(1, 2, "assert failed in %s", 0, no_fpos, "TrieRead: state"); };
     break;
 ...
 1, no_fpos, FileName(unpacked_fnum));
    fclose(unpacked_fp);
    CompressTrie(T);
# 947 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Benchmarks/MiBench/consumer-typeset/z36.c"
  }
# 981 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Benchmarks/MiBench/consumer-typeset/z36.c"
                                            ;
  *success = 1;
                                        ;
  return T;
}
# 1013 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Benchmarks/MiBench/consumer-typeset/z36.c"
unsigned ReadHyphTable(LANGUAGE_NUM lnum)
{ unsigned res;
                                            ;
  { if( !(lnum > 0) ) Error(1, 2, "assert failed in %s", 0, no_fpos, "ReadHyphTable: lnum <= 0!"); };
  { if( !(HyphTables[lnum]==(TRIE) ((void*)0) && !TriedFile[lnum]) ) Error(1, 2, "assert failed in %s", 0, no_fpos, "ReadHyphTable!"); };
  HyphTables[lnum] = TrieRead(lnum, &res);
  TriedFile[lnum] = 1;
                                                                    ;
  return res;
}
# 1033 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Benchmarks/MiBench/consumer-typeset/z36.c"
OBJECT Hyphenate(OBJECT x)
{ OBJECT link, y, z, next_link; TRIE T; LANGUAGE_NUM lnum;
  FULL_CHAR str[2048 +2], rate[2048 +3], val[2048 +3],
    *class, *key, *ss, *s, *p, *rem, *lig, *a, *b;
  int start, stop, i, curr_node, next_node, pos;
  unsigned hyphenated, success;
  { if( !((x)->os1.ou1.os11.otype == 17) ) Error(1, 2, "assert failed in %s", 0, no_fpos, "Hyphenate: type(x) != ACAT!"); };
                                                ;
  ...
  { for( y = (link)->os1.olist[1].opred; (y)->os1.ou1.os11.otype == 0; y = (y)->os1.olist[1].opred );
    if( !(((y)->os1.ou1.os11.otype) == 11 || ((y)->os1.ou1.os11.otype) == 12) || (y)->os1.ostring[0] == '\0' || !(y)->os1.ou2.os22.oword_hyph )
    {
      if( (y)->os1.ou1.os11.otype == 1 && ((y)->os5.ogap).omode == 2 )
 ((y)->os5.ogap).onobreak = 0;
      continue;
    }
                                                              ;


    lnum = (y)->os1.ou2.os22.oword_language;
    if( lnum == 0 )
      Error(36, 19, "no current language for word %s",
 1, &(y)->os1.ou1.ofpos, (y)->os1.ostring);
    T = HyphTables[lnum];
    ...
    key = (y)->os1.ostring; class = T->class;
    for( start = 0; class[key[start]] == 1; start++ );
    for( stop = start; class[key[stop]] > 1; stop++ );
 ...
 continue;
      ...
      next_link = (link)->os1.olist[0].osucc;
      z = MakeWord(11, &key[stop+1], &(y)->os1.ou1.ofpos);
      (z)->os1.ou2.os22.oword_font = (y)->os1.ou2.os22.oword_font;
      (z)->os1.ou2.os22.oword_colour = (y)->os1.ou2.os22.oword_colour;
      (z)->os1.ou2.os22.oword_outline = (y)->os1.ou2.os22.oword_outline;
      (z)->os1.ou2.os22.oword_language = (y)->os1.ou2.os22.oword_language;
      (z)->os1.ou2.os22.oword_hyph = (y)->os1.ou2.os22.oword_hyph;
      (z)->os1.ou2.os22.ounderline = (y)->os1.ou2.os22.ounderline;
                                                                            ;
      FontWordSize(z);
      ...
      ( ((z)->os5.ogap).onobreak = 0, ((z)->os5.ogap).omark = 0, ((z)->os5.ogap).ojoin = 1, ((z)->os5.ogap).ounits = 1, ((z)->os5.ogap).omode = 2, ((z)->os5.ogap).owidth = 0 );
      (z)->os1.ou2.os22.ounderline = (y)->os1.ou2.os22.ounderline;
      { { ; { ; if( (zz_size=(zz_lengths[0])) >= ( ((sizeof(struct word_type)+2048) - 1)/(sizeof(ALIGN)) + 1 ) ) Error(1, 1, "word is too long", 1, no_fpos); else if( zz_free[zz_size] == ( (OBJECT) ((void*)0) ) ) zz_hold = GetMemory(zz_size, no_fpos); else { zz_hold = zz_hold = zz_free[zz_size]; ; zz_free[zz_size] = (zz_hold)->os1.olist[0].opred; } }; (zz_hold)->os1.ou1.os11.otype = 0; ; xx_link = (zz_hold)->os1.olist[0].opred = (zz_hold)->os1.olist[0].osucc = (zz_hold)->os1.olist[1].opred = (zz_hold)->os1.olist[1].osucc = zz_hold; }; ( zz_res = (xx_link), zz_hold = (((link)->os1.olist[0].osucc)), zz_hold == ( (OBJECT) ((void*)0) ) ? zz_res : zz_res == ( (OBJECT) ((void*)0) ) ? zz_hold : ( zz_tmp = (zz_hold)->os1.olist[0].opred, (zz_hold)->os1.olist[0].opred = (zz_res)->os1.olist[0].opred, ((zz_res)->os1.olist[0].opred)->os1.olist[0].osucc = zz_hold, (zz_res)->os1.olist[0].opred = zz_tmp, (zz_tmp)->os1.olist[0].osucc = zz_res ) ); ( zz_res = (xx_link), zz_hold = ((z)), zz_hold == ( (OBJECT) ((void*)0) ) ? zz_res : zz_res == ( (OBJECT) ((void*)0) ) ? zz_hold : ( zz_tmp = (zz_hold)->os1.olist[1].opred, (zz_hold)->os1.olist[1].opred = (zz_res)->os1.olist[1].opred, ((zz_res)->os1.olist[1].opred)->os1.olist[1].osucc = zz_hold, (zz_res)->os1.olist[1].opred = zz_tmp, (zz_tmp)->os1.olist[1].osucc = zz_res ) ); };
      { { ; { ; if( (zz_size=(zz_lengths[0])) >= ( ((sizeof(struct word_type)+2048) - 1)/(sizeof(ALIGN)) + 1 ) ) Error(1, 1, "word is too long", 1, no_fpos); else if( zz_free[zz_size] == ( (OBJECT) ((void*)0) ) ) zz_hold = GetMemory(zz_size, no_fpos); else { zz_hold = zz_hold = zz_free[zz_size]; ; zz_free[zz_size] = (zz_hold)->os1.olist[0].opred; } }; (zz_hold)->os1.ou1.os11.otype = 0; ; xx_link = (zz_hold)->os1.olist[0].opred = (zz_hold)->os1.olist[0].osucc = (zz_hold)->os1.olist[1].opred = (zz_hold)->os1.olist[1].osucc = zz_hold; }; ( zz_res = (xx_link), zz_hold = ((z)), zz_hold == ( (OBJECT) ((void*)0) ) ? zz_res : zz_res == ( (OBJECT) ((void*)0) ) ? zz_hold : ( zz_tmp = (zz_hold)->os1.olist[0].opred, (zz_hold)->os1.olist[0].opred = (zz_res)->os1.olist[0].opred, ((zz_res)->os1.olist[0].opred)->os1.olist[0].osucc = zz_hold, (zz_res)->os1.olist[0].opred = zz_tmp, (zz_tmp)->os1.olist[0].osucc = zz_res ) ); ( zz_res = (xx_link), zz_hold = ((MakeWord(11, ( (FULL_CHAR *) ("0ch") ), &(y)->os1.ou1.ofpos))), zz_hold == ( (OBJECT) ((void*)0) ) ? zz_res : zz_res == ( (OBJECT) ((void*)0) ) ? zz_hold : ( zz_tmp = (zz_hold)->os1.olist[1].opred, (zz_hold)->os1.olist[1].opred = (zz_res)->os1.olist[1].opred, ((zz_res)->os1.olist[1].opred)->os1.olist[1].osucc = zz_hold, (zz_res)->os1.olist[1].opred = zz_tmp, (zz_tmp)->os1.olist[1].osucc = zz_res ) ); };
      key[stop + 1] = '\0';
      FontWordSize(y);
    ...
    if( key[stop] != '\0' && class[key[stop]] == 0 ) continue;
 ...
 if( pos < 0 )
 { { register FULL_CHAR *p, *q, xval; int i, skip; q = (val); for( p = (&(T->string_mem[- pos])); *p != (FULL_CHAR) '\0'; p++ ) { ((skip) = (*p) >> 4, (xval) = (*p) & 15); for( i = 0; i < skip; i++ ) *q++ = (FULL_CHAR) '0'; *q++ = (FULL_CHAR) (xval + '0' - 2); } *q++ = (FULL_CHAR) '\0'; ; };
   { FULL_CHAR *p = val, *q = rate+(ss-str); while( *p ) { if( *p > *q ) *q = *p; p++, q++; } };
                                    ;
 }
 ...
 { rem = &(T->string_mem[-next_node]);
   do
   { if( *rem == '\0' )
     { { register FULL_CHAR *p, *q, xval; int i, skip; q = (val); for( p = (rem+1); *p != (FULL_CHAR) '\0'; p++ ) { ((skip) = (*p) >> 4, (xval) = (*p) & 15); for( i = 0; i < skip; i++ ) *q++ = (FULL_CHAR) '0'; *q++ = (FULL_CHAR) (xval + '0' - 2); } *q++ = (FULL_CHAR) '\0'; ; };
       { FULL_CHAR *p = val, *q = rate+(ss-str); while( *p ) { if( *p > *q ) *q = *p; p++, q++; } };
                                        ;
       break;
     }
 ...
 curr_node = 4*next_node; s++;
      }
    } while( *(++ss + 1) != 1 );
                                                              ;


    lig = finfo[(y)->os1.ou2.os22.oword_font].lig_table;
    ...
    for( i = stop - start - 1; i >= 3; i-- )
    {
      ...
      if( ( (rate[i]) & 1 ) )
      { z = MakeWord(11, &key[start+i-1], &(y)->os1.ou1.ofpos);
 (z)->os1.ou2.os22.oword_font = (y)->os1.ou2.os22.oword_font;
 (z)->os1.ou2.os22.oword_colour = (y)->os1.ou2.os22.oword_colour;
 (z)->os1.ou2.os22.oword_outline = (y)->os1.ou2.os22.oword_outline;
 (z)->os1.ou2.os22.oword_language = (y)->os1.ou2.os22.oword_language;
 ...
 hyphenated = 1;
    ...
    if( hyphenated )
    { FontWordSize(y);
      link = (next_link)->os1.olist[0].opred;
    }
