# 1 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Benchmarks/DOE-ProxyApps-C/RSBench/xs_kernel.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 399 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Benchmarks/DOE-ProxyApps-C/RSBench/xs_kernel.c" 2
...
# 2 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Benchmarks/DOE-ProxyApps-C/RSBench/xs_kernel.c" 2
...
double _Complex fast_nuclear_W( double _Complex Z )
{
 ...
 if( cabs(Z) < 6.0 )
 {
  ...
  double _Complex prefactor = 8.124330e+01 * (__extension__ 1.0iF);
  double an[10] = {
   2.758402e-01,
   2.245740e-01,
   1.594149e-01,
  ...
  double _Complex W = (__extension__ 1.0iF) * ( 1 - cexp((__extension__ 1.0iF)*12.*Z) ) / (12. * Z );
  double _Complex sum = 0;
  for( int n = 0; n < 10; n++ )
  {
   _Complex double top = neg_1n[n] * cexp((__extension__ 1.0iF)*12.*Z) - 1.;
   _Complex double bot = denominator_left[n] - 144.*Z*Z;
   sum += an[n] * (top/bot);
  }
  W += prefactor * Z * sum;
  return W;
 ...
 double _Complex W = (__extension__ 1.0iF) * Z * (a/(Z*Z - b) + c/(Z*Z - d));
...
void calculate_micro_xs( double * micro_xs, int nuc, double E, Input input, CalcDataPtrs data, _Complex double * sigTfactors)
{
 ...
 for( int i = w.start; i < w.end; i++ )
 {
  ...
  PSIIKI = -(0.0 - 1.0 * (__extension__ 1.0iF) ) / ( pole.MP_EA - sqrt(E) );
  CDUM = PSIIKI / E;
...
void calculate_micro_xs_doppler( double * micro_xs, int nuc, double E, Input input, CalcDataPtrs data, _Complex double * sigTfactors, long * abrarov, long * alls)
{
 ...
 for( int i = w.start; i < w.end; i++ )
 {
  ...
  double _Complex Z = (E - pole.MP_EA) * dopp;
  if( cabs(Z) < 6.0 )
   (*abrarov)++;
  (*alls)++;


  _Complex double faddeeva = fast_nuclear_W( Z );
...
void calculate_sig_T( int nuc, double E, Input input, CalcDataPtrs data, _Complex double * sigTfactors )
{
 ...
 for( int i = 0; i < input.numL; i++ )
 {
  ...
  sigTfactors[i] = cos(phi) - sin(phi) * (__extension__ 1.0iF);
 }
}
