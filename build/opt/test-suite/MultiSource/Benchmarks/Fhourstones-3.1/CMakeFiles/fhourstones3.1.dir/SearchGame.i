# 1 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Benchmarks/Fhourstones-3.1/SearchGame.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 399 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Benchmarks/Fhourstones-3.1/SearchGame.c" 2
# 13 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Benchmarks/Fhourstones-3.1/SearchGame.c"
# 1 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Benchmarks/Fhourstones-3.1/TransGame.h" 1
# 23 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Benchmarks/Fhourstones-3.1/TransGame.h"
# 1 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Benchmarks/Fhourstones-3.1/Game.h" 1
# 23 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Benchmarks/Fhourstones-3.1/Game.h"
...
# 24 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Benchmarks/Fhourstones-3.1/Game.h" 2
...
# 25 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Benchmarks/Fhourstones-3.1/Game.h" 2
# 45 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Benchmarks/Fhourstones-3.1/Game.h"
...
# 46 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Benchmarks/Fhourstones-3.1/Game.h" 2
typedef uint64_t uint64;
typedef int64_t int64;

uint64 color[2];
int moves[(6*7)],nplies;
char height[7];
...
uint64 positioncode()
{
  return color[nplies&1] + color[0] + color[1] + ((((uint64)1<<((6 +1)*7))-(uint64)1) / (((uint64)1<<(6 +1))-(uint64)1));
...
{
  return (newboard & (((((uint64)1<<((6 +1)*7))-(uint64)1) / (((uint64)1<<(6 +1))-(uint64)1)) << 6)) == 0;
}
...
{
  uint64 y = newboard & (newboard>>6);
  if ((y & (y >> 2*6)) != 0)
    return 1;
  y = newboard & (newboard>>(6 +1));
  if ((y & (y >> 2*(6 +1))) != 0)
    return 1;
  y = newboard & (newboard>>(6 +2));
  if ((y & (y >> 2*(6 +2))) != 0)
    return 1;
...
# 24 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Benchmarks/Fhourstones-3.1/TransGame.h" 2
# 37 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Benchmarks/Fhourstones-3.1/TransGame.h"
typedef struct {
  unsigned biglock:26;
  unsigned bigwork:6;
  unsigned newlock:26;
...
void trans_init()
{
  ht = (hashentry *)calloc(8306069, sizeof(hashentry));
}
...
void hash()
{
  ...
  if (nplies < 10) {
    uint64 htemp2 = 0;
    for (htmp=htemp; htmp!=0; htmp>>=(6 +1))
      htemp2 = htemp2<<(6 +1) | (htmp & (((uint64)1<<(6 +1))-(uint64)1));
    if (htemp2 < htemp)
      htemp = htemp2;
  }
  lock = (unsigned int)(htemp >> (((6 +1)*7)-26));
  htindex = (unsigned int)(htemp % 8306069);
}
...
void htstat()
{
  ...
  for (total=0,i=1; i<=5; i++)
    total += typecnt[i];
  if (total > 0) {
    printf("- %5.3f  < %5.3f  = %5.3f  > %5.3f  + %5.3f\n",
      typecnt[1]/(double)total, typecnt[2]/(double)total,
      typecnt[3]/(double)total, typecnt[4]/(double)total,
      typecnt[5]/(double)total);
  }
}
# 14 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Benchmarks/Fhourstones-3.1/SearchGame.c" 2
...
# 15 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Benchmarks/Fhourstones-3.1/SearchGame.c" 2
...
void inithistory()
{
  ...
  for (side=0; side<2; side++)
    for (i=0; i<(7 +1)/2; i++)
      for (h=0; h<(6 +1)/2; h++)
        history[side][(6 +1)*i+h] = history[side][(6 +1)*(7 -1-i)+6 -1-h] =
        history[side][(6 +1)*i+6 -1-h] = history[side][(6 +1)*(7 -1-i)+h] =
...
int ab(int alpha, int beta)
{
  ...
  for (i = nav = 0; i < 7; i++) {
    newbrd = other | ((uint64)1 << height[i]);
    if (!islegal(newbrd))
      continue;
  ...
  for (i = 0; i < nav; i++) {
    ...
    for (j = av[l]; l>i; l--)
      av[l] = av[l-1];
    makemove(av[i] = j);
    val = 6 -ab(6 -beta,6 -alpha);
    backmove();
    if (val > score) {
      besti = i;
      if ((score=val) > alpha && nplies >= 0 && (alpha=val) >= beta) {
        if (score == 3 && i < nav-1)
          score = 4;
        if (besti > 0) {
          for (i = 0; i < besti; i++)
            history[side][(int)height[av[i]]]--;
