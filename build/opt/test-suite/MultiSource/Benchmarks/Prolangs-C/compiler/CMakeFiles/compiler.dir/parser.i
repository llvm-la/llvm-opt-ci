# 1 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Benchmarks/Prolangs-C/compiler/parser.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 399 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Benchmarks/Prolangs-C/compiler/parser.c" 2


# 1 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Benchmarks/Prolangs-C/compiler/global.h" 1
...
# 5 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Benchmarks/Prolangs-C/compiler/global.h" 2
...
extern size_t strlcat (char *__restrict __dest,
         const char *__restrict __src, size_t __n)
  __attribute__ ((__nothrow__ )) __attribute__ ((__nonnull__ (1, 2))) ;
# 6 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Benchmarks/Prolangs-C/compiler/global.h" 2
...
# 7 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Benchmarks/Prolangs-C/compiler/global.h" 2
# 89 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Benchmarks/Prolangs-C/compiler/global.h"
int lookahead;
int tokenval;
float ftokenval;
int FloatFlag;
...
int NextTokenval;
float NextFtokenval;
int PreviousLookahead;
int PreviousTokenval;
float PreviousFtokenval;
int Scope;
...
struct entry GlobalTable[100];
struct entry LocalTable[100];
# 4 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Benchmarks/Prolangs-C/compiler/parser.c" 2
...
void PushArrayCellAddr(void)
{
  ...
  if ( !(ArrayParsed))
  {
    ...
    if (LocalIndex)
      emit(2049, LocalIndex, 0.0);
    else if (GlobalIndex)
      emit(2050, GlobalIndex, 0.0);
    else
    {
...
void DeclOrF(void)
{
  ...
  while(1)
  {
    if (lookahead == 259)
    {
      ...
      temp = GlobalInsert ("main",259,2002,0,1,1);
      if (ErrorFlag)
        return;
...
void GlobalScopeAndInsert(int type)
{
  ...
  else
  {
    if (NextLookahead == '[')
    {
      tempGlobalIndex = GlobalIndex = GlobalInsert(lexbuf, 2006, type,0,0,0);
      if (ErrorFlag)
        return;
      match(259);
      if (ErrorFlag)
      ...
      if (ErrorFlag)
        return;
      emit(2048, tempGlobalIndex, 0.0);
      DecCount += GlobalTable[tempGlobalIndex].size;
    }
    else
    {
      GlobalIndex = GlobalInsert(lexbuf, 259, type, 1, 0, 0);
      if (ErrorFlag)
        return;
      emit(2048, GlobalIndex, 0.0);
      ++DecCount;
      match(259);
      if (ErrorFlag)
        return;
...
void GlobalVarList(int type)
{
  while (lookahead == ',')
  {
    ...
    if (ErrorFlag)
      return;
    if (lookahead == 259 || lookahead == 2006)
    {
      GlobalScopeAndInsert (type);
      if (ErrorFlag)
        return;
...
void exprTest(void)
{
  ...
  while ((lookahead == 2012) || (lookahead == 2013))
  {
    ...
    ++LabelCounter;
    emit(2015, tempTrue, 0.0);
    if (tempOperation == 2012)
      emit(2017, 0, 0.0);
    else
      emit(2025, 0, 0.0);
    emit(256, 0, 0.0);
    emit(2015, tempDone, 0.0);
    emit(2018, 0, 0.0);
    emit(2014, tempTrue, 0.0);
    emit(256, 1, 0.0);
    emit(2014, tempDone, 0.0);
  }
}
  ...
  switch (lookahead)
  {
    case 2006:
      ...
      if (NextLookahead == '=')
      {
        ...
        if (tempLocalIndex)
        {
          if ((LocalTable[tempLocalIndex].type == 2002) && (FloatFlag))
          {
            emit(2002, 0, 0.0);
            FloatFlag = 0;
          }
          else if ((LocalTable[tempLocalIndex].type == 2003) && ( !FloatFlag))
          {
            emit(2003, 0, 0.0);
            FloatFlag = 1;
          }
        ...
        {
          if ((GlobalTable[tempGlobalIndex].type == 2002) && (FloatFlag))
          {
            emit(2002, 0, 0.0);
            FloatFlag = 0;
          }
          else if((GlobalTable[tempGlobalIndex].type == 2003) && (!FloatFlag))
          {
            emit(2003, 0, 0.0);
            FloatFlag = 1;
          }
...
void parseAStmt(void)
{
  ...
  switch (lookahead)
  {
    case 2007:
      ...
      ++LabelCounter;
      emit(2014, tempLabel1, 0.0);
      emit(2015, tempLabel2, 0.0);
      match('(');
      if (ErrorFlag)
        return;
      FloatFlag = 0;
      ...
      if (ErrorFlag)
        return;
      emit(2015, tempLabel1, 0.0);
      emit(2018, 0, 0.0);
      emit(2014, tempLabel2, 0.0);
      break;
    case 2008:
      match(2008);
      if (ErrorFlag)
 ...
 ++LabelCounter;
    ...
    case 2042:
      if (NextLookahead == ';')
      {
        ...
        if (ErrorFlag)
          return;
        if (GlobalTable[FuncNameIndex].type == 2002)
          emit(256, 1, 0.0);
 else
          emit(2001, 0, 1.0);
      }
...
int factor(void)
{
  ...
  case 259:
    if (NextLookahead == '(')
    {
      ...
      ++LabelCounter;
      emit(2015, CallReturnAddr, 0.0);
      emit(2059, FuncNameIndex, 0.0);
      emit(2060, 0, 0.0);
      ...
      else if (tempGlobalIndex)
      {
        ...
        {
          emit(2015,GlobalTable[tempGlobalIndex].functionlabel,0.0);
        }
      }
      else
      {
        tempGlobalIndex = GlobalInsert (templexbuf,259,2002,0,1,LabelCounter);
        if (ErrorFlag)
          ErrorFlag = 0;
        ++LabelCounter;
        emit(2015, GlobalTable[tempGlobalIndex].functionlabel, 0.0);
      ...
      emit(2014, CallReturnAddr, 0.0);
      emit(2059, FuncNameIndex, 0.0);
      emit(2061, 0, 0.0);
      if (GlobalTable[tempGlobalIndex].function < 0)
      {
        if (GlobalTable[tempGlobalIndex].type == 2002)
          return(0);
        else
          return(1);
      }
    ...
    if (LocalIndex)
      emit(2051, LocalIndex, 0.0);
    else if (GlobalIndex)
      emit(2052, GlobalIndex, 0.0);
    else
    {
    ...
    if (tempLocalIndex)
    {
      if (LocalTable[tempLocalIndex].type == 2003)
        return(1);
      else
        return(0);
    }
    ...
    {
      if (GlobalTable[tempGlobalIndex].type == 2003)
        return(1);
      else
        return(0);
    }
...
void FunctionDef(int type)
{
  ...
  {
    GlobalIndex = GlobalInsert (lexbuf,259,type,0,-1,LabelCounter);
    if (ErrorFlag)
      ErrorFlag = 0;
    FuncNameIndex = GlobalIndex;
    ++LabelCounter;
  ...
  emit(2054, GlobalIndex, 0.0);
  emit(2014, Scope, 0.0);


  emit(2056, FuncNameIndex, 0.0);
  ...
  if (GlobalTable[FuncNameIndex].type == 2002)
    ...
    emit(2001, 0, 1.0);
  emit(2014, ReturnLabel, 0.0);
  emit(2058, FuncNameIndex, 0.0);
  emit(2018, 0, 0.0);
}
...
void MoreParams(void)
{
  ...
  if (lookahead == ',')
  {
    ...
    {
      tempLocalIndex = LocalIndex = LocalInsert (lexbuf,259,2041,1,0,0);
      if (ErrorFlag)
        return;
    }
    match (259);
    ...
    if (ErrorFlag)
      return;
    emit(2047, tempLocalIndex, 0.0);
    emit(2049, tempLocalIndex, 0.0);
    emit(2055, 0, 0.0);
    ++offset;
  }
}
...
void DecList(void)
{
  while ((lookahead == 2002) || (lookahead == 2003))
  {
    match (lookahead);
    if (ErrorFlag)
      return;
...
void VarList(int type)
{
  ...
  do
  {
    ...
    if (LocalIndex)
    {
      ...
      if (LocalTable[LocalIndex].type == 2041)
      {
        if (NextLookahead == '[')
    ...
    else
    {
      if (NextLookahead == '[')
      {
        tempLocalIndex = LocalIndex = LocalInsert (lexbuf,2006,type,0,0,0);
        if (ErrorFlag)
          return;
        lookahead = 2006;
        match (2006);
      ...
      {
        LocalIndex = LocalInsert(lexbuf,259,type,1,0,0);
        if (ErrorFlag)
          return;
        emit(2047, LocalIndex, 0.0);
        ++offset;
        match (259);
        if (ErrorFlag)
          return;
 ...
 } while ((lookahead == 259) || (lookahead == 2006));
