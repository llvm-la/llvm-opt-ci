# 1 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Benchmarks/Prolangs-C/football/common.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 399 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Benchmarks/Prolangs-C/football/common.c" 2
...
# 5 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Benchmarks/Prolangs-C/football/common.c" 2
...
# 6 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Benchmarks/Prolangs-C/football/common.c" 2
# 1 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Benchmarks/Prolangs-C/football/definitions.h" 1
# 22 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Benchmarks/Prolangs-C/football/definitions.h"
typedef int TEAM;

typedef int
   SCHED_TYPE[17 +1][(int) 28/2 +1 ][3];

typedef int
   SCORES_TYPE[17 +2][(int) 28/2 +1 ][3];
...
typedef TEAM_INFO TEAMS_STATS[28 +1];
typedef TEAM_INFO H_TO_H_STATS[28 +1][28 +1];
# 60 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Benchmarks/Prolangs-C/football/definitions.h"
typedef int DIVISION_TYPE[28 +1][2];
typedef int TEAM_ORDERING[28 +1];
typedef int ORDERING[28 +1];
# 7 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Benchmarks/Prolangs-C/football/common.c" 2
# 1 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Benchmarks/Prolangs-C/football/stats.h" 1
extern int terminate;
extern int num_games;
extern int iyear;
extern int a_wild_card(TEAM team);
extern int a_champ(TEAM team);
extern int find_name(FILE *output,char name[]);
extern int current_with_disk;
...
extern int team_plays[28 +1][28 +1];
extern H_TO_H_STATS h_to_h_stats;
# 8 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Benchmarks/Prolangs-C/football/common.c" 2

TEAMS_STATS common_games;
TEAM common_teams[28 +1];

extern void display_split(FILE *stream,TEAM *teams,int split,int num);

int matches_any_team(int num,TEAM code)
{
  int i;
  for (i=1;i<=num;i++)
...
void common(int num)
{
  ...
  for (tmp_team=1;tmp_team<=28; tmp_team++)
  ...
  for (temp=1;temp<=28;temp++) {
    if (in_common[temp] < num)
      in_common[temp] = 0;
    else in_common[temp] = 1;
  }
  ...
  for (week = 1; week <= num_games;week++)
    for (game = 1;game<=28/2; game++)
      for (i = 1; i<=2; i++) {
 if ( i == 1) {
   team1 = sched[week][game][2]; site1 = 2;
   team2 = sched[week][game][1]; site2 = 1;
 } else {
   team1 = sched[week][game][1]; site1 = 1;
   team2 = sched[week][game][2]; site2 = 2;
 }
 if ( team1 != 0 && in_common[team1] )
   if (matches_any_team(num,team2))
     if (scores[week][game][2] >= 0 &&
  scores[week][game][1] >=0) {
...
{
  TEAM temp1,temp2;
  int sweap_win=1,sweap_lose=1;

  for (temp1=0;temp1<num;temp1++) {
      sweap_win = sweap_lose = 1;
...
int break_common(TEAM *teams,int num)
{
  ...
  num_best = 0;
  for (i=0;i<num;i++) {
    pct[i] = ((common_games[teams[i]].wins+common_games[teams[i]].loses+common_games[teams[i]].ties)==0 ? 0.0 : ((common_games[teams[i]].wins + .5*common_games[teams[i]].ties)/(common_games[teams[i]].wins+common_games[teams[i]].loses+common_games[teams[i]].ties)));
    if (pct[i] > best_pct) {
      best_pct = pct[i];
      num_best = 0;
    }
...
int div_conf_rec(TEAM *teams,int num,int which)
{
  ...
  for (i=0;i<num;i++) {
    if (which == 1)
      pct[i] = ((team_info_wi_div[teams[i]].wins+team_info_wi_div[teams[i]].loses+team_info_wi_div[teams[i]].ties)==0 ? 0.0 : ((team_info_wi_div[teams[i]].wins + .5*team_info_wi_div[teams[i]].ties)/(team_info_wi_div[teams[i]].wins+team_info_wi_div[teams[i]].loses+team_info_wi_div[teams[i]].ties)));
    else pct[i] = ((team_info_wi_conf[teams[i]].wins+team_info_wi_conf[teams[i]].loses+team_info_wi_conf[teams[i]].ties)==0 ? 0.0 : ((team_info_wi_conf[teams[i]].wins + .5*team_info_wi_conf[teams[i]].ties)/(team_info_wi_conf[teams[i]].wins+team_info_wi_conf[teams[i]].loses+team_info_wi_conf[teams[i]].ties)));
    if (pct[i] > best_pct) {
      best_pct = pct[i];
      num_best = 0;
    }
...
int break_net_points(TEAM *teams,int num,int which)
{
  ...
  for (i=0;i<num;i++) {
    if (which == 1)
      pct[i] = (double) (team_info_wi_div[teams[i]].points_for-team_info_wi_div[teams[i]].points_against);
    else if (which == 0)
      pct[i] = (double) (team_info_wi_conf[teams[i]].points_for-team_info_wi_conf[teams[i]].points_against);
    else pct[i] = (double) (team_info[teams[i]].points_for-team_info[teams[i]].points_against);
    if (pct[i] > best_pct) {
      best_pct = pct[i];
      num_best = 0;
    }
...
void break_a_tie(TEAM *teams,int num,int display)
{
  int ALL_SAME_DIV;
  int value,i;
  ...
  ALL_SAME_DIV = 1;
  for (i=1;i<num;i++)
    ALL_SAME_DIV = ALL_SAME_DIV && (divisions[teams[0]][1] ==
         divisions[teams[i]][1]);
  if (display) {
 ...
 (void) printf("Split because of head to head sweap.\n");
  ...
  if (ALL_SAME_DIV) {
    value = div_conf_rec(teams,num,1);
    if (value != num) {
      if (display) {
 display_split(stdout,teams,value,num);
 (void) printf("Split because of divisional record.\n");
  ...
  value = div_conf_rec(teams,num,0);
  if (value != num) {
    if (display) {
      display_split(stdout,teams,value,num);
  ...
  if (ALL_SAME_DIV) {
    value = break_net_points(teams,num,1);
    if (value != num) {
      if (display) {
 display_split(stdout,teams,value,num);
 (void) printf("Split because of division net points.\n");
    ...
    value = break_net_points(teams,num,0);
    if (value != num) {
      if (display) {
 display_split(stdout,teams,value,num);
 (void) printf("Split because of conference net points.\n");
  ...
  value = break_net_points(teams,num,2);
  if (value != num) {
    if (display) {
      display_split(stdout,teams,value,num);
...
void display_split(FILE *stream,TEAM *teams,int split,int num)
{
  ...
  for (i=0;i<num;i++) {
    (void) fprintf(stream,"%s ",team[teams[i]][1]);
