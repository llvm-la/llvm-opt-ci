# 1 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Benchmarks/Prolangs-C/agrep/sgrep.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 399 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Benchmarks/Prolangs-C/agrep/sgrep.c" 2
...
# 3 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Benchmarks/Prolangs-C/agrep/sgrep.c" 2
...
# 4 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Benchmarks/Prolangs-C/agrep/sgrep.c" 2
...
extern size_t strlcat (char *__restrict __dest,
         const char *__restrict __src, size_t __n)
  __attribute__ ((__nothrow__ )) __attribute__ ((__nonnull__ (1, 2))) ;
# 5 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Benchmarks/Prolangs-C/agrep/sgrep.c" 2
...
# 6 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Benchmarks/Prolangs-C/agrep/sgrep.c" 2
...
# 7 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Benchmarks/Prolangs-C/agrep/sgrep.c" 2
# 24 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Benchmarks/Prolangs-C/agrep/sgrep.c"
...
unsigned char temp[256];
 ...
 if(WORDBOUND) {
  for(i=0; i<128; i++) {
   if(!((*__ctype_b_loc ())[(int) ((i))] & (unsigned short int) _ISalnum)) TR[i] = 128;
  }
 }
 if(WHOLELINE) {
  memcpy(temp, pat, *m);
...
void s_output (unsigned char *text, int *i)
{
int bp;
        if(SILENT) return;
        if(COUNT) {
...
int verify(register int m, register int n, register int D, unsigned char *pat, unsigned char *text)
{
    int A[256], B[256];
    register int last = D;
    register int cost = 0;
    register int k, i, c;
    register int m1 = m+1;
    unsigned char *textend = text+n;
    unsigned char *textbegin = text;
...
void bm(unsigned char *pat, int m, unsigned char *text, unsigned char *textend)
{
register int shift;
...
while (text <= textend) {
 ...
 while(shift) {
  ...
  while(TR[pat[m1 - j]] == TR[*(text - j)]) {
   ...
   if(WORDBOUND) {
    if(TR[*(text+1)] != 128) goto CONT;
    if(TR[*(text-m)] != 128) goto CONT;
   }
   num_of_matched++;
...
void initmask(unsigned char *pattern, unsigned *Mask, register int m, register int D, unsigned *endposition)
{
...
void prep(unsigned char *Pattern, register int M, register int D)
{
...
void agrep( register unsigned char *pat, int M, register unsigned char *text, register unsigned char *textend, int D )
{
  register int i;
  register int m = M/(D+1);
  register unsigned char *textstart;
  register int shift, HASH;
  int j=0, k, d1;
  int n, cdx;
  int Candidate[2048][2], round, lastend=0;
  unsigned R1[20 +1], R2[20 +1];
...
void a_monkey( register unsigned char *pat, register int m, register unsigned char *text, register unsigned char *textend,
               register int D )
{
...
void am_preprocess(unsigned char *Pattern)
{
int i, m;
 m = strlen(Pattern);
 for (i = 1, Hashmask = 1 ; i<16 ; i++) Hashmask = (Hashmask << 1) + 1 ;
 for (i = 0; i < 65536; i++) MEMBER_1[i] = 0;
 for (i = m-1; i>=0; i--) {
  MEMBER_1[Pattern[i]] = 1;
        }
 for (i = m-1; i > 0; i--) {
     MEMBER_1[(Pattern[i] << 8) + Pattern[i-1]] = 1;
 }
}



void m_preprocess(unsigned char *Pattern)
{
int i, j, m;
unsigned hash;
 m = strlen(Pattern);
 for (i = 0; i < 4096; i++) SHIFT_2[i] = m;
 for (i = m-1; i>=1; i--) {
  hash = Pattern[i];
  hash = hash << 3;
  for (j = 0; j< 256; j++) {
...
register int m1=m-1-D, pos;
  ...
  while (text < textend) {
     ...
     while(suffix_error <= D) {
 hash = char_map[*text--];
 hash = ((hash << 3) + char_map[*(text--)]) & hashmask;
 while(MEMBER_D[hash]) {
       hash = ((hash << 3) + char_map[*(text--)]) & hashmask;
 }
 suffix_error++;
...
char_map['n'] = char_map['n'] = 5;
 ...
 BSize = blog(4, m);
 for (i = 1, Hashmask = 1 ; i<BSize*3; i++) Hashmask = (Hashmask << 1) + 1 ;
 ...
 for (j=0; j < BSize; j++) {
            for(i=m-1; i >= j; i--) {
               hash = 0;
        for(k=0; k <= j; k++)
    hash = (hash << 3) +char_map[Pattern[i-k]];
...
void sgrep(unsigned char *pat, int m, int fd, int D)
{
    unsigned char text[8192 +2*1024 +256];
    int offset = 2*1024;
    ...
    if(D == 0) {
 if(m > 20) m_preprocess(pat);
 else prep_bm(pat, m);
    }
    else if (DNA) prep4(pat, m);
  else if(m >= 24) am_preprocess(pat);
  else {
   prep(pat, m, D);
   initmask(pat, Mask, m, 0, &endposition);
  }
    for(i=1; i<=m; i++) text[8192 +offset+i] = pat[m-1];

    while( (num_read = read(fd, text+offset, 8192)) > 0)
    {
       ...
       text[start-1] = '\n';
       if(D==0) {
  if(m > 20) monkey(pat, m, text+start, text+end);
  else bm(pat, m, text+start, text+end);
       }
       else {
  if(DNA) monkey4( pat, m, text+start, text+end, D );
  else {
    if(m >= 24) a_monkey(pat, m, text+start, text+end, D);
