# 1 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Benchmarks/Prolangs-C/agrep/preprocess.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 399 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Benchmarks/Prolangs-C/agrep/preprocess.c" 2
# 15 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Benchmarks/Prolangs-C/agrep/preprocess.c"
...
# 16 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Benchmarks/Prolangs-C/agrep/preprocess.c" 2
...
extern size_t strlcat (char *__restrict __dest,
         const char *__restrict __src, size_t __n)
  __attribute__ ((__nothrow__ )) __attribute__ ((__nonnull__ (1, 2))) ;
# 17 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Benchmarks/Prolangs-C/agrep/preprocess.c" 2
...
# 18 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Benchmarks/Prolangs-C/agrep/preprocess.c" 2

extern int SIMPLEPATTERN, WHOLELINE, REGEX, RE_ERR, DELIMITER, TAIL, WORDBOUND;
extern int HEAD;
extern unsigned char Progname[];
extern int D_length;
extern int table[32][32];

extern int init(char *s, int table[32][32]);

void preprocess(unsigned char *D_pattern, unsigned char *Pattern)
{
  unsigned char temp[1024], *r_pat, *old_pat;
  ...
  for(i=0; i< m; i++) {
      if(Pattern[i] == '\\') i++;
      else if(Pattern[i] == '|' || Pattern[i] == '*' ) REGEX = 1;
  }
  r_pat = (unsigned char *) malloc(strlen(Pattern)+2*strlen(D_pattern));
  strcpy(temp, D_pattern);
  d_end = t = strlen(temp);
  if (WHOLELINE) { temp[t++] = 246;
                   temp[t++] = 239;
                   temp[t++] = 247;
                   temp[t] = '\0';
                   strcat(temp, Pattern);
     ...
     if (WORDBOUND) { temp[t++] = 246;
                      temp[t++] = 241;
                      temp[t++] = 247;
                      temp[t] = '\0'; }
     strcat(temp, Pattern);
     m = strlen(temp);
     if (WORDBOUND) { temp[m++] = 246;
                      temp[m++] = 241;
                      temp[m++] = 247; }
     temp[m] = '\0';
  }
  ...
  for (i=d_end; i < m ; i++)
  {
       switch(temp[i])
       {
        ...
        IN_RANGE = 0;
           ...
           case '>': Pattern[j++] = 247;
                      break;
           case '^': if (temp[i-1] == '[') Pattern[j++] = 248;
                      else Pattern[j++] = '\n';
                      r_pat[rp++] = '^';
                      break;
           case '$': Pattern[j++] = '\n';
