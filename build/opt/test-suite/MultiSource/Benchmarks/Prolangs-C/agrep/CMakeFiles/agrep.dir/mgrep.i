# 1 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Benchmarks/Prolangs-C/agrep/mgrep.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 399 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Benchmarks/Prolangs-C/agrep/mgrep.c" 2
...
# 4 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Benchmarks/Prolangs-C/agrep/mgrep.c" 2
...
# 5 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Benchmarks/Prolangs-C/agrep/mgrep.c" 2
...
extern size_t strlcat (char *__restrict __dest,
         const char *__restrict __src, size_t __n)
  __attribute__ ((__nothrow__ )) __attribute__ ((__nonnull__ (1, 2))) ;
# 6 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Benchmarks/Prolangs-C/agrep/mgrep.c" 2
...
# 7 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Benchmarks/Prolangs-C/agrep/mgrep.c" 2
...
# 8 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Benchmarks/Prolangs-C/agrep/mgrep.c" 2
# 20 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Benchmarks/Prolangs-C/agrep/mgrep.c"
...
int SHORT = 0;
int p_size=0;
unsigned char SHIFT1[4096];
unsigned char tr[256];
unsigned char tr1[256];
struct pat_list {
 int index;
 struct pat_list *next;
} *HASH[8192];
struct pat_list *pt, *qt;
unsigned char buf[260000 +8192];
unsigned char pat_spool[260000 +2*30000 +256];
unsigned char *patt[30000];
unsigned char pat_len[30000];
...
void prepf(int fp)
{
 ...
 if(length > 260000) {
  ...
  exit(2);
    ...
    while(i<length) {
 patt[p] = pat_ptr;
 if(WORDBOUND) *pat_ptr++ = 128;
 if(WHOLELINE) *pat_ptr++ = 10;
 while((*pat_ptr = buf[i++]) != '\n') pat_ptr++;
 if(WORDBOUND) *pat_ptr++ = 128;
 if(WHOLELINE) *pat_ptr++ = 10;
 *pat_ptr++ = 0;
 ...
 exit(2);
    ...
    for(i=1; i<20; i++) *pat_ptr = i;
    for(i=0; i< 256; i++) tr[i] = i;
    if(NOUPPER) {
 for(i='A'; i<= 'Z'; i++) tr[i] = i + 'a' - 'A';
    }
    if(WORDBOUND) {
 for(i=0; i<128; i++) if(!((*__ctype_b_loc ())[(int) ((i))] & (unsigned short int) _ISalnum)) tr[i] = 128;
    }
    for(i=0; i< 256; i++) tr1[i] = tr[i]&Mask;
    num_pat = p-1;
    p_size = 256;
    for(i=1 ; i <= num_pat; i++) {
 p = strlen(patt[i]);
 ...
 exit(2);
    ...
    if(p_size == 1) SHORT = 1;
    for(i=0; i<4096; i++) SHIFT1[i] = p_size - 2;
    for(i=0; i<8192; i++) {
 HASH[i] = 0;
    }
...
# 264 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Benchmarks/Prolangs-C/agrep/mgrep.c"
      }
                 }
   if(MATCHED) break;
  }
...
void mgrep(int fd)
{
    ...
    unsigned char text[2*8192 +1024];
    register int buf_end, num_read, start, end, residue = 0;

    text[1024 -1] = '\n';
    start = 1024 -1;

    while( (num_read = read(fd, text+1024, 8192)) > 0)
    {
       if(INVERSE && COUNT) countline(text+1024, num_read);
       buf_end = end = 1024 + num_read -1 ;
       while(text[end] != r_newline && end > 1024) end--;
