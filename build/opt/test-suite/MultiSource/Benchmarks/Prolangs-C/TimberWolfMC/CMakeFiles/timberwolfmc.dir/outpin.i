# 1 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Benchmarks/Prolangs-C/TimberWolfMC/outpin.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 399 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Benchmarks/Prolangs-C/TimberWolfMC/outpin.c" 2
...
# 2 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Benchmarks/Prolangs-C/TimberWolfMC/outpin.c" 2
...
for( cell = numcells + 1 ; cell <= numcells + numpads ; cell++ ) {
    if( cellarray[cell]->padside == 1 ) {
 c[1] = 1 ;
 break ;
    }
}
for( cell = numcells + 1 ; cell <= numcells + numpads ; cell++ ) {
    if( cellarray[cell]->padside == 2 ) {
 c[2] = 1 ;
 break ;
    }
}
for( cell = numcells + 1 ; cell <= numcells + numpads ; cell++ ) {
    if( cellarray[cell]->padside == 3 ) {
 c[3] = 1 ;
 break ;
    }
}
for( cell = numcells + 1 ; cell <= numcells + numpads ; cell++ ) {
    if( cellarray[cell]->padside == 4 ) {
 c[4] = 1 ;
 break ;
    }
}
...
for( net = 1 ; net <= numnets ; net++ ) {
    count = 0 ;
    dimptr = netarray[net] ;
    for( netptr = dimptr->netptr ; netptr != (NETBOXPTR) ((void*)0) ;
 ...
 if( netptr->cell <= numcells ) {
     ...
     if( cellarray[netptr->cell]->padside == 1 ) {
  cell = c[1] ;
     } else if( cellarray[netptr->cell]->padside == 2 ) {
  cell = c[2] ;
     } else if( cellarray[netptr->cell]->padside == 3 ) {
  cell = c[3] ;
     } else if( cellarray[netptr->cell]->padside == 4 ) {
  cell = c[4] ;
     }
 }
