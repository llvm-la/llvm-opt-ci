# 1 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Benchmarks/MallocBench/espresso/sharp.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 398 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Benchmarks/MallocBench/espresso/sharp.c" 2
...
# 6 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Benchmarks/MallocBench/espresso/sharp.c" 2

long start_time;



pset_family cv_sharp(A, B)
pset_family A, B;
{
    pset last, p;
    pset_family T;
...
pset_family sharp(a, b)
pset a, b;
{
    register int var;
    register pset d=cube.temp[0], temp=cube.temp[1], temp1=cube.temp[2];
    pset_family r = sf_new(cube.num_vars, cube.size);
 ...
 for(var = 0; var < cube.num_vars; var++) {
  ...
  set_or(((r)->data + (r)->wsize * (r->count++)), temp, temp1);
     }
 }
    } else {
 r = sf_addset(r, a);
...
pset_family T;
{
 ...
 set_copy(((Y)->data + (Y)->wsize * (Y->count++)), c);
 for(p=T->data,last=p+T->count*T->wsize;p<last;p+=T->wsize) {
     Y1 = cb1_dsharp(Y, p);
     sf_free(Y);
     Y = Y1;
...
pset_family dsharp(a, b)
pset a, b;
{
    register pset mask, diff, and, temp, temp1 = cube.temp[0];
    int var;
    pset_family r;
    ...
    if (cdist0(a, b)) {
 diff = set_diff(set_clear(((unsigned int *) malloc(sizeof(unsigned int) * (((cube.size) <= 32 ? 2 : (((((cube.size)-1) >> 5) + 1) + 1))))), cube.size), a, b);
 and = set_and(set_clear(((unsigned int *) malloc(sizeof(unsigned int) * (((cube.size) <= 32 ? 2 : (((((cube.size)-1) >> 5) + 1) + 1))))), cube.size), a, b);
 ...
 for(var = 0; var < cube.num_vars; var++) {
  ...
  temp = ((r)->data + (r)->wsize * (r->count++));
  (void) set_and(temp, diff, cube.var_mask[var]);


  {register int i_=(and[0] & 0x03ff); (temp1[0] &= ~0x03ff, temp1[0] |= (i_)); do temp1[i_] = and[i_] & mask[i_]; while (--i_>0);};
  {register int i_=(temp[0] & 0x03ff); (temp[0] &= ~0x03ff, temp[0] |= (i_)); do temp[i_] = temp[i_] | temp1[i_]; while (--i_>0);};


  set_or(mask, mask, cube.var_mask[var]);
  {register int i_=(a[0] & 0x03ff); (temp1[0] &= ~0x03ff, temp1[0] |= (i_)); do temp1[i_] = a[i_] & ~ mask[i_]; while (--i_>0);};
  {register int i_=(temp[0] & 0x03ff); (temp[0] &= ~0x03ff, temp[0] |= (i_)); do temp[i_] = temp[i_] | temp1[i_]; while (--i_>0);};
     }
 }
 if ((diff)) { (void) free((char *) (diff)); (diff) = 0; };
 if ((and)) { (void) free((char *) (and)); (and) = 0; };
...
pset_family cv_intersect(A, B)
pset_family A, B;
{
    register pset pi, pj, lasti, lastj, pt;
    pset_family T, Tsave = ((void*)0);


    T = sf_new(500, cube.size);
    pt = T->data;
