# 1 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Benchmarks/MallocBench/espresso/cvrm.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 398 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Benchmarks/MallocBench/espresso/cvrm.c" 2
...
# 10 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Benchmarks/MallocBench/espresso/cvrm.c" 2
      ...
      pset_family B1;
{
 ...
 if ((size = set_dist(c, cube.var_mask[var])) < 2) {
    ...
    offset = B1->count;
    B1->count += expansion;
    for(p=((B1)->data + (B1)->wsize * (offset-1))+B1->wsize,last=B1->data+B1->count*B1->wsize;p<last;p+=B1->wsize) {
 {register int i_=(base[0] & 0x03ff); do p[i_]=base[i_]; while (--i_>=0);};
    }
 ...
 if ((size = set_dist(c, cube.var_mask[var])) > 1) {
  ...
  if ((c[(((i) >> 5) + 1)] & (1 << ((i) & (32 -1))))) {
   ...
   for(k = 0; k < place; k++) {
       p = ((B1)->data + (B1)->wsize * (j+k+offset));
       (void) (p[(((i) >> 5) + 1)] |= 1 << ((i) & (32 -1)));
   }
      }
      n += place;
  }
...
pset_family unravel_range(B, start, end)
   pset_family B;
   int start, end;
{
    pset_family B1;
    int var, total_size, expansion, size;
    register pset p, last, startbase = cube.temp[1];
...
pset_family lex_sort(T)
pset_family T;
{
    pset_family T1 = sf_unlist(sf_sort(T, lex_order), T->count, T->sf_size);
    sf_free(T);
    return T1;
}
...
pset_family size_sort(T)
pset_family T;
{
    pset_family T1 = sf_unlist(sf_sort(T, descend), T->count, T->sf_size);
    sf_free(T);
    return T1;
}
...
pset_family mini_sort(F, compare)
pset_family F;
int (*compare)();
{
    register int *count, cnt, n = cube.size, i;
    register pset p, last;
    pset_family F_sorted;
    pset *F1;
 ...
 for(i = 0; i < n; i++)
     if ((p[(((i) >> 5) + 1)] & (1 << ((i) & (32 -1)))))
  cnt += count[i];
 (p[0] &= 0xffff, p[0] |= ((cnt) << 16));
    }
    if ((count)) { (void) free((char *) (count)); (count) = 0; };


    qsort((char *) (F1 = sf_list(F)), F->count, sizeof(pset), compare);
...
pset_family sort_reduce(T)
   pset_family T;
{
    register pset p, last, largest = ((void*)0);
    register int bestsize = -1, size, n = cube.num_vars;
    pset_family T_sorted;
    pset *T1;
 ...
 if ((size = set_ord(p)) > bestsize)
    ...
    for(p=T->data,last=p+T->count*T->wsize;p<last;p+=T->wsize)
 (p[0] &= 0xffff, p[0] |= ((((n - cdist(largest,p)) << 7) + ((set_ord(p)) < (127) ? (set_ord(p)) : (127))) << 16));

    qsort((char *) (T1 = sf_list(T)), T->count, sizeof(pset), descend);
...
pset_family random_order(F)
register pset_family F;
{
    pset temp;
    register int i, k;
    ...
    temp = set_clear(((unsigned int *) malloc(sizeof(unsigned int) * (((F->sf_size) <= 32 ? 2 : (((((F->sf_size)-1) >> 5) + 1) + 1))))), F->sf_size);
 ...
 set_copy(temp, ((F)->data + (F)->wsize * (k)));
 set_copy(((F)->data + (F)->wsize * (k)), ((F)->data + (F)->wsize * (i)));
 set_copy(((F)->data + (F)->wsize * (i)), temp);
    }
    if ((temp)) { (void) free((char *) (temp)); (temp) = 0; };
    return F;
}
...
{
    register pset *T1, p, seed, cof;
    pset *A1, *B1;
    int change;
    int count, numcube;

    numcube = (((pset *) T[1] - T) - 3);
 ...
 (p[0] &= ~ (0x0800));
    ...
    seed = set_copy(((unsigned int *) malloc(sizeof(unsigned int) * ((((32 * (T[2][0] & 0x03ff))) <= 32 ? 2 : ((((((32 * (T[2][0] & 0x03ff)))-1) >> 5) + 1) + 1))))), T[2]);
    cof = T[0];
    (T[2][0] |= (0x0800));
    count = 1;
 ...
 change = 0;
 for(T1 = T+2; (p = *T1++) != ((void*)0); ) {
     if (! (p[0] & (0x0800)) && ccommon(p, seed, cof)) {
  {register int i_=(seed[0] & 0x03ff); (seed[0] &= ~0x03ff, seed[0] |= (i_)); do seed[i_] = seed[i_] & p[i_]; while (--i_>0);};
  (p[0] |= (0x0800));
  change = 1;
  count++;
     }
    ...
    if ((seed)) { (void) free((char *) (seed)); (seed) = 0; };
 ...
 *A = A1 = ((pset *) malloc(sizeof(pset) * (numcube+3)));
 *B = B1 = ((pset *) malloc(sizeof(pset) * (numcube+3)));
 (*A)[0] = set_copy(((unsigned int *) malloc(sizeof(unsigned int) * ((((32 * (T[0][0] & 0x03ff))) <= 32 ? 2 : ((((((32 * (T[0][0] & 0x03ff)))-1) >> 5) + 1) + 1))))), T[0]);
 (*B)[0] = set_copy(((unsigned int *) malloc(sizeof(unsigned int) * ((((32 * (T[0][0] & 0x03ff))) <= 32 ? 2 : ((((((32 * (T[0][0] & 0x03ff)))-1) >> 5) + 1) + 1))))), T[0]);
 A1 = *A + 2;
 B1 = *B + 2;
...
pset_family cof_output(T, i)
pset_family T;
register int i;
{
    pset_family T1;
    register pset p, last, pdest, mask;

    mask = cube.var_mask[cube.output];
    T1 = sf_new(T->count, cube.size);
    for(p=T->data,last=p+T->count*T->wsize;p<last;p+=T->wsize) {
 if ((p[(((i) >> 5) + 1)] & (1 << ((i) & (32 -1))))) {
     pdest = ((T1)->data + (T1)->wsize * (T1->count++));
     {register int i_=(p[0] & 0x03ff); (pdest[0] &= ~0x03ff, pdest[0] |= (i_)); do pdest[i_] = p[i_] | mask[i_]; while (--i_>0);};
     (pdest[0] &= ~ (0x8000));
 }
    }
    return T1;
...
pset_family uncof_output(T, i)
pset_family T;
int i;
{
    register pset p, last, mask;
 ...
 return T;
    ...
    mask = cube.var_mask[cube.output];
    for(p=T->data,last=p+T->count*T->wsize;p<last;p+=T->wsize) {
 {register int i_=(p[0] & 0x03ff); (p[0] &= ~0x03ff, p[0] |= (i_)); do p[i_] = p[i_] & ~ mask[i_]; while (--i_>0);};
 (p[(((i) >> 5) + 1)] |= 1 << ((i) & (32 -1)));
    }
    return T;
}
# 351 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Benchmarks/MallocBench/espresso/cvrm.c"
...
int i;
{
    ...
    {long t=util_cpu_time();PLA->F = espresso(PLA->F, PLA->D, PLA->R);if(summary)print_trace(PLA->F,word,util_cpu_time()-t);};
    return 1;
}
...
int i;
{
    ...
    {long t=util_cpu_time();PLA->F = minimize_exact(PLA->F, PLA->D, PLA->R, 1);if(summary)print_trace(PLA->F,word,util_cpu_time()-t);};
    return 1;
}
...
int i;
{
    ...
    skip_make_sparse = 1;
    {long t=util_cpu_time();PLA->R = espresso(PLA->R, PLA->D, PLA->F);if(summary)print_trace(PLA->R,word,util_cpu_time()-t);};
...
int i;
{
    ...
    skip_make_sparse = 1;
    {long t=util_cpu_time();PLA->R = minimize_exact(PLA->R, PLA->D, PLA->F, 1);if(summary)print_trace(PLA->R,word,util_cpu_time()-t);};
