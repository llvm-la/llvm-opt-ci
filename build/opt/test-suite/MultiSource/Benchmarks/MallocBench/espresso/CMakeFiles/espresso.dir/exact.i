# 1 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Benchmarks/MallocBench/espresso/exact.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 398 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Benchmarks/MallocBench/espresso/exact.c" 2
...
# 2 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Benchmarks/MallocBench/espresso/exact.c" 2


static void dump_irredundant();
static pset_family do_minimize();
# 17 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Benchmarks/MallocBench/espresso/exact.c"
pset_family
minimize_exact(F, D, R, exact_cover)
pset_family F, D, R;
int exact_cover;
{
    return do_minimize(F, D, R, exact_cover, 0);
}
...
int exact_cover;
{
    return do_minimize(F, D, R, exact_cover, 1);
}
...
int weighted;
{
 ...
 weights = ((int *) 0);
    ...
    {long t=util_cpu_time();cover=sm_minimum_cover(table,weights,heur,level);if(trace)print_trace(F,"MINCOV     ",util_cpu_time()-t);};
    if (weights != 0) {
 if ((weights)) { (void) free((char *) (weights)); (weights) = 0; };
    }
 ...
 newF = sf_addset(newF, ((F)->data + (F)->wsize * (pe->col_num)));
    }
...
sm_matrix *table;
{
 ...
 file = ((char *) malloc(sizeof(char) * (strlen(filename)+20)));
 (void) sprintf(file, "%s.primes", filename);
 if ((fp_primes = fopen(file, "w")) == ((void*)0)) {
     fprintf(stderr, "espresso: Unable to open %s\n", file);
     fp_primes = stdout;
 }
 (void) sprintf(file, "%s.pi", filename);
 if ((fp_pi_table = fopen(file, "w")) == ((void*)0)) {
     fprintf(stderr, "espresso: Unable to open %s\n", file);
     fp_pi_table = stdout;
 }
 if ((file)) { (void) free((char *) (file)); (file) = 0; };
    }

    PLA = new_PLA();
    PLA_labels(PLA);

    fpr_header(fp_primes, PLA, 1);
    free_PLA(PLA);
 ...
 (void) fprintf(fp_primes, "%s\n", pc1(p));
    ...
    for(p=Rt->data,last=p+Rt->count*Rt->wsize;p<last;p+=Rt->wsize) {
 (void) fprintf(fp_primes, "%s\n", pc1(p));
    }
