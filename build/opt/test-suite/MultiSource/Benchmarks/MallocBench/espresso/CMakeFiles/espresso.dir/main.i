# 1 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Benchmarks/MallocBench/espresso/main.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 398 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Benchmarks/MallocBench/espresso/main.c" 2
...
# 8 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Benchmarks/MallocBench/espresso/main.c" 2
# 1 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Benchmarks/MallocBench/espresso/main.h" 1
enum keys {
...
struct {
    ...
    "ESPRESSO", KEY_ESPRESSO, 1, 1, 1,
    "many", KEY_MANY_ESPRESSO, 1, 1, 1,
...
# 9 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Benchmarks/MallocBench/espresso/main.c" 2
...
char *argv[];
{
    ...
    extern char *optarg;
    extern int optind;
# 47 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Benchmarks/MallocBench/espresso/main.c"
    start = util_cpu_time();
    ...
    while ((i = espresso_getopt(argc, argv, "D:S:de:o:r:stv:x")) != (-1)) {
 switch(i) {
     case 'D':
 ...
 break;
    ...
    case KEY_expand:
 {long t=util_cpu_time();PLA->F=expand(PLA->F,PLA->R,0);totals(t,4,PLA->F,&(cost));};
 break;

    case KEY_irred:
 {long t=util_cpu_time();PLA->F = irredundant(PLA->F, PLA->D);totals(t,5,PLA->F,&(cost));};
 break;

    case KEY_reduce:
 {long t=util_cpu_time();PLA->F = reduce(PLA->F, PLA->D);totals(t,6,PLA->F,&(cost));};
 break;

    case KEY_essen:
 for(p=PLA->F->data,last1=p+PLA->F->count*PLA->F->wsize;p<last1;p+=PLA->F->wsize) {
     (p[0] |= (0x0400));
     (p[0] &= ~ (0x4000));
 }
 ...
 break;
    ...
    case KEY_primes:
 {long t=util_cpu_time();PLA->F = primes_consensus(cube2list(PLA->F, PLA->D));if(trace)print_trace(PLA->F,"PRIMES     ",util_cpu_time()-t);};
 ...
 break;
    ...
    case KEY_PLA_verify:
 {long t=util_cpu_time();error = PLA_verify(PLA, PLA1);totals(t,14,PLA->F,&(cost));};
 if (error) {
 ...
 print_trace(PLA->F, option_table[option].name, util_cpu_time()-start);
    ...
    if (print_solution) {
 {long t=util_cpu_time();fprint_pla(stdout, PLA, out_type);totals(t,15,PLA->F,&(cost));};
    }
...
int out_type;
{
 ...
 if (strcmp(fname, "-") == 0) {
     fp = stdin;
 } else if ((fp = fopen(argv[opt], "r")) == ((void*)0)) {
