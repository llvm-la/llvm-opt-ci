# 1 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Benchmarks/MallocBench/espresso/compl.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 398 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Benchmarks/MallocBench/espresso/compl.c" 2
# 18 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Benchmarks/MallocBench/espresso/compl.c"
...
# 19 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Benchmarks/MallocBench/espresso/compl.c" 2
...
static int compl_special_cases();
static pset_family compl_merge();
static void compl_d1merge();
static pset_family compl_cube();
static void compl_lift();
static void compl_lift_onset();
static void compl_lift_onset_complex();
static int simp_comp_special_cases();
static int simplify_special_cases();



pset_family complement(T)
pset *T;
{
    register pset cl, cr;
    register int best;
 ...
 if ((cl)) { (void) free((char *) (cl)); (cl) = 0; };
 if ((cr)) { (void) free((char *) (cr)); (cr) = 0; };
 if ((T[0])) { (void) free((char *) (T[0])); (T[0]) = 0; }; if ((T)) { (void) free((char *) (T)); (T) = 0; };;
    }
...
static int compl_special_cases(T, Tbar)
pset *T;
pset_family *Tbar;
{
 ...
 *Tbar = compl_cube(set_or(cof, cof, T[2]));
 ...
 if ((ceil)) { (void) free((char *) (ceil)); (ceil) = 0; };
...
# 160 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Benchmarks/MallocBench/espresso/compl.c"
static pset_family compl_merge(T1, L, R, cl, cr, var, lifting)
pset *T1;
pset_family L, R;
register pset cl, cr;
int var;
int lifting;
{
 ...
 case 3:
    ...
    if ((L1)) { (void) free((char *) (L1)); (L1) = 0; };
    if ((R1)) { (void) free((char *) (R1)); (R1) = 0; };
...
# 289 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Benchmarks/MallocBench/espresso/compl.c"
...
int var;
{
 ...
 if ((a[0] & (0x2000))) {
     ...
     {register int i_=(bcube[0] & 0x03ff); (lift[0] &= ~0x03ff, lift[0] |= (i_)); do lift[i_] = bcube[i_] & mask[i_]; while (--i_>0);};
     {register int i_=(a[0] & 0x03ff); (lift[0] &= ~0x03ff, lift[0] |= (i_)); do lift[i_] = a[i_] | lift[i_]; while (--i_>0);};
  ...
  if (cdist0(p, lift)) {
     ...
     {register int i_=(lift[0] & 0x03ff); do a[i_]=lift[i_]; while (--i_>=0);};
     (a[0] |= (0x2000));
nolift : ;
 }
    }
...
register pset *L1, *R1;
{
    ...
    for(pl = *L1, pr = *R1; (pl != ((void*)0)) && (pr != ((void*)0)); )
...
static pset_family compl_cube(p)
register pset p;
{
...
static int simp_comp_special_cases(T, Tnew, Tbar)
pset *T;
pset_family *Tnew;
pset_family *Tbar;
{
...
static int simplify_special_cases(T, Tnew)
pset *T;
pset_family *Tnew;
{
    register pset *T1, p, ceil, cof=T[0];
