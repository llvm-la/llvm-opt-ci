# 1 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Benchmarks/MallocBench/espresso/hack.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 398 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Benchmarks/MallocBench/espresso/hack.c" 2
...
# 2 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Benchmarks/MallocBench/espresso/hack.c" 2
...
pPLA PLA;
{
 ...
 PLA->D = d1merge(PLA->D, i);
    ...
    if ((compress)) { (void) free((char *) (compress)); (compress) = 0; };
}
...
# 276 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Benchmarks/MallocBench/espresso/hack.c"
map_symbolic(PLA)
pPLA PLA;
{
    symbolic_t *p1;
...
pset_family map_symbolic_cover(T, list, base)
pset_family T;
symbolic_list_t *list;
int base;
{
...
static pset_family fsm_simplify(F)
pset_family F;
{
    pset_family D, R;
...
int verbose_mode;
{
 ...
 for(p=PLA->F->data,last=p+PLA->F->count*PLA->F->wsize;p<last;p+=PLA->F->wsize) {
  ...
  if ((p[(((next_state + i) >> 5) + 1)] & (1 << ((next_state + i) & (32 -1))))) {
      tF = sf_addset(tF, p);
  }
     }
 }
 ...
 for(j = 0; j < nstates; j++) {
  ...
  if (verbose_mode) {
    ...
    if ((state_mask)) { (void) free((char *) (state_mask)); (state_mask) = 0; };
    if ((present_state_mask)) { (void) free((char *) (present_state_mask)); (present_state_mask) = 0; };
    if ((next_state_mask)) { (void) free((char *) (next_state_mask)); (next_state_mask) = 0; };
