# 1 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Benchmarks/MallocBench/espresso/essen.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 398 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Benchmarks/MallocBench/espresso/essen.c" 2
...
# 7 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Benchmarks/MallocBench/espresso/essen.c" 2
# 27 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Benchmarks/MallocBench/espresso/essen.c"
pset_family essential(Fp, Dp)
   pset_family *Fp, *Dp;
{
    register pset last, p;
    pset_family E, F = *Fp, D = *Dp;
...
# 71 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Benchmarks/MallocBench/espresso/essen.c"
int essen_cube(F, D, c)
   pset_family F, D;
   pset c;
{
    pset_family H, FD;
    ...
    if ((H1[0])) { (void) free((char *) (H1[0])); (H1[0]) = 0; }; if ((H1)) { (void) free((char *) (H1)); (H1) = 0; };;
...
pset_family cb_consensus(T, c)
register pset_family T;
register pset c;
{
    register pset temp, last, p;
    register pset_family R;
...
pset_family cb_consensus_dist0(R, p, c)
pset_family R;
register pset p, c;
{
    int var;
    int got_one;
    register pset temp, mask;
    register pset p_diff_c=cube.temp[0], p_and_c=cube.temp[1];
 ...
 return R;
    ...
    temp = set_clear(((unsigned int *) malloc(sizeof(unsigned int) * (((cube.size) <= 32 ? 2 : (((((cube.size)-1) >> 5) + 1) + 1))))), cube.size);
    got_one = 0;
    {register int i_=(p[0] & 0x03ff); (p_diff_c[0] &= ~0x03ff, p_diff_c[0] |= (i_)); do p_diff_c[i_] = p[i_] & ~ c[i_]; while (--i_>0);};
    {register int i_=(p[0] & 0x03ff); (p_and_c[0] &= ~0x03ff, p_and_c[0] |= (i_)); do p_and_c[i_] = p[i_] & c[i_]; while (--i_>0);};

    for(var = cube.num_binary_vars; var < cube.num_vars; var++) {

 mask = cube.var_mask[var];
 if (! setp_disjoint(p_diff_c, mask)) {
     {register int i_=(c[0] & 0x03ff); (temp[0] &= ~0x03ff, temp[0] |= (i_)); do temp[i_] = (c[i_]&mask[i_]) | (p_and_c[i_]&~mask[i_]); while (--i_>0);};
     R = sf_addset(R, temp);
     got_one = 1;
 }
    }
 ...
 {register int i_=(p[0] & 0x03ff); (temp[0] &= ~0x03ff, temp[0] |= (i_)); do temp[i_] = p[i_] & c[i_]; while (--i_>0);};
 R = sf_addset(R, temp);
    }
