# 1 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Benchmarks/MallocBench/espresso/espresso.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 398 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Benchmarks/MallocBench/espresso/espresso.c" 2
# 39 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Benchmarks/MallocBench/espresso/espresso.c"
...
# 40 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Benchmarks/MallocBench/espresso/espresso.c" 2

pset_family espresso(F, D1, R)
pset_family F, D1, R;
{
    pset_family E, D, Fsave;
    pset last, p;
...
begin:
    ...
    if (recompute_onset) {
 {long t=util_cpu_time();E = simplify(cube1list(F));if(trace)print_trace(E,"SIMPLIFY   ",util_cpu_time()-t);};
 sf_free(F);
 F = E;
    }
    cover_cost(F, &cost);
    if (unwrap_onset && (cube.part_size[cube.num_vars - 1] > 1)
      ...
      && (cost.out < 5000))
 {long t=util_cpu_time();F = sf_contain(unravel(F, cube.num_vars - 1));if(trace)print_trace(F,"SETUP      ",util_cpu_time()-t);};


    for(p=F->data,last=p+F->count*F->wsize;p<last;p+=F->wsize) {
 (p[0] &= ~ (0x8000));
    }
    {long t=util_cpu_time();F = expand(F, R, 0);totals(t,4,F,&(cost));};
    {long t=util_cpu_time();F = irredundant(F, D);totals(t,5,F,&(cost));};

    if (! single_expand) {
 if (remove_essential) {
     {long t=util_cpu_time();E = essential(&F, &D);totals(t,3,E,&(cost));};
 } else {
 ...
 do {
  ...
  {long t=util_cpu_time();F = reduce(F, D);totals(t,6,F,&(cost));};
  {long t=util_cpu_time();F = expand(F, R, 0);totals(t,4,F,&(cost));};
  {long t=util_cpu_time();F = irredundant(F, D);totals(t,5,F,&(cost));};
