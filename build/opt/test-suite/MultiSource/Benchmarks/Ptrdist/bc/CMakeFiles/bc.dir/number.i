# 1 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Benchmarks/Ptrdist/bc/number.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 399 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Benchmarks/Ptrdist/bc/number.c" 2
# 29 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Benchmarks/Ptrdist/bc/number.c"
...
# 34 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Benchmarks/Ptrdist/bc/bcdefs.h" 2
...
# 30 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Benchmarks/Ptrdist/bc/number.c" 2
...
int yyparse (void);
int yylex (void);
# 31 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Benchmarks/Ptrdist/bc/number.c" 2
...
num2long (bc_num num)
{
  ...
  val = 0;
  nptr = &num->n_value[0];
  for (index=num->n_len; (index>0) && (val<=(9223372036854775807L/10)); index--)
    val = val*10 + *nptr++;
...
# 196 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Benchmarks/Ptrdist/bc/number.c"
static int
_do_compare (bc_num n1, bc_num n2, int use_sign, int ignore_last)
{
      ...
      else
 {
  ...
  count = n1->n_len + (n1->n_scale>n2->n_scale?n2->n_scale:n1->n_scale);
  n1ptr = &n1->n_value[0];
  n2ptr = &n2->n_value[0];
...
_do_add (bc_num n1, bc_num n2)
{
  ...
  sum_scale = (n1->n_scale>n2->n_scale?n1->n_scale:n2->n_scale);
  sum_digits = (n1->n_len>n2->n_len?n1->n_len:n2->n_len) + 1;
...
_do_sub (bc_num n1, bc_num n2)
{
  ...
  diff_len = (n1->n_len>n2->n_len?n1->n_len:n2->n_len);
  diff_scale = (n1->n_scale>n2->n_scale?n1->n_scale:n2->n_scale);
  min_len = (n1->n_len>n2->n_len?n2->n_len:n1->n_len);
  min_scale = (n1->n_scale>n2->n_scale?n2->n_scale:n1->n_scale);
...
bc_multiply (bc_num n1, bc_num n2, bc_num * prod, int scale)
{
  ...
  prod_scale = (full_scale>(scale>(n1->n_scale>n2->n_scale?n1->n_scale:n2->n_scale)?scale:(n1->n_scale>n2->n_scale?n1->n_scale:n2->n_scale))?(scale>(n1->n_scale>n2->n_scale?n1->n_scale:n2->n_scale)?scale:(n1->n_scale>n2->n_scale?n1->n_scale:n2->n_scale)):full_scale);
  ...
  for (indx = 0; indx < toss; indx++)
    {
      n1ptr = n1end - (0>indx-len2+1?0:indx-len2+1);
      n2ptr = n2end - (indx>len2-1?len2-1:indx);
...
bc_divide (bc_num n1, bc_num n2, bc_num * quot, int scale)
{
      ...
      if (n2->n_len == 1 && n2->n_value[0] == 1)
 {
   ...
   memcpy (qval->n_value, n1->n_value,
    n1->n_len + (n1->n_scale>scale?scale:n1->n_scale));
   free_num (quot);
   *quot = qval;
 }
    }
...
bc_raise (bc_num num1, bc_num num2, bc_num * result, int scale)
{
   ...
   if (exponent < 0)
     ...
     {
       neg = 0;
       rscale = (num1->n_scale*exponent>(scale>num1->n_scale?scale:num1->n_scale)?(scale>num1->n_scale?scale:num1->n_scale):num1->n_scale*exponent);
     }
   temp = copy_num (_one_);
   power = copy_num (num1);
...
bc_sqrt (bc_num * num, int scale)
{
      ...
      if (cmp_res == 0)
 {
  ...
  rscale = (scale>(*num)->n_scale?scale:(*num)->n_scale);
  cscale = rscale + 2;
  init_num (&guess);
  init_num (&guess1);
  point5 = new_num (1,1);
  ...
  while (!done)
    {
      ...
      cmp_res = _do_compare (guess,guess1,0,1);
      if (cmp_res == 0) done = 1;
    }
