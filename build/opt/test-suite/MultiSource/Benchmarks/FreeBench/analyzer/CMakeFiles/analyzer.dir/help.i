# 1 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Benchmarks/FreeBench/analyzer/help.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 399 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Benchmarks/FreeBench/analyzer/help.c" 2
...
# 2 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/MultiSource/Benchmarks/FreeBench/analyzer/help.c" 2
...
void show_help(char *prog_name)
{
  ...
  fprintf(stderr,"Questions about this software should be directed to biff@ce.chalmers.se\n\n");
  fprintf(stderr,"To determine the data hazards between loop iterations this program analyzes\n");
  fprintf(stderr,"trace data from a run of the program examined. The program also analyzes the\n");
  fprintf(stderr,"potential speedup obtainable from running the loop iterations in parallel.\n\n");
  fprintf(stderr,"The program requires a number of settings to be done either on the\n");
  fprintf(stderr,"command line or in the settings file.\n\n");
  fprintf(stderr,"Available flags. Many of these can be set in the settings file.\n");;
  fprintf(stderr,"-c #      : CPU limit. Only effective with 'realistic restart'. 0==unlimited.\n");;
  fprintf(stderr,"-C #      : CPU limit for run through several configurations.\n");;
  fprintf(stderr,"-d        : Check for DATA dependencies (RAW)\n");;
  fprintf(stderr,"-e #      : Epoch length (number of loop iterations per epoch).\n");;
  fprintf(stderr,"-E        : Use epoch length as total number of epochs\n");;
  fprintf(stderr,"-f        : Do NOT use forwarding in speedup analysis\n");;
  fprintf(stderr,"-g #      : Specify Thread start penalty\n");;
  fprintf(stderr,"-G #      : Specify Commit penalty\n");;
  fprintf(stderr,"-h        : Show complete help message\n");;
  fprintf(stderr,"-i #      : Print SpeedUp for 1 - Loop, 2 - Program, 3 - Both.\n");;
  fprintf(stderr,"-k #      : The test kernel to be used in the test.\n");;
  fprintf(stderr,"            1 - Optimum restart. 2 - Realistic restart. 3 - Both\n");;
  fprintf(stderr,"-l #      : Max length of hazard top-list. -- NOT IMPLEMENTED --\n");;
  fprintf(stderr,"-m        : Analyze instruction mix\n");;
  fprintf(stderr,"-n        : Check for NAME dependencies (WAR & WAW) -- NOT IMPLEMENTED --\n");;
  fprintf(stderr,"-o <FILE> : Output graphing data to FILE\n");;
  fprintf(stderr,"-p #      : Specify load penalty\n");;
  fprintf(stderr,"-P #      : Specify store penalty\n");;
  fprintf(stderr,"-q        : Quiet mode (do not print processing status)\n");;
  fprintf(stderr,"-s        : Do speedup analysis\n");;
  fprintf(stderr,"-S        : Do speedup analysis, start early\n");;
  fprintf(stderr,"-t #      : Specify the size of the def_hash_table.\n");;
  fprintf(stderr,"-x <FILE> : Use other settings file. DEFAULT: analyzer.conf\n");;
  fprintf(stderr,"-X        : Do not read settings from a file\n");;
  fprintf(stderr,"\n");;
}
