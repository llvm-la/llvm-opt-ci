# 1 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/SingleSource/Benchmarks/McGill/chomp.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 399 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/SingleSource/Benchmarks/McGill/chomp.c" 2
...
# 2 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/SingleSource/Benchmarks/McGill/chomp.c" 2
...
# 3 "/home/date/work/actions-runner/_work/llvm-opt-ci/llvm-opt-ci/repos/llvm-test-suite/SingleSource/Benchmarks/McGill/chomp.c" 2
...
int *data;
{
  int *new = (int *)malloc(ncol * sizeof(int));
  int counter = ncol;
  while (counter --)
...
int in_wanted(int *data)
{
  ...
  while (current != ((void*)0))
    {
      if (equal_data(current -> data,data)) break;
...

{
  ...
  *value = 1;
  ...
  current = head -> next;
  free(head);
  if (current != ((void*)0)) *value = 1 - *value;
  return current;
}
...
struct _play *make_play(int all)
{
  ...
  while (next_data(temp))
   ...
   if (game_tree == ((void*)0)) game_tree = current -> next;
...
{
  int *temp;
  while (play -> next != ((void*)0)) play = play -> next;
  ...
  switch (row)
    {
      case 1:
        ...
        for (;nrow <= maxrow;nrow ++)
          {
            make_wanted(make_data(nrow,ncol));
            tree = make_play(0);
            win = get_winning_move(tree);
